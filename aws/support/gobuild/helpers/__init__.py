...
BRANCH_REGEX = re.compile(r'^[^ \t]*[ \t]*(//.+/)build/([^/]+)/'
                          r'gobuild-starter-kit/support/gobuild/')
DEFAULT_DEPOT = '//depot/'
...
def GetBranch():
    ...
    For git repo, GetBranch is not required, we can just define the default branch name to 'master'.
    """
    return 'master'
