#! /bin/bash
cat << DELIMIT > /home/ubuntu/air-gapped/user-airgapped.env
...
#CLI_DIR=/home/ubuntu/packages/cli
...
if [[ ${fips_enabled} == "true" ]]; then
	echo -e "TKG_CUSTOM_COMPATIBILITY_IMAGE_PATH=fips/tkg-compatibility" >> /home/ubuntu/air-gapped/user-airgapped.env
fi
# hostname and password for tkg extensions if not set then don't use it
if [[ -n "${harbor_host_name}" ]]; then
	echo -e "HARBOR_HOSTNAME=${harbor_host_name}" >> /home/ubuntu/air-gapped/user-airgapped.env
fi
if [[ -n "${prometheus_host_name}" ]]; then
	echo -e "PROMETHEUS_HOSTNAME=${prometheus_host_name}" >> /home/ubuntu/air-gapped/user-airgapped.env
fi
if [[ -n "${grafana_host_name}" ]]; then
	echo -e "GRAFANA_HOSTNAME=${grafana_host_name}" >> /home/ubuntu/air-gapped/user-airgapped.env
fi
if [[ -n "${harbor_extension_password}" ]]; then
	echo -e "HARBOR_PASSWORD=${harbor_extension_password}" >> /home/ubuntu/air-gapped/user-airgapped.env
fi
if [[ "${enable_identity_management}" == "true" ]]; then
  if [[ "${identity_management_type}" == "ldap" ]]; then
    echo -e "IDENTITY_MANAGEMENT_TYPE=${identity_management_type}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "LDAP_HOST=${ldap_host}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "LDAP_USER_SEARCH_BASE_DN=${ldap_user_search_base_dn}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "LDAP_GROUP_SEARCH_BASE_DN=${ldap_group_search_base_dn}" >> /home/ubuntu/air-gapped/user-airgapped.env
  elif [[ "${identity_management_type}" == "oidc" ]]; then
    echo -e "IDENTITY_MANAGEMENT_TYPE=${identity_management_type}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_CLIENT_ID=${oidc_identity_provider_client_id}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_CLIENT_SECRET=${oidc_identity_provider_client_secret}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_GROUPS_CLAIM=${oidc_identity_provider_groups_claim}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_ISSUER_URL=${oidc_identity_provider_issuer_url}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_SCOPES=${oidc_identity_provider_scopes}" >> /home/ubuntu/air-gapped/user-airgapped.env
    echo -e "OIDC_IDENTITY_PROVIDER_USERNAME_CLAIM=${oidc_identity_provider_username_claim}" >> /home/ubuntu/air-gapped/user-airgapped.env
  else
    echo "Invalid identity management type provided for Pinniped - ${identity_management_type}"
  fi
fi
cd /home/ubuntu/air-gapped
