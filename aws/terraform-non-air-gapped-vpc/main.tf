...
module "management_vpc" {
  source        = "./infra_create"
  vpc_subnet    = "${var.control_plane_cidr == "" ? "172.16.0.0/16" : var.control_plane_cidr}"
  transit_gw    = aws_ec2_transit_gateway.transitgw.id
  transit_block = "${var.worker_cidr == "" ? "172.19.0.0/16" : var.worker_cidr}"
...
module "workload_vpc" {
  source        = "./infra_create"
  vpc_subnet    = "${var.worker_cidr == "" ? "172.19.0.0/16" : var.worker_cidr}" // avoiding .17 and .18 so that we don't conflict with docker
  transit_gw    = aws_ec2_transit_gateway.transitgw.id
  transit_block = "${var.control_plane_cidr == "" ? "172.16.0.0/16" : var.control_plane_cidr}"
  name          = "workload-vpc"
