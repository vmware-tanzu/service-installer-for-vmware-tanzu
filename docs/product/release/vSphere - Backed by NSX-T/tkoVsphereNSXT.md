...
7. Use the following command to cleanup the deployment.
   ...
   **Note:** If you interrupt the deployment process (i.e. using a `ctrl-c`), you need to restart Service Installer to properly refresh the service. You can do this with `systemctl restart arcas`.
   ...
   | --load_tanzu_image_to_harbor     | This helps load the TKG dependencies to Harbor which is pre-bundled with SIVT.
...
- User must download Service Installer for VMware Tanzu OVA (`service-installer-for-VMware-Tanzu-with-Harbor.ova`) file from Marketplace. This OVA comes bundled with Tanzu Kubernetes Grid 2.1.1 dependencies which are not compliant.

**Note:** For Tanzu Kubernetes Grid compliant dependency upload, complete the following steps.

- Download the available tanzu_21.tar file from marketplace (TKG dependency). This is compliant dependency and must be uploaded to Harbor.
    
  **Note:** For Tanzu Kubernetes Grid dependency upload, complete the following steps.

1. Download the Service Installer for VMware Tanzu OVA file which includes pre-bundled Harbor: `service-installer-for-VMware-Tanzu-with-Harbor.ova`
2. Download the dependency tar file 'tanzu_21.tar' which is available for download in Marketplace along with the Service Installer for VMware Tanzu OVA file and copy it to the following location on the Service Installer for VMware Tanzu VM: `/opt/vmware/arcas/tools`
3. Run the following command:

   - `arcas --load_tanzu_image_to_harbor --repo_name=tanzu_21 --tkg_binaries_path /opt/vmware/arcas/tools/tanzu_21.tar`

   -  It will take some time to load all the images which are around 17 GB in size.
4. Log in to Harbor at `https://<sivt-ip>:9443`
    - Credential -> user:admin password:`<sivt password>`
5. Verify that all binaries are uploaded to Harbor with the mentioned repo name `tanzu_21`.
