...
class GenerateTektonDockerImage:
    ...
    def get_meta_details_marketplace(self):
        ...
        try:
            ...
            payload = {"refreshToken": self.reftoken}
            json_object = json.dumps(payload, indent=4)
            sess = requests.request(
                "POST", MarketPlaceUrl.URL + "/api/v1/user/login", headers=headers, data=json_object
            )
            if sess.status_code != 200:
                return None, "Failed to login and obtain csp-auth-token"
            else:
                self.token = sess.json()["access_token"]
            headers = {"Accept": "application/json", "Content-Type": "application/json", "csp-auth-token": self.token}
            slug = "true"
            ...
            for metalist in product.json()["response"]["data"]["metafilesList"]:
                if metalist["appversion"] == self.tkg_version:
                    if grp == "Kubectl Cluster CLI":
                        if (
                            metalist["version"] == self.kube_version[1:]
                            and str(metalist["groupname"]).strip("\t") == grp
                        ):
                        ...
                        if str(metalist["groupname"]).strip("\t") == grp:
                            ...
                            break
            print(
                "ovaName: {ovaName} app_version: {app_version} metafileid: {metafileid}".format(
                    ovaName=file_name, app_version=app_version, metafileid=metafileid
                )
            )
            meta_dict.update(
                {
    ...
    def download_files_from_marketplace(self, meta_info):
        ...
        try:
            ...
            url = MarketPlaceUrl.URL
            presigned_url = requests.request(
                "POST",
                url + "/api/v1/products/" + meta_info["product_id"] + "/download",
                headers=headers,
                data=json_object,
            )
            ...
            # curl_inspect = f"curl -I -X GET {download_url} --output /tmp/resp.txt"
            URL = download_url
            response = requests.get(URL)
            open("/tmp/resp.txt", "wb").write(response.content)
    ...
    def get_product_slug_id(product_name, headers) -> tuple:
        ...
        """
        try:
            product = requests.get(MarketPlaceUrl.PRODUCT_SEARCH_URL, headers=headers)
            if product.status_code != 200:
