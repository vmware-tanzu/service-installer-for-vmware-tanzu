...
class KubectlClient:
    ...
    def get_cluster_template_json(self, cluster_name):
        logger.info(f"Getting machine template for {cluster_name}")
        # kubectl get cluster clustername -o json
        get_template = KubectlCommands.GET_CLUSTER_DEPLOYMENT.format(cluster_name=cluster_name)
        return self.rcmd.run_cmd_output(get_template)
