...
from util.common_utils import getVersionOfPackage, switchToContext, loadBomFile, \
     ...
     checkToEnabled, checkFluentBitInstalled, deploy_fluent_bit, checkExtentionDeployed, checkRepositoryAdded
from util.ShellHelper import runShellCommandAndReturnOutputAsList, \
    verifyPodsAreRunning, runShellCommandAndReturnOutput, runProcess
...
class deploy_tkg_extensions():
    ...
    def fluent_bit(self, fluent_bit_type):
        status_ = checkRepositoryAdded(self.jsonspec)
        if status_[1] != 200:
...
def generateYamlFile(extention, version, certKey_Path, cert_Path, fqdn, secret, yaml_file_name, skip_tls_verify=False):
    extention = extention.lower()
    get_repo = ["kubectl", "-n", TKG_Package_Details.NAMESPACE, "get", "packages",
    ...
    if skip_tls_verify:
        generate_file = ["imgpkg", "pull", "-b", get_repo_state[0].replace("'", "").strip(), "-o",
                         "/tmp/" + extention + "-package", "--registry-verify-certs=false"]
    else:
        generate_file = ["imgpkg", "pull", "-b", get_repo_state[0].replace("'", "").strip(), "-o",
        ...
        if enable == "true":
            ...
            if not verifyPodsAreRunning(appName, command_fluent_bit[0],
                ...
                if version is None:
                    ...
                    return json.dumps(d), 500
                if checkAirGappedIsEnabled(jsonspec):
                    file_status = generateYamlFile(extention, version, certKey_Path, cert_Path, fqdn, password,
                                                   yamlFile, skip_tls_verify=True)
                else:
                    file_status = generateYamlFile(extention, version, certKey_Path, cert_Path, fqdn, password,
                                                   yamlFile)
                ...
                if Upgrade_Extensions.UPGRADE_EXTN:
                    ...
                    deply_extension_command = ["tanzu", "package", "install", extention.lower(), "--package",
                                               extention.lower() + ".tanzu.vmware.com", "--version", version,
                                               "--values-file", yamlFile, "--namespace", extra_ns]
