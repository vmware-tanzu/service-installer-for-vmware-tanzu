...
from util.common_utils import (
    ...
    getClusterID,
    getVersionOfPackage,
    installExtentionFor14,
    isClusterRunning,
    waitForGrepProcessWithoutChangeDir,
)
from util.extensions_helper import checkPromethusEnabled, checkTanzuExtensionEnabled
from util.logger_helper import LoggerHelper
from util.shared_config import certChanging
from util.ShellHelper import (
    grabPipeOutput,
    runProcess,
...
from src.common.util.kubectl_util import KubectlUtil
...
def deploy_tkgs_extensions(jsonspec):
    try:
        ...
        if checkTanzuExtensionEnabled(jsonspec):
            ...
            config_kubectl = kubectl_util.configure_kubectl(cluster_endpoint)
            if config_kubectl[1] != 200:
                return config_kubectl[0], 500
            ####################
            ...
            if deploy_ext[1] != 200:
                d = {"responseType": "ERROR", "msg": deploy_ext[1], "ERROR_CODE": 500}
                return json.dumps(d), 500
            d = {"responseType": "SUCCESS", "msg": "Extensions deployed successfully", "ERROR_CODE": 200}
            return json.dumps(d), 200
        else:
            logger.info("Extensions are not enabled")
            d = {"responseType": "SUCCESS", "msg": "Extensions are not enabled", "ERROR_CODE": 200}
            return json.dumps(d), 200
    except Exception as e:
        logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while deploying extensions", "ERROR_CODE": 500}
        return json.dumps(d), 500


def tkgsExtensionsPrecheck(vcenter_ip, vcenter_username, password, cluster, jsonspec):
    # Creating cluster folder
    workload_name = jsonspec["tkgsComponentSpec"]["tkgsVsphereNamespaceSpec"]["tkgsVsphereWorkloadClusterSpec"][
...
def generateYamlFile(extention, version, certKey_Path, cert_Path, fqdn, secret, yaml_file_name):
    ...
    if cert_Path and certKey_Path:
        ...
        if state_harbor_change_host_password_cert[1] == 500:
            logger.error("Failed to change  cert " + str(state_harbor_change_host_password_cert[0]))
            d = {
                "responseType": "ERROR",
...
def checkClusterRoleStatus():
    try:
        ...
        if output[1] == 0:
            if output[0].__contains__("arcas-automation-authenticated-user-privileged-binding") and output[
                0
            ].__contains__("ClusterRole/psp:vmware-system-privileged"):
    ...
    if not verifyPodsAreRunning("tanzu-standard", repo_output[0], RegexPattern.RECONCILE_SUCCEEDED):
        ...
        for line in repo_output[0]:
            if line.__contains__(repo_string) and line.__contains__("tanzu-standard"):
                logger.warn("Standard Package Repository is already added but " "it's not in Reconcile Succeed status")
                return False
    else:
        return True
    ...
    if checkTmcEnabled(jsonspec):
        ...
        if not isRepoConfigured():
            ...
            deploy_command = [
                ...
                "tanzu-standard",
                "--url",
                Tkgs_Extension_Details.PACKAGE_REPO_URL,
                "-n",
                "tanzu-package-repo-global",
            ]
...
def deploy_extensions(cluster_name, jsonspec):
    try:
        ...
        if status[1] != 200:
            d = {"responseType": "ERROR", "msg": "Failed to deploy extension" + str(status[0]), "ERROR_CODE": 500}
            return json.dumps(d), 500
        if str(checkHarborEnabled).lower() == "true":
            ...
            if state[1] != 200:
                d = {"responseType": "ERROR", "msg": state[0], "ERROR_CODE": 500}
                return json.dumps(d), 500
            else:
                logger.info(state[0])
            ...
            if len(listOfExtention) == 0:
                ...
                for extension_name in listOfExtention:
                    ...
                    if monitor_status[1] != 200:
                        ...
                        d = {"responseType": "ERROR", "msg": monitor_status[0], "ERROR_CODE": 500}
                        return json.dumps(d), 500
        ...
        if is_enabled[0]:
            ...
            if not is_deployed[0]:
                ...
                if response[1] != 200:
                    ...
                    d = {"responseType": "ERROR", "msg": response[0], "ERROR_CODE": 500}
                    return json.dumps(d), 500
    ...
    try:
        ...
        if enable.lower() == "true":
            if monitoringType == Tkg_Extention_names.PROMETHEUS:
                ...
                cert_Path = jsonspec["tanzuExtensions"]["monitoring"]["prometheusCertPath"]
                fqdn = jsonspec["tanzuExtensions"]["monitoring"]["prometheusFqdn"]
                certKey_Path = jsonspec["tanzuExtensions"]["monitoring"]["prometheusCertKeyPath"]
                ...
                certKey_Path = jsonspec["tanzuExtensions"]["monitoring"]["grafanaCertKeyPath"]
            ...
            if not verifyPodsAreRunning(appName, command_fluent_bit[0], RegexPattern.RECONCILE_SUCCEEDED):
                ...
                if version is None:
                    ...
                    d = {"responseType": "ERROR", "msg": "Capture the available Prometheus version", "ERROR_CODE": 500}
                    return json.dumps(d), 500
                ...
                if update_sc_response[1] != 200:
                    d = {"responseType": "ERROR", "msg": update_sc_response[0], "ERROR_CODE": 500}
                    return json.dumps(d), 500
                ...
                if state_extention_apply[1] != 0:
                    logger.error(extention + "install command failed. Checking for reconciliation " "status...")
                ...
                if not found:
                    ...
                    d = {"responseType": "SUCCESS", "msg": appName + " is deployed", "ERROR_CODE": 200}
                    return json.dumps(d), 200
            else:
                logger.info(appName + " is already running")
                d = {"responseType": "SUCCESS", "msg": appName + " is already running", "ERROR_CODE": 200}
                return json.dumps(d), 200
        else:
            logger.info("Monitoring extension deployment is not enabled")
            d = {"responseType": "SUCCESS", "msg": "Monitoring extension deployment is not enabled", "ERROR_CODE": 200}
            return json.dumps(d), 200

    except Exception as e:
        logger.info("Failed to  deploy monitoring " + str(e))
        d = {"responseType": "ERROR", "msg": "Failed to  deploy monitoring " + str(e), "ERROR_CODE": 500}
        return json.dumps(d), 500
...
def updateStorageClass(yamlFile, extension):
    try:
        ...
        if get_sc_output[1] != 0:
            ...
            d = {"responseType": "ERROR", "msg": "Command to get storage class name failed.", "ERROR_CODE": 500}
            return json.dumps(d), 500
        else:
        ...
        if sc is None:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to obtain storage class name for cluster", "ERROR_CODE": 500}
            return json.dumps(d), 500
        ...
        if inject_sc_response[1] == 500:
            ...
            d = {"responseType": "SUCCESS", "msg": extension + " yaml file updated successfully", "ERROR_CODE": 200}
            return json.dumps(d), 200
    except Exception as e:
        logger.error(str(e))
        d = {
...
def installHarborTkgs(harborCertPath, harborCertKeyPath, harborPassword, host, clusterName):
    ...
    if not (
        verifyPodsAreRunning(AppName.HARBOR, out[0], RegexPattern.RECONCILE_SUCCEEDED)
        ...
        if not verify_contour:
            ...
            d = {"responseType": "ERROR", "msg": "Contour is not running ", "ERROR_CODE": 500}
            return json.dumps(d), 500
        if not verify_cert_manager:
            logger.error("Cert manager is not running")
            d = {"responseType": "ERROR", "msg": "Cert manager is not running ", "ERROR_CODE": 500}
            return json.dumps(d), 500
        ...
        if status[1] != 0:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to get harbor image url " + str(status[0]), "ERROR_CODE": 500}
            return json.dumps(d), 500
        ...
        os.system(
            "cp /tmp/harbor-package/config/values.yaml " + Paths.CLUSTER_PATH + clusterName + "/harbor-data"
            "-values.yaml"
        )
        ...
        if cer[1] != 200:
            logger.error(cer[0])
            d = {"responseType": "ERROR", "msg": cer[0], "ERROR_CODE": 500}
            return json.dumps(d), 500
        os.system(f"chmod +x {Paths.INJECT_VALUE_SH}")

        update_sc_resp = updateStorageClass(
            Paths.CLUSTER_PATH + clusterName + "/harbor-data-values.yaml", AppName.HARBOR
        )
        update_sc_resp = json.loads(update_sc_resp[0]), update_sc_resp[1]
        if update_sc_resp[1] != 200:
            logger.error(update_sc_resp[0])
            d = {"responseType": "ERROR", "msg": update_sc_resp[0], "ERROR_CODE": 500}
            return json.dumps(d), 500
        else:
        ...
        if running:
            d = {"responseType": "SUCCESS", "msg": "Deployed harbor successfully", "ERROR_CODE": 200}
            return json.dumps(d), 200
            ...
            if overlay_status[0] is None:
                ...
                d = {"responseType": "ERROR", "msg": overlay_status[1], "ERROR_CODE": 500}
                return json.dumps(d), 500
        else:
        ...
        logger.info("Harbor is already deployed and it's status is - " + out[0].split()[4] + " " + out[0].split()[5])
        d = {
            "responseType": "SUCCESS",
            "msg": "Harbor is already deployed and it's status is - " + out[0].split()[4] + " " + out[0].split()[5],
            "ERROR_CODE": 200,
        }
        return json.dumps(d), 200
