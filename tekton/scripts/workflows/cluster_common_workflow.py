...
class ClusterCommonWorkflow:
    ...
    def get_all_non_mgmt_clusters_name(self):

        """
        run tanzu cluster list json
        cluster ast literal and parse for name
        :return:
        """
        clusters_info = self.tkg_cli_client.get_clusters()
        cls_dump = ast.literal_eval(str(clusters_info))
        non_mgmt_clusters_list = [cls['name'] for cls in cls_dump]
        return non_mgmt_clusters_list

    def get_mgmt_cluster_name(self):

        """
                execute tanzu mc get to know if the provided cluster is mgmt.
                :return: mgmt_cluster_name
        """
        all_cluster_info = self.tkg_cli_client.get_all_clusters()
        logger.debug(f"all cluster info: {all_cluster_info}")
        cls_dump = ast.literal_eval(str(all_cluster_info))
        logger.debug(f"cls_dump: {cls_dump}")
        for k in cls_dump:
            if "management" in k["roles"]:
                return k["name"]
        return None

    def is_mgmt_cluster(self, target_cluster):
        """
        execute tanzu mc get to know if the provided cluster is mgmt.
        :param cluster_name:
        :return: True, if mgmt cluster
                 False, if not
        """
        all_cluster_info = self.tkg_cli_client.get_all_clusters()
        cls_dump = ast.literal_eval(all_cluster_info)
        if "management" in next(item for item in cls_dump if item["name"] == target_cluster)["roles"]:
            return True
        else:
            return False
