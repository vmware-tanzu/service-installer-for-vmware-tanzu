...
class RaNSXTWorkflow:
    ...
    def configure_avi_nsxt_config(self):
        ...
        if arcas_fw[1] != 200:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to create firewall " + FirewallRuleCgw.DISPLAY_NAME_VCF_ARCAS_BACKEND + " " + str(
                    arcas_fw[0]['msg']),
                "ERROR_CODE": 500
            }
            return json.dumps(d), 500
        ...
        if fw[1] != 200:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to create firewall " + FirewallRuleCgw.DISPLAY_NAME_VCF_TKG_and_AVI_DNS + " " + str(
                    fw[0]['msg']),
                "ERROR_CODE": 500
            }
            return json.dumps(d), 500
        ...
        fw_vip = self.nsxObj.createFirewallRule(Policy_Name.POLICY_NAME, "Alb",
                                    payload)
        if fw_vip[1] != 200:
            logger.error(
            ...
            return json.dumps(d), 500
        nsx_fqdn = str(self.jsonspec['envSpec']['vcenterDetails']["nsxtAddress"])
        nsx_ip = socket.gethostbyname(nsx_fqdn)
        nsxt_group = self.nsxObj.createGroup(VCF.NSXT_GROUP, None, "true", nsx_ip)
        if nsxt_group[1] != 200:
            logger.error(
        ...
        fw_alb_to_nsx = self.nsxObj.createFirewallRule(Policy_Name.POLICY_NAME, "alb-to-nsx",
                                    payload)
        ...
        fw_vip = self.nsxObj.createFirewallRule(Policy_Name.POLICY_NAME, FirewallRuleCgw.DISPLAY_NAME_VCF_TKG_and_AVIMgmt,
                                    payload)
        if fw_vip[1] != 200:
            logger.error(
    ...
    def configure_workload_nsxt_config(self):
        ...
        if fw[1] != 200:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to create firewall " + FirewallRuleCgw.DISPLAY_NAME_VCF_WORKLOAD_TKG_and_AVI_DNS + " " + str(
                    fw[0]['msg']),
                "ERROR_CODE": 500
            }
            return json.dumps(d), 500
        ...
        if fw[1] != 200:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to create firewall " + FirewallRuleCgw.DISPLAY_NAME_VCF_TKG_WORKLOAD_to_vCenter + " " + str(
                    fw[0]['msg']),
                "ERROR_CODE": 500
            }
            return json.dumps(d), 500
        ...
        if fw[1] != 200:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to create firewall " + FirewallRuleCgw.DISPLAY_NAME_VCF_WORKLOAD_TKG_and_AVI_to_Internet + " " + str(
                    fw[0]['msg']),
                "ERROR_CODE": 500
            }
            return json.dumps(d), 500
