...
from util.common_utils import downloadAndPushKubernetesOvaMarketPlace, runSsh, getNetworkFolder, \
    ...
    checkAviL7EnabledForShared, get_avi_version
...
class RaSharedClusterWorkflow:
    def __init__(self, run_config: RunConfig):
        ...
        self.raMgmtObj = RaMgmtClusterWorkflow(self.run_config)
    ...
    def akoDeploymentConfigSharedCluster(self,shared_cluster_name, aviVersion):
        ...
        if csrf2 is None:
            ...
            return json.dumps(d), 500
        aviVersion = get_avi_version(self.env)
        if self.env == Env.VSPHERE:
        ...
        if cluster_vip_cidr[0] is None or cluster_vip_cidr[0] == "NOT_FOUND":
            logger.error("Failed to get Vip network netmask")
            d = {
                "responseType": "ERROR",
        ...
        if self.env ==  Env.VSPHERE:
            self.createAkoFile(avi_fqdn, shared_cluster_name, tkg_mgmt_data_netmask[0], tkg_mgmt_data_pg, tkg_cluster_vip_name,
                    shared_service_network, cluster_vip_cidr[0], tier1, self.env)
        else:
            self.createAkoFile(avi_fqdn, shared_cluster_name, cluster_vip_cidr[0], tkg_cluster_vip_name, tkg_cluster_vip_name,
                      shared_service_network, cluster_vip_cidr[0], tier1, self.env)
    ...
    def createAkoFile(self,ip, shared_cluster_name, tkgMgmtDataVipCidr, tkgMgmtDataPg, cluster_vip_name, shared_network,
        ...
        if checkAviL7EnabledForShared(env, self.jsonspec):
    ...
    def deploy(self):
        try:
            ...
            if Tkg_version.TKG_VERSION == "2.1" and checkTmcEnabled(self.jsonspec, self.env):
                if self.env == Env.VCF:
            ...
            if command_status[0] is None:
                if Tkg_version.TKG_VERSION == "2.1" and checkTmcEnabled(self.jsonspec, self.env):
                    ...
                    if command_status[1] != 0:
                        ...
                        logger.info("Shared cluster is successfully deployed and running " + str(command_status[0]))
            else:
            ...
            if tmc_flag and (Tkg_version.TKG_VERSION != "1.5"):
                ...
                if state[1] != 200:
                    ...
                    return json.dumps(d), 500
            elif checkTmcEnabled(self.jsonspec, self.env) and Tkg_version.TKG_VERSION == "2.1":
