...
from constants.constants import TKG_EXTENSIONS_ROOT, Constants, Paths, Task, ControllerLocation, \
    ...
    Sizing, ClusterType, Repo, Avi_Version, Avi_Tkgs_Version, Env, SePrefixName
...
from util.common_utils import downloadAndPushKubernetesOvaMarketPlace, getCloudStatus, \
    ...
    enable_data_protection, checkEnableIdentityManagement, checkPinnipedInstalled, checkAirGappedIsEnabled, \
    obtain_second_csrf, checkAviL7EnabledForWorkload, create_virtual_service
from util.oidc_helper import createRbacUsers
from util.ShellHelper import runShellCommandAndReturnOutput
from util.avi_api_helper import isAviHaEnabled
...
class RaWorkloadClusterWorkflow:
    ...
    def createAkoFile(self, ip, cluster_name, wipCidr, tkgMgmtDataPg, cluster_vip_name, workload_network,
        ...
        if checkAviL7EnabledForWorkload(env, self.jsonspec):
            import ipaddress
            net = self.jsonspec['tkgWorkloadComponents']['tkgWorkloadGatewayCidr']
    ...
    def networkConfig(self, aviVersion, cluster_name, data_store):
        ...
        if self.env == Env.VCF:
            workload_network_name = self.jsonspec['tkgComponentSpec']['tkgClusterVipNetwork'][
        ...
        if self.env == Env.VSPHERE:
            ...
            if update[0] is None:
                logger.error("Failed to update service engine Ipam " + str(update[1]))
                d = {
                    "responseType": "ERROR",
        ...
        if get_se_cloud[0] == "NOT_FOUND":
            isGen = True
            logger.info("Creating New se cloud " + group_name)
            cloud_se = self.clusterops.createSECloud(ip, csrf2, cloud_url,
                                                     group_name,
                                                     cluster_status[0],
                                                     data_store, aviVersion, SePrefixName.WORKLOAD)
        ...
        if self.env == Env.VSPHERE:
            ...
            data_network_workload = self.jsonspec['tkgComponentSpec']['tkgClusterVipNetwork'][
                'tkgClusterVipNetworkName']
        ...
        if get_management_data_pg[0] is None:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to get workload data network details " + str(
        ...
        if self.env == Env.VSPHERE:
            ...
            prefixIpNetmask = seperateNetmaskAndIp(
                self.jsonspec["tkgWorkloadDataNetwork"]["tkgWorkloadDataNetworkGatewayCidr"])
        else:
            startIp = self.jsonspec['tkgComponentSpec']['tkgClusterVipNetwork'][
                'tkgClusterVipIpStartRange']
            endIp = self.jsonspec['tkgComponentSpec']['tkgClusterVipNetwork']['tkgClusterVipIpEndRange']
            prefixIpNetmask = seperateNetmaskAndIp(
        ...
        cluster_vip_name = self.jsonspec['tkgComponentSpec']['tkgClusterVipNetwork'][
            'tkgClusterVipNetworkName']
        ...
        if self.env == Env.VCF:
            ...
            status, value = self.nsxObj.getCloudConnectUser(avi_fqdn, headers)
            nsxt_cred = value["nsxUUid"]
            tier1_id, status_tier1 = self.nsxObj.fetchTier1GatewayId(avi_fqdn, headers, nsxt_cred)
            if tier1_id is None:
        ...
        self.createAkoFile(ip, workload_cluster_name, wip[0], data_network_workload, cluster_vip_name, workload_network_name,
                      cluster_vip_cidr_[0], tier1, self.env)
