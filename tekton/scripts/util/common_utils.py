...
from constants.constants import Paths, ControllerLocation, KubernetesOva, MarketPlaceUrl, VrfType, \
    ...
    Tkg_version, TKG_Package_Details, TKGCommands, VeleroAPI, Upgrade_Extensions, AkoType, Type, \
     Versions, ServiceName, Avi_Version
...
def createSubscribedLibrary(vcenter_ip, vcenter_username, password, jsonspec):
    try:
        ...
        if str(output[0]).__contains__(ControllerLocation.SUBSCRIBED_CONTENT_LIBRARY):
            ...
            if check_tkgs_proxy_enabled(jsonspec):
...
def check_tkgs_proxy_enabled(jsonspec):
    proxyEnabled = False
    try:
        isProxyEnabled = jsonspec['tkgsComponentSpec']['tkgServiceConfig']['proxySpec'][
            'enableProxy']
...
def getSeNewBody(newCloudUrl, seGroupName, clusterUrl, dataStore, se_prefix_name):
    body = {
        "max_vs_per_se": 10,
...
def getClusterStatusOnTanzu(cluster_name, typen=None, return_dict = False):
    try:
        cluster_status_dict = {"deployed": False,
                           "running": False,
                           "out": ""}
        tanzu_get_clstr_cmd = ["tanzu", "cluster", "list", "--include-management-cluster", "-A", "-o", "json"]
...
def template14deployYaml(cluster_name, clusterPlan, datacenter, dataStorePath,
    ...
    air_gapped_repo = ""
    repo_certificate = ""
    if checkEnableIdentityManagement(env, jsonspec=jsonspec):
        if env == Env.VSPHERE or env == Env.VCF:
            identity_mgmt_type = str(
                jsonspec["tkgComponentSpec"]["identityManagementSpec"][
                    "identityManagementType"])
        elif env == Env.VMC:
            identity_mgmt_type = str(
                jsonspec["componentSpec"]["identityManagementSpec"][
                    "identityManagementType"])
    else:
        identity_mgmt_type = ""
    if checkAirGappedIsEnabled(jsonspec):
    ...
    vsphere_thumb_print = getVCthumbprint(jsonspec)
    FileHelper.write_to_file(
        t.render(config=vsSpec, clustercidr=clustercidr, sharedClusterName=cluster_name,
                 ...
                 dataStorePath=dataStorePath, avi_label_key=AkoType.KEY, avi_label_value=ako,
                 folderPath=folderPath, mgmt_network=mgmt_network,
                 ...
                 vsphere_tls_thumbprint=vsphere_thumb_print), cluster_name + ".yaml")
...
def createContourDataValues(clusterName):
    ...
    with open(Paths.LOCAL_VSPHERE_ALB_CONTOUR_CONFIG, "w") as f2:
        f2.write(content)
        ...
        try:
            ...
            if not str(status[0]).__contains__(REPOSITORY_URL):
                list_command = ["tanzu", "package", "repository", "add", Repo.NAME, "--url",
                                os.path.join(REPOSITORY_URL, f'{os.sep}'.join(TKG_Package_Details.REPOSITORY_URL.split(os.sep)[1:])), "-n",
                                "tkg-system"]
...
def getVCthumbprint(jsonspec):
    logger.info("Fetching VC thumbprint")
    try:
        vCenter = jsonspec['envSpec']['vcenterDetails']["vcenterAddress"]
    except:
        logger.error('Failed to fetch VC details')
        return 500
    if not vCenter:
        logger.error('Failed to fetch VC details')
        return 500
    ...
    try:
        ...
        logger.error('vCenter connection failed')
        return 500
    ...
    if thumb_sha1:
        ...
        logger.error('Failed to obtain VC SHA1')
        return 500
...
def checkAviL7EnabledForShared(env, jsonspec):
    if env == Env.VCF:
        try:
            enable_L7 = str(jsonspec['tkgComponentSpec']['tkgSharedserviceSpec']['tkgSharedserviceEnableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    elif env == Env.VSPHERE:
        try:
            enable_L7 = str(jsonspec['tkgComponentSpec']['tkgMgmtComponents']['tkgSharedserviceEnableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    elif env == Env.VMC:
        try:
            enable_L7 = str(jsonspec['componentSpec']['tkgSharedServiceSpec']['tkgSharedserviceEnableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    else:
        enable_L7 = "false"
...
def checkAviL7EnabledForWorkload(env, jsonspec):
    if env == Env.VCF or env == Env.VSPHERE:
        try:
            enable_L7 = str(jsonspec['tkgWorkloadComponents']['tkgWorkloadEnableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    elif env == Env.VMC:
        try:
            enable_L7 = str(jsonspec['componentSpec']['tkgWorkloadSpec']['tkgWorkloadEnableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    else:
        enable_L7 = "false"
...
def getNetworkDetailsVip(ip, csrf2, vipNetworkUrl, startIp, endIp, prefixIp, netmask,
                             aviVersion, env ):
    url = vipNetworkUrl
    headers = {
        "Accept": "application/json",
...
def create_virtual_service(ip, csrf2, avi_cloud_uuid, se_name, vip_network_url, se_count, tier_id, vrf_url_tier1,
    ...
    if response_csrf.status_code != 200:
        ...
        if createVs:
            ...
            startIp = jsonspec["tkgComponentSpec"]['tkgClusterVipNetwork'][
                "tkgClusterVipIpStartRange"]
            endIp = jsonspec["tkgComponentSpec"]['tkgClusterVipNetwork'][
                "tkgClusterVipIpEndRange"]
            prefixIpNetmask = seperateNetmaskAndIp(
                jsonspec["tkgComponentSpec"]['tkgClusterVipNetwork'][
