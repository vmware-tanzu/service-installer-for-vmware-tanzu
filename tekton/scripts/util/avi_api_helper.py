...
from util.vcenter_operations import verifyVcenterVersion
from util.common_utils import checkAirGappedIsEnabled, getProductSlugId, obtain_second_csrf
...
def pushAviToContenLibrary(jsonspec):
    ...
    avi_jwt_token = str(
        jsonspec['envSpec']['resourceSpec']["aviPulseJwtToken"])
    avi_version = Avi_Version.VSPHERE_AVI_VERSION
    if my_file.exists():
        logger.info("AVI ova is already downloaded")
    else:
        logger.info("Downloading AVI controller.")
...
def pushAviToContenLibraryMarketPlace(jsonspec):
    ...
    if my_file.exists():
        ...
        logger.info("Retrieved solution name from MarketPlace...")
        ...
        logger.info("Retrieved product ID from MarketPlace...")
        ...
        logger.info("Retrieved download URL from MarketPlace...")
        logger.info("Download will take about 5 minutes to complete...")
        logger.info("Downloaded AVI controller will be saved to /tmp on SIVT VM")
        ...
        if response_csfr.status_code != 200:
            ...
            command = ["rm", "-rf", ls[0]]
            rcmd.runShellCommandAndReturnOutputAsList(command)
...
def downloadAviControllerAndPushToContentLibrary(vcenter_ip, vcenter_username, password, jsonspec):
    try:
        ...
        rcmd = cmd_runner.RunCmd()
        if not checkAirGappedIsEnabled(jsonspec):
            ...
            find_command = ["govc", "library.ls"]
            output = rcmd.runShellCommandAndReturnOutputAsList(find_command)
            VC_Content_Library_name = jsonspec['envSpec']['vcenterDetails'][
                "contentLibraryName"]
