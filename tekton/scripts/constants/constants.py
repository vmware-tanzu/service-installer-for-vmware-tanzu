...
logger = LoggerHelper.get_logger(name='constant.py')
...
class Paths(str, Enum):
    ...
    DESIRED_STATE_PATH = "desired-state/day0-desired-state.yml"
    ...
    DAY2_PATH = "desired-state/day2-desired-state.yml"
    ...
    # For Airgapped only
    TMP_KIND_YAML_PATH = f"{EXT_COMMON_ROOT_DIR}/kind_tkg.yaml"
...
class KubernetesOva:
    # TODO: To move to common class, initialize it and inherit it here in Tekton 2.0
    TKG_SUPPORTED_VERSIONS = ["2.1.0"]
    tkg_version = ""
    tkg_type = ""
    with open(Paths.DESIRED_STATE_PATH, 'r') as f:
        content = f.read().split('\n')
    for line in content:
        if "tkg" in line:
            line = re.sub(" ", "", line)
            tkg_type, tkg_version = line.strip().split(':')[0], line.strip().split(':')[1]
            break
    else:
        logger.error(f"Tkg version string not found in {Paths.DESIRED_STATE_PATH}")

    if tkg_type == "tkgm" and tkg_version not in TKG_SUPPORTED_VERSIONS:
        raise Exception("TKGm Version is not in supported list")

    try:
        kubernetes_ova_dict = {"2.1.0": {
                    "UBUNTU_KUBERNETES_FILE_NAME": "ubuntu-2004-kube-v1.24.9-vmware.1-tkg.1-b030088fe71fea7ff1ecb87a4d425c93.ova",
                    "PHOTON_KUBERNETES_FILE_NAME": "photon-3-kube-v1.24.9-vmware.1-tkg.1-f5e94dab9dfbb9988aeb94f1ffdc6e5e.ova",
                    "PHOTON_KUBERNETES_TEMPLATE_FILE_NAME": "photon-3-kube-v1.24.9+vmware.1",
                    "UBUNTU_KUBERNETES_TEMPLATE_FILE_NAME": "ubuntu-2004-kube-v1.24.9+vmware.1",
                    "MARKETPLACE_KUBERNETES_SOLUTION_NAME": "tanzu-kubernetes-grid-1-1",
                    ...
                    "KUBERNETES_OVA_LATEST_VERSION": "v1.24.9",
                    "TARGET_VERSION": "2.1.0"}
                }
        if tkg_type == "tkgm":
            UBUNTU_KUBERNETES_FILE_NAME = kubernetes_ova_dict[tkg_version]['UBUNTU_KUBERNETES_FILE_NAME']
            PHOTON_KUBERNETES_FILE_NAME = kubernetes_ova_dict[tkg_version]['PHOTON_KUBERNETES_FILE_NAME']
            PHOTON_KUBERNETES_TEMPLATE_FILE_NAME = \
                kubernetes_ova_dict[tkg_version]['PHOTON_KUBERNETES_TEMPLATE_FILE_NAME']
            UBUNTU_KUBERNETES_TEMPLATE_FILE_NAME = \
                kubernetes_ova_dict[tkg_version]['UBUNTU_KUBERNETES_TEMPLATE_FILE_NAME']
            MARKETPLACE_KUBERNETES_SOLUTION_NAME = \
                kubernetes_ova_dict[tkg_version]['MARKETPLACE_KUBERNETES_SOLUTION_NAME']
            MARKETPLACE_UBUNTU_KUBERNETES_FILE_NAME = \
                kubernetes_ova_dict[tkg_version]['MARKETPLACE_UBUNTU_KUBERNETES_FILE_NAME']
            MARKETPLACE_PHOTON_KUBERNETES_FILE_NAME = \
                kubernetes_ova_dict[tkg_version]['MARKETPLACE_PHOTON_KUBERNETES_FILE_NAME']
            MARKETPLACE_PHOTON_GROUPNAME = kubernetes_ova_dict[tkg_version]['MARKETPLACE_PHOTON_GROUPNAME']
            MARKETPLACE_UBUTNU_GROUPNAME = kubernetes_ova_dict[tkg_version]['MARKETPLACE_UBUTNU_GROUPNAME']
            KUBERNETES_OVA_LATEST_VERSION = kubernetes_ova_dict[tkg_version]['KUBERNETES_OVA_LATEST_VERSION']
            TARGET_VERSION = kubernetes_ova_dict[tkg_version]['TARGET_VERSION']
    except Exception as e:
        logger.error(f"ERROR: Please specify supported tkg version: {e}")


class UpgradeVersions:

    common_mp_dict = {"MARKETPLACE_KUBERNETES_SOLUTION_NAME": "tanzu-kubernetes-grid-1-1",
                      "MARKETPLACE_UBUNTU_KUBERNETES_FILE_NAME": "arcas-ubuntu-kube",
                      "MARKETPLACE_PHOTON_KUBERNETES_FILE_NAME": "arcas-photon-kube",
                      "MARKETPLACE_PHOTON_GROUPNAME": "Photon-OVA",
                      "MARKETPLACE_UBUTNU_GROUPNAME": "Ubuntu-OVA"
    ...
    upgrade_mapping_dict = {
        "2.1.0": {
            "UBUNTU_KUBERNETES_FILE_NAME":
...
class TKGCommands(str, Enum):
    ...
    LIST_ALL_CLUSTERS_JSON = "tanzu cluster list --include-management-cluster -A --output json"
...
class KubectlCommands(str, Enum):
    ...
    GET_CLUSTER_DEPLOYMENT = "kubectl get cluster {cluster_name} -o json"
...
class Avi_Version:
    ...
    avi_version_dict = {"2.1.0": {"VSPHERE_AVI_VERSION": "21.1.4",
                                  "VMC_AVI_VERSION": "21.1.4",
                                  "AVI_VERSION_UPDATE_THREE": "21.1.4",
                                  "AVI_VERSION_UPDATE_TWO": "21.1.4",
                                  }}
    if tkg_type == "tkgm":
        VSPHERE_AVI_VERSION = avi_version_dict[tkg_version]["VSPHERE_AVI_VERSION"]
        VMC_AVI_VERSION = avi_version_dict[tkg_version]["VMC_AVI_VERSION"]
        AVI_VERSION_UPDATE_THREE = avi_version_dict[tkg_version]["AVI_VERSION_UPDATE_THREE"]
        AVI_VERSION_UPDATE_TWO = avi_version_dict[tkg_version]["AVI_VERSION_UPDATE_TWO"]

class Avi_Tkgs_Version:
    # TODO: To move to common class, initialize it and inherit it here in Tekton 2.0
    TKGS_SUPPORTED_VERSIONS = ["2.1"]
    tkg_version = ""
    tkg_type = ""
    with open(Paths.DESIRED_STATE_PATH, 'r') as f:
        content = f.read().split('\n')
    ...
    if tkg_type == "tkgs" and tkg_version not in TKGS_SUPPORTED_VERSIONS:
        raise Exception("TKGs Version is not in supported list")

    avi_version_dict = {"2.1": {"VSPHERE_AVI_VERSION": "21.1.4",
                                  "VMC_AVI_VERSION": "21.1.4",
                                  "AVI_VERSION_UPDATE_THREE": "21.1.4",
                                  "AVI_VERSION_UPDATE_TWO": "21.1.4"},
                        }
...
class Tkg_version:
    ...
    if tkg_type == "tkgm" and tkg_version not in TKG_SUPPORTED_VERSIONS:
        print(f"TKGm {tkg_version} Version is not in supported list {TKG_SUPPORTED_VERSIONS}")
    if tkg_type == "tkgs" and tkg_version not in TKGS_SUPPORTED_VERSIONS:
        print(f"TKGs {tkg_version} Version is not in supported list {TKGS_SUPPORTED_VERSIONS}")

    # Extract base TKG version
    TKG_VERSION = '.'.join(tkg_version.split(".")[0:2])

    # Extract TKG version tag
    TAG = f"v{tkg_version}"
...
class Extentions:
    ...
    BOM_LOCATION_14 = f"/root/.config/tanzu/tkg/bom/tkg-bom-v{tkg_version}.yaml"
...
class TKG_Package_Details:
    ...
    REPOSITORY_URL = f"projects.registry.vmware.com/tkg/packages/standard/repo:v{tkg_version}"
