...
export class TanzuSaasStepComponent extends StepFormDirective implements OnInit {
    ...
    getSupervisorClustersForTMC(refreshToken, tmcInstanceURL) {
        ...
        this.apiClient.getSupervisorClustersForTMC(refreshToken, tmcInstanceURL).subscribe((data: any) => {
            if (data && data !== null) {
    ...
    onTMCRefreshTokenChange(){
        if(this.apiClient.tkgsStage === 'namespace'){
            if (this.formGroup.get('refreshToken').value !== '') {
                this.getSupervisorClustersForTMC(this.formGroup.get('refreshToken').value, this.formGroup.get('tmcInstanceURL').value);
            }
        }
    }
    ...
    verifyTmcRefreshToken() {
        ...
        this.apiClient.verifyTmcRefreshToken(refreshToken).subscribe((data: any) => {
            if (data && data !== null) {
                if (data.responseType === 'SUCCESS') {
                    this.getSupervisorClustersForTMC(refreshToken, tmcInstanceURL);
    ...
    validateSupervisorCluster(clusterName, tmcInstanceURL, tmcRefreshToken) {
        this.apiClient.validateSupervisorCluster(clusterName, tmcInstanceURL, tmcRefreshToken).subscribe((data: any) => {
            if (data && data !== null) {
                if (data.responseType === 'SUCCESS') {
                    this.errorNotification = null;
    ...
    onSupervisorClusterChange() {
        this.tmcClusterValid = false;
        if(this.formGroup.get('clusterName').value !== '' && this.formGroup.get('tmcInstanceURL').value !== "" && this.formGroup.get('refreshToken').value){
            this.validateSupervisorCluster(this.formGroup.get('clusterName').value, this.formGroup.get('tmcInstanceURL').value, this.formGroup.get('refreshToken').value);
        }
    }
