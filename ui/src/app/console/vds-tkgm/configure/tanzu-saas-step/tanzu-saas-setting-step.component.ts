...
export class TanzuSaasStepComponent extends StepFormDirective implements OnInit {
    ...
    toggleTMCSetting() {
        ...
        if (this.formGroup.value['tmcSettings']) {
            ...
            this.apiClient.tmcEnabled = false;
            this.apiClient.toEnabled = false;
            if(this.apiClient.compliantDeployment) this.apiClient.baseImage = ['ubuntu'];
            else this.apiClient.baseImage = ['photon', 'ubuntu'];
    ...
    verifyTmcRefreshToken() {
        ...
        let instanceUrl = this.formGroup.controls['tmcInstanceURL'].value;
    ...
    // Enable Proxy and then connect to TMC
    connectTMC() {
        this.loadingState = ClrLoadingState.LOADING;
