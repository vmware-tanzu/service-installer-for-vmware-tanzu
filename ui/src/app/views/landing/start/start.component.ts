...
export class StartComponent extends BasicSubscriber implements OnInit {
    ...
    navigateToTKGsUploadPanel(provider: string, infraType: string): void {
        this.loading = true;
        this.apiClient.tkgsStage = "";
        this.appDataService.setProviderType(provider);
        this.appDataService.setInfraType(infraType);
        const upload = APP_ROUTES.TKGS_VSPHERE_UPLOAD_WIZARD;
        this.router.navigate([upload]);
    }

    navigateToVcdUploadPanel(provider: string, infraType: string): void {
        this.loading = true;
        this.apiClient.tkgsStage = "";
        this.appDataService.setProviderType(provider);
        this.appDataService.setInfraType(infraType);
        const upload = APP_ROUTES.VCD_UPLOAD_WIZARD;
        this.router.navigate([upload]);
    }
