...
export const isValidFqdn = (arg: string) => {
    const regexPattern = /^[a-z0-9]+([-.][a-z0-9]+)*\.[a-z0-9]{2,}$/i;
    return regexPattern.test(arg.trim());
}
...
export const isValidFqdnWithHttpsProtocol = (arg: string) => {
    const regexPattern = /((https):\/)\/([a-z0-9]+([-.][a-z0-9]+)*\.[a-z0-9]{2,})((\/\w+)*([\/].*)?)+$/i;
    return regexPattern.test(arg.trim());
}
...
/**
 * @method isValidAviPassword decide if AVI Password is valid
 * @return boolean
 */
export const isValidAviPassword = (arg: string) => {
    const upperCaseCharacters = /[A-Z]+/g;
    const lowerCaseCharacters = /[a-z]+/g;
    const numberCharacters = /[0-9]+/g;
    const specialCharacters = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/;
    let counter = 0;
    if (upperCaseCharacters.test(arg.trim())) {
        counter = counter + 1;
    }
    if (lowerCaseCharacters.test(arg.trim())) {
        counter = counter + 1;
    }
    if (numberCharacters.test(arg.trim())) {
        counter = counter + 1;
    }
    if (specialCharacters.test(arg.trim())) {
        counter = counter + 1;
    }
...
//         counter = counter + 1;
//     }
    if (lowerCaseCharacters.test(arg.trim()) && numberCharacters.test(arg.trim()) && specialCharacters.test(arg.trim())) {
        return true;
    } else {
        return false;
    }
