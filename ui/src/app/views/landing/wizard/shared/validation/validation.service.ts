...
export class ValidationService {
    ...
    /**
     * @method isValidAviPassword
     * NOTE: if string has 3 out of the following Uppercase, lowercase,
     * numeric and special character
     */
    isValidAviPassword(): any {
        return (control: AbstractControl) => {
            const ctrlValue: string = control.value;
            if (ctrlValue) {
                if (ctrlValue.length !== ctrlValue.toString().trim().length) {
                ...
                return validationMethods.isValidAviPassword(ctrlValue) ?
                    null : { [ValidatorEnum.VALID_AVI_PASSWORD]: true };
            }
            return null;
        }
    }

    checkIPWithCidr(cidrCtrl: AbstractControl): any {
        return (control: AbstractControl) => {
            const ctrlValue: string = control.value;
            console.log(ctrlValue);
            if (ctrlValue) {
                if (validationMethods.isValidIp(ctrlValue)) {
                    const cidr = cidrCtrl.value;
                    console.log(cidr);
                    if (validationMethods.isValidCidr(cidr)) {
                        if (validationMethods.isIpInCidr(ctrlValue, cidr)){
                            console.log("Returned NULL");
                            return null;
                        }
                        return {[ValidatorEnum.IP_IN_CIDR]: true};
                    }
                    return null;
                }
                return {[ValidatorEnum.VALID_IP]: true};
            }
            return null;
        }
    }
     ...
     * numeric and special character
     */
    isValidVcenterPassword(): any {
        return (control: AbstractControl) => {
            const ctrlValue: string = control.value;
            console.log("isValidVcenterPassword");
            console.log(ctrlValue);
            if (ctrlValue) {
                if (ctrlValue.length !== ctrlValue.toString().trim().length) {
                    return {
                ...
                return validationMethods.isValidVcenterPassword(ctrlValue) ?
                    null : { [ValidatorEnum.VALID_VCENTER_PASSWORD]: true };
            }
            return null;
        }
    }
    ...
    isIpInSubnet2(cidrHolder: {}, cidr: string): any {
        console.log("Inside Ip in Subnet");
        return (control: AbstractControl) => {
