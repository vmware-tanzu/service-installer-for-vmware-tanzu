...
export abstract class WizardBaseDirective extends BasicSubscriber implements AfterViewInit, OnInit {
    ...
    vsphereTkgsNamespaceSteps = [true, false, false, false, false, false, false, false, false];

    vcdSteps = [true, false, false, false, false, false, false, false, false, false, false, false];
    review = false;
    ...
    ngOnInit() {
        if (this.infraType === 'tkgm') {
            ...
            } else if (this.providerType === 'vcd') {
                this.steps = this.vcdSteps;
                this.subscription = this.vcdDataService.currentInputFileStatus.subscribe(
                    (upload) => this.uploadStatus = upload);
    ...
    onDnsNextStepVds() {
        this.dataService.changeDnsServer(
            this.getFieldValue('dnsNTPStepForm', 'dnsServer'));
        this.onNextStep();
    }
    ...
    onCustomRepoNextClick() {
        ...
        if(!this.uploadStatus && this.apiClient.isAirgapped) {
            this.apiClient.compliantDeployment = true;
        }
        this.onNextStep();
    }

    onCustomRepoNextClickVCF() {
        if(!this.uploadStatus && this.apiClient.isAirgapped) {
            this.apiClient.compliantDeployment = true;
        }
        this.onNextStep();
    ...
    onTkgMgmtDataNextClick() {
        if (this.uploadStatus) {
            ...
            this.form.get('vsphereMgmtNodeSettingForm').get('compliantDeployment').setValue(this.apiClient.compliantDeployment);
        }
        this.onNextStep();
    }
    ...
    onMgmtNextClick() {
        if (this.uploadStatus) {
            ...
            if(this.apiClient.compliantDeployment) {
                this.form.get('vsphereWorkloadNodeSettingForm').get('baseImage').setValue('ubuntu');
                this.form.get('vsphereSharedServiceNodeSettingForm').get('baseImage').setValue('ubuntu');
                this.form.get('vsphereWorkloadNodeSettingForm').get('baseImageVersion').setValue('v1.27.5');
                this.form.get('vsphereSharedServiceNodeSettingForm').get('baseImageVersion').setValue('v1.27.5');
            }
        }
        this.onNextStep();
    }
    ...
    onNamespaceNext() {
        ...
        if (this.uploadStatus) {
            ...
            let clusterKind;
            this.vsphereTkgsDataService.currentClusterKind.subscribe(
                (kind) => clusterKind = kind);

            let clusterVersion;
            this.vsphereTkgsDataService.currentClusterVersion.subscribe(
                (version) => clusterVersion = version);
            if (clusterKind === 'TanzuKubernetesCluster' && this.apiClient.clusterVersionsAlpha.indexOf(clusterVersion) !== -1) {
                this.form.get('workloadClusterForm').get('clusterVersion').setValue(clusterVersion);
            } else if (clusterKind === 'ClusterClass' && this.apiClient.clusterVersionsBeta.indexOf(clusterVersion) !== -1) {
                this.form.get('workloadClusterForm').get('clusterVersion').setValue(clusterVersion);
            }
    ...
    // ================================================== VCD ======================================================
    onVcdSpecNextStep() {
        this.vcdDataService.changeVcdAddress(this.form.get('vcdSpecForm').get('vcdAddress').value);
        this.vcdDataService.changeVcdUsername(this.form.get('vcdSpecForm').get('vcdSysAdminUserName').value);
        this.vcdDataService.changeVcdPassword(this.form.get('vcdSpecForm').get('vcdSysAdminPasswordBase64').value);
        this.onNextStep();
    }

    onAviControllerNextStep() {
        if(this.vcdDataService.aviGreenfield) {
            this.vcdDataService.changeAviController01Ip(this.form.get('aviControllerForm').get('aviController01Ip').value);
            this.vcdDataService.changeAviUsername(this.form.get('aviControllerForm').get('aviUsername').value);
            this.vcdDataService.changeAviPasswordBase64(this.form.get('aviControllerForm').get('aviPasswordBase64').value);
            this.vcdDataService.configureAviNsxtCloud = true;
        } else {
            this.vcdDataService.changeAviClusterIp(this.form.get('aviControllerForm').get('aviClusterIp').value);
            this.vcdDataService.changeAviUsername(this.form.get('aviControllerForm').get('aviUsername').value);
            this.vcdDataService.changeAviPasswordBase64(this.form.get('aviControllerForm').get('aviPasswordBase64').value);
        }
        if(this.form.get('aviNsxtCloudForm').get('configureAviNsxtCloud')) {
            this.form.get('aviNsxtCloudForm').get('configureAviNsxtCloud').setValue(this.vcdDataService.configureAviNsxtCloud);
        }
        this.onNextStep();
    }

    onNsxCloudNextStep() {
        this.vcdDataService.changeAviNsxCloudName(this.form.get('aviNsxtCloudForm').get('aviNsxCloudName').value);
        this.vcdDataService.changeVcenterAddressCloud(this.form.get('aviNsxtCloudForm').get('vcenterAddress').value);
        this.vcdDataService.changeVcenterSsoUserCloud(this.form.get('aviNsxtCloudForm').get('vcenterSsoUser').value);
        this.vcdDataService.changeVcenterSsoPasswordBase64Cloud(this.form.get('aviNsxtCloudForm').get('vcenterSsoPasswordBase64').value);
        this.vcdDataService.changeNsxtAddress(this.form.get('aviNsxtCloudForm').get('nsxtAddress').value);
        this.vcdDataService.changeNsxtUser(this.form.get('aviNsxtCloudForm').get('nsxtUser').value);
        this.vcdDataService.changeNsxtUserPasswordBase64(this.form.get('aviNsxtCloudForm').get('nsxtUserPasswordBase64').value);
        this.onNextStep();
    }

    onT0GatewayNext() {
        if(this.vcdDataService.importTier0Nsxt) {
            this.vcdDataService.changeExtNetgatewayCIDR(this.form.get('t0RouterForm').get('extNetGatewayCIDR').value);
            this.vcdDataService.t0GatewayCidr = this.form.get('t0RouterForm').get('extNetGatewayCIDR').value;
            this.vcdDataService.t0StartAddress = this.form.get('t0RouterForm').get('extNetStartIP').value
            this.vcdDataService.t0EndAddress = this.form.get('t0RouterForm').get('extNetEndIP').value
        }
        this.onNextStep();
    }
    ...
    onSvcOrgNextButton() {
        if(this.uploadStatus) {
            let storageSpecUpload;
            this.vcdDataService.currentStorageSpec.subscribe(
                (storagePolicy) => storageSpecUpload = storagePolicy);
            this.apiClient.storagePolicy = storageSpecUpload;
            const storagePolicyUpload = [...this.apiClient.storagePolicy.keys()];
            const storageLimitUpload = [...this.apiClient.storagePolicy.values()];
            ...
            this.form.get('svcOrgVdcForm').get('storageSpec').setValue(this.apiClient.storagePolicy);
            let defaultStoragePolicy;
            this.subscription = this.vcdDataService.currentDefaultStoragePolicy.subscribe((defaultPolicy) => defaultStoragePolicy = defaultPolicy);
            if([...this.apiClient.storagePolicy.keys()].indexOf(defaultStoragePolicy) !== -1) {
                this.form.get('svcOrgVdcForm').get('defaultStoragePolicy').setValue(defaultStoragePolicy);
            }
        }
        if(this.form.get('svcOrgForm').get('svcOrgName').value === 'CREATE NEW') {
            this.vcdDataService.changeSvcOrgName(this.form.get('svcOrgForm').get('svcOrgNameInput').value);
            this.vcdDataService.changeSvcOrgFullName(this.form.get('svcOrgForm').get('svcOrgFullName').value);
        } else {
            this.vcdDataService.changeSvcOrgName(this.form.get('svcOrgForm').get('svcOrgName').value);
            this.vcdDataService.changeSvcOrgFullName('');
        }

        this.onNextStep();
    }

    public onSvcOrgVdcNextStep() {
        if (this.uploadStatus){
            this.form.get('svcOrgEdgeGatewayForm').get('t0Start').setValue(this.vcdDataService.t0StartAddress);
            this.form.get('svcOrgEdgeGatewayForm').get('t0End').setValue(this.vcdDataService.t0EndAddress);
        }
        this.onNextStep();
    }


    onEdgeGatewayNextStep() {
        this.vcdDataService.changeStaticIpPoolstartAddress(this.form.get('svcOrgEdgeGatewayForm').get('staticIpPoolStartAddress').value);
        this.vcdDataService.changeStaticIpPoolendAddress(this.form.get('svcOrgEdgeGatewayForm').get('staticIpPoolEndAddress').value);
        if(this.form.get('segForm').get('configureAviNsxtCloud')) {
            this.form.get('aviNsxtCloudForm').get('configureAviNsxtCloud').setValue(this.vcdDataService.configureAviNsxtCloud);
        }
        this.onNextStep();
    }
