...
export class AVINetworkSettingComponent extends StepFormDirective implements OnInit {
    ...
    typesOfLicense = ['enterprise', 'essentials'];
...
//     private aviLicenseKey: string;
    ...
    private modeOfDeployment: string;
    private typeOfLicense: string;
    ...
    ngOnInit() {
        ...
        this.formGroup.addControl('typeOfLicense', new FormControl('enterprise', []));
        this.formGroup.addControl(
            'aviSize',
            new FormControl('', [
                Validators.required
...
//             ]));
        ...
        this.formGroup['canMoveToNext'] = () => {
            ...
            if(!this.nameResolution) this.errorNotification = "Please click on Validate Name Resolution";
        ...
        setTimeout(_ => {
            ...
            if (this.uploadStatus) {
                ...
                this.subscription = this.dataService.currentTypeOfLicense.subscribe(
                    (mode) => this.typeOfLicense = mode);
                this.formGroup.get('typeOfLicense').setValue(this.typeOfLicense);
...
//                 this.formGroup.get('aviLicenseKey').setValue(this.aviLicenseKey);
                ...
                if (this.apiClient.networks.indexOf(this.aviSegmentName) === -1) {
    ...
    onAVIMgmtCidrChange() {
        if (this.formGroup.get('aviMgmtGatewayIp').valid){
            this.cidrHolder['AVIMGMT'] = this.formGroup.get('aviMgmtGatewayIp').value;
        }
    }

    onControllerIPChange() {
        if(this.formGroup.get('aviControllerIp').valid) {
            this.onAVIMgmtCidrChange();
            let ip = this.formGroup.get('aviControllerIp').value;
            const cidrValidator = this.validationService.isIpInSubnet2(this.cidrHolder, "AVIMGMT");
            this.resurrectField('aviControllerIp', [
                Validators.required,
                this.validationService.isValidIp(),
                this.validationService.noWhitespaceOnEnds(),
    ...
    checkLicenseType() {
        if (this.formGroup.get('typeOfLicense').value === 'enterprise') {
...
//                 this.loadingState = ClrLoadingState.LOADING;
//                 this.pingTestMgmt(this.formGroup.get('startAddress').value);
//             }
//         }
//     }
}
