...
resource "azurerm_key_vault_secret" "AZURE_CONTROL_PLANE_SUBNET_NAME_mgmt" {
  ...
  value        = element([for k, v in local.tkgm_subnets : k if v.cluster_type == "management" && v.subnet_type == "controlplane"], 0)
  key_vault_id = data.azurerm_key_vault.this.id
}

resource "azurerm_key_vault_secret" "AZURE_CONTROL_PLANE_SUBNET_NAME_wrk" {
  name         = "AZURE-CONTROL-PLANE-SUBNET-NAME-workload"
  value        = element([for k, v in local.tkgm_subnets : k if v.cluster_type == "workload" && v.subnet_type == "controlplane"], 0)
  key_vault_id = data.azurerm_key_vault.this.id
}

resource "azurerm_key_vault_secret" "AZURE_NODE_SUBNET_NAME_mgmt" {
  name         = "AZURE-NODE-SUBNET-NAME-management"
  value        = element([for k, v in local.tkgm_subnets : k if v.cluster_type == "management" && v.subnet_type == "node"], 0)
  key_vault_id = data.azurerm_key_vault.this.id
}

resource "azurerm_key_vault_secret" "AZURE_NODE_SUBNET_NAME_wrk" {
  name         = "AZURE-NODE-SUBNET-NAME-workload"
  value        = element([for k, v in local.tkgm_subnets : k if v.cluster_type == "workload" && v.subnet_type == "node"], 0)
  key_vault_id = data.azurerm_key_vault.this.id
}
...
resource "azurerm_key_vault_secret" "CLUSTER_NAME_mgmt" {
  name         = "CLUSTER-NAME-management"
  value        = "${var.tkg_cluster_name}-management"
  key_vault_id = data.azurerm_key_vault.this.id
}

resource "azurerm_key_vault_secret" "CLUSTER_NAME_wrk" {
  name         = "CLUSTER-NAME-workload"
  value        = "${var.tkg_cluster_name}-workload"
  key_vault_id = data.azurerm_key_vault.this.id
}