...
output "ssh_pip_cmd" {
  value = "ssh -o IdentitiesOnly=yes -i ${local_file.bootstrap_priv_key.filename} ${var.user}@${azurerm_linux_virtual_machine.this.public_ip_address}"
}

# Bootstrap Private IP can be presented in output if private IPs are used
output "ssh_priv_cmd" {
  value = "ssh -o IdentitiesOnly=yes -i ${local_file.bootstrap_priv_key.filename} ${var.user}@${azurerm_linux_virtual_machine.this.private_ip_address}"
}