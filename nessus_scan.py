...
class LibNessus:
    ...
    def get_cred_id(self, credentials_name=cred_name):
        nessus_obj = self.login_nessus()
        cred_resp = nessus_obj.credentials.list()
        nessus_obj.logout()
        for k in cred_resp["usable"]:
            if k["name"] == credentials_name:
                return k["id"]
    ...
    def generate_cred(self):
        if cred_name:
            return self.get_cred_id(self.cred_name)
        elif self.host_name == "root" and self.host_password == "VMware1!":
            return self.get_cred_id()
        else:
            nessus_obj = self.login_nessus()
            creds = nessus_obj.credentials.create(
                self.scan_name + "_cred", "ssh", "password", username=self.host_name, password=self.host_password
            )
            nessus_obj.logout()
            return creds.get("id")
    ...
    def get_vuln_with_severity(self, severity, launch_id):
        nessus_obj = self.login_nessus()
        vul_list = nessus_obj.analysis.vulns(("severity", "=", str(severity)), scans_id=str(launch_id))
        nessus_obj.logout()
        return vul_list
    ...
    def create_launch_scans(self):
        ...
        if not scans_ids:
            resp = nessus_obj.scans.create(
                ...
                scans_zone=self.get_scans_zone_id(),
                creds=[self.get_cred_id()],
                email_complete=True,
            )
            scans_ids = resp["id"]
