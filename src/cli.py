...
class ArcasCli:
    ...
    def set_payload_and_env(self):
        ...
        """
        self.headers = {"Accept": "application/json", "Content-Type": "application/json", "Env": self.env}
    ...
    def avi_configuration(self):
        ...
        try:
            ...
            if self.env == Env.VCD:
                self.headers["Env"] = "vcf"
                temp_avi_file = "/opt/vmware/arcas/src/vcd/vcd_avi.json"
                res = self.req_api_obj.exec_req(
                    req_type="POST",
                    api_url=ApiUrl.AVI_CONFIG_URL[Env.VCF],
                    headers=self.headers,
                    data=open(temp_avi_file, "rb"),
                )
            else:
                res = self.req_api_obj.exec_req(
                    req_type="POST",
    ...
    def shared_service_configuration(self):
        ...
        try:
            if self.env not in SUPPORTED_NON_VCD_ENV_LIST:
                print("Wrong env type, please specify vmc or vsphere")
                self.safe_exit()
            self.tiny_db.update_db_file(SivtStatus.IN_PROGRESS, Component.SS)
            ...
            if not self.req_api_obj.verify_resp(res):
                self.tiny_db.update_db_file(SivtStatus.FAILED, Component.SS)
                print("Shared service configuration failed " + str(res.json()))
                self.safe_exit()
            else:
                self.tiny_db.update_db_file(SivtStatus.SUCCESS, Component.SS)
    ...
    def initialize_db(self):
        ...
        try:
            items = [
                {"name": Component.ENV, "status": self.env},
                ...
                {"name": Component.SS, "status": SivtStatus.NOT_STARTED},
            ...
            self.tiny_db.db_obj.insert_multiple(items)

            if self.env != Env.VCF:
                self.tiny_db.update_db_file(SivtStatus.NA, Component.VCF_PRECONFIG)
            if self.env != Env.VMC:
                self.tiny_db.update_db_file(SivtStatus.NA, Component.VMC_PRECONFIG)

            try:
                if self.env == Env.VSPHERE:
                    json_data = self.file_helper.load_json(self.json_file)
                    tkgs = json_data["envSpec"]["envType"]
                    if tkgs.lower() in ["tkgs-ns", "tkgs-wcp"]:
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.MGMT)
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.WORKLOAD_PRECONFIG)
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.SS)
                    else:
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.WCP_CONFIG)
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.WCP)
                        self.tiny_db.update_db_file(SivtStatus.NA, Component.NAMESPACE)
            except KeyError:
                self.tiny_db.update_db_file(SivtStatus.NA, Component.WCP_CONFIG)
                self.tiny_db.update_db_file(SivtStatus.NA, Component.WCP)
                self.tiny_db.update_db_file(SivtStatus.NA, Component.NAMESPACE)
    ...
    def vcd_avi_configuration(self):
        ...
        if self.env == Env.VCD:
            ...
            headers = {"Accept": "application/json", "Content-Type": "application/json", "Env": self.env}
    ...
    def precheck_env(self):
        ...
        headers = {"Accept": "application/json", "Content-Type": "application/json", "Env": self.env}
        if self.env == Env.VCD:
            headers1 = {"Accept": "application/json", "Content-Type": "application/json", "Env": Env.VSPHERE}
            with open(self.json_file, "r") as file_read:
    ...
    def check_db_file(self):
        if not CommonUtils.is_file_exist(self.sivt_db_file):
            print("DB file not exists, initializing DB")
            self.initialize_db()
        elif Path(self.sivt_db_file).stat().st_size == 0:
            print("DB file exists but empty, Creating it")
            self.initialize_db()
        if not self.tiny_db.db_obj.contains(self.tiny_db.query_obj.status == self.env):
            print(f"DB file exists and have content, but existing env {self.env} missing, Creating it")
            self.tiny_db.db_obj.truncate()
            self.initialize_db()
