...
from common.operation.constants import MarketPlaceUrl
from common.operation.ShellHelper import runShellCommandWithPolling
from common.operation.vcenter_operations import get_dc, get_ds
...
@token_required
def prechecks_vcd(current_user):
    """
    ...
    try:
        ...
        if not common_result[0]:
            ...
            d = {"responseType": "ERROR", "msg": "Pre-check failed " + str(common_result[1]), "STATUS_CODE": 500}
        ...
        if str(deploy_avi).lower() == "false":
            ...
            if not result[0]:
                ...
                d = {"responseType": "ERROR", "msg": "Pre-check failed " + str(result[1]), "STATUS_CODE": 500}
                return jsonify(d), 500
        ...
        if not cse_result[0]:
            ...
            d = {"responseType": "ERROR", "msg": "Pre-check failed " + str(cse_result[1]), "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Pre-checks Failed", "STATUS_CODE": 500}
        return jsonify(d), 500
...
def vc_validations(vcdSpec):
    try:
        ...
        if str(create_se).lower() == "true":
            try:
                datacenter = (
                    vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.serviceEngineGroup.vcenterPlacementDetails.vcenterDatacenter
                )
                cluster = (
                    vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.serviceEngineGroup.vcenterPlacementDetails.vcenterCluster
                )
                datastore = (
                    vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.serviceEngineGroup.vcenterPlacementDetails.vcenterDatastore
                )
                ...
                return (
                    False,
                    "vCenter details missing for creating SE group, please add "
...
def avi_spec_prechecks(vcdSpec):
    ...
    if avi_vcd_check[0] and avi_vcd_check[2] is not None:
        ...
        current_app.logger.info("NSX Cloud is not imported to VCD." " Hence, skipping NSX-T Cloud VCD pre-checks")
...
def cse_spec_validation(vcdSpec):
    try:
        ...
        if org_found:
            current_app.logger.info("Organization name found in VCD, " "checking if catalog sharing property enabled")
        ...
        input_policies = []
        input_policy_list = (
            vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.svcOrgVdcResourceSpec.storagePolicySpec.storagePolicies
        )
        default_policy = (
            vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.svcOrgVdcResourceSpec.storagePolicySpec.defaultStoragePolicy
        )
        ...
        if str(create_seg).lower() == "false":
            ...
            if not seg_found:
                current_app.logger.error(
                    'Provided Service engine group not found in NSX ALB. Set "createSeGroup" to '
                    "true if you wish SIVT to create this service engine group"
                )
                return False, "Service engine group not found in NSX ALB"
        else:
            seg_vcd_display_name = (
                vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.serviceEngineGroup.serviceEngineGroupVcdDisplayName
            )
                ...
                for seg in seg_vcd_list[0]:
                    if seg == seg_vcd_display_name:
                        ...
                        current_app.logger.info(
                            "Service engine group is already imported " "to VCD with provided VCD object name"
                        )
            if not seg_found:
        ...
        if str(import_tier0.lower()) == "true":
            ...
            gateway_vcd_name = (
                vcdSpec.envSpec.cseSpec.svcOrgVdcSpec.svcOrgVdcGatewaySpec.tier0GatewaySpec.tier0GatewayName
            )
            ...
            if tier0_found:
                ...
                current_app.logger.info(
                    'Provided Tier-0 gateway VCD object not found. Set "importTier0" to true if '
                    "this needs to be imported using SIVT"
                )
                return False, "Provided Tier-0 gateway VCD object not found"

        """
        If tier1GatewayName -> exists, make sure enabled=true,
        connected=true and whether or not the IP range is configured on the Tier-1
                    ...
                    for entry in catalogs.json()["record"]:
                        ...
                        if catalog == cse_catalog or catalog == k8s_template_catalog:
                            if entry["_type"] == "QueryResultCatalogRecordType" and entry["orgName"] == org_name:
                                ...
                                return (
                                    False,
                                    "Catalog - " + catalog + " exists but it's " "not created under org - " + org_name,
                                )
                except Exception:
                    pass
...
def get_avi_version(ip, vcdSpec):
    ...
    payload = {"username": avi_username, "password": password_avi}
    ...
    for alb in response.json()["values"]:
        if alb["name"] == avi_vcd_display_name:
            if alb["url"] in avi_urls:
                return True, "NSX ALB VCD object is mapped " "to given NSX ALB controller " + str(alb["url"]), alb["id"]
            else:
                current_app.logger.error("Please provide correct aviVcdDisplayName to continue")
                return (
                    False,
                    "Provided NSX ALB VCD object " + avi_vcd_display_name + " is already mapped "
                    "to different NSX ALB Controller" + str(alb["url"]),
                )
        else:
            if alb["url"] in avi_urls:
                return False, "Provided NSX ALB is already imported to " " a different object in VCD - " + str(
                    alb["name"]
                )
...
def get_csrf(ip, vcdSpec):
    ...
    enc_bytes_avi = base64.b64decode(base64_bytes_avi)
    password_avi = enc_bytes_avi.decode("ascii").rstrip("\n")
    payload = {"username": username, "password": password_avi}
    modified_payload = json.dumps(payload, indent=4)
        ...
        if nsxt_imported:
            ...
            for record in api_response.json()["values"]:
                if str(record["alreadyImported"]).lower() == "true" and record["displayName"] == nsxt_cloud_name:
                    current_app.logger.info(
                        "NSX-T Cloud VCD object - "
                        + nsxtCloud_vcd_displayName
                        + " - is mapped to correct NSX-T Cloud - "
                        + nsxt_cloud_name
                    )
                else:
...
def validate_network_pool(vcd, pool_name, vcdspec):
    ...
    if pool_id is None:
        return (
            False,
            "Network pool " + pool_name + " not available in VCD, "
    ...
    try:
        ...
        for record in response.json()["values"]:
            if gateway_name == record["name"]:
                nw_cidr = (
                    str(record["subnets"]["values"][0]["gateway"])
                    + "/"
                    + str(record["subnets"]["values"][0]["prefixLength"])
                )
        ...
        return (
            ...
            None,
            None,
            "Exception occurred while fetching start and end IP Ranges for T0 gateway: " + gateway_name,
        )
