...
from common.constants.vcd_api_constants import VcdApiEndpoint
from common.login_auth.authentication import token_required
from common.model.vcdSpec import VcdMasterSpec
from vcd.vcd_prechecks.vcd_utils import (
    configure_cse_plugin,
    create_server_config_cse,
    get_access_token_vapp,
    upload_avi_cert,
    upload_cse_ova_to_catalog,
    upload_kubernetes_ova_to_catalog,
)
from vcd.vcd_prechecks.vcdPrechecks import (
    get_catalog_list,
    get_cloud_list,
    get_ip_pool_for_selected_T0,
    get_np_list,
    get_org_vcd_list,
    get_org_vdc_list,
    get_provider_vcd_list,
    get_seg_list_nsxCloud,
    get_seg_vcd_list,
    get_storage_policies_vcd_list,
    get_tier0_nsx_list,
    get_tier0_vcd_list,
    get_vcd_session,
    getNetworksList,
...
@token_required
def connect_to_vcd(current_user):
    """
    ...
    try:
        ...
        if result[0] is None:
            ...
            d = {"responseType": "ERROR", "msg": result[2], "STATUS_CODE": 500}
            return jsonify(d), 500

        d = {"responseType": "SUCCESS", "msg": "VCD Connection established successfully", "STATUS_CODE": 200}
...
@token_required
def list_nsxt_cloud(current_user):
    """
    ...
    try:
        ...
        if cloud_list[0] is None:
            if cloud_list[1].__contains__("List is empty"):
                ...
                d = {"responseType": "ERROR", "msg": cloud_list[1], "STATUS_CODE": 500}
                return jsonify(d), 500
        else:
        ...
        current_app.logger.error(
            "Exception occurred while fetching the list of configured NSX-T " "clouds from NSX ALB Controller"
        )
        d = {"responseType": "ERROR", "msg": str(e), "STATUS_CODE": 500}
        return jsonify(d), 500
...
@token_required
def get_avi_vcd_list(current_user):
    """
...
@token_required
def get_nsxt_cloud_vcd_list(current_user):
    """
...
@token_required
def get_provider_vdc(current_user):
    """
    ...
    try:
        ...
        if result[0] is None:
            ...
            d = {"responseType": "ERROR", "msg": result[1], "STATUS_CODE": 500}
            return jsonify(d), 500
...
@token_required
def get_network_pool_vcd(current_user):
    """
    ...
    try:
        ...
        if not result:
            d = {"responseType": "ERROR", "msg": "No Network pool found on VCD", "STATUS_CODE": 500}
            return jsonify(d), 500
...
@token_required
def get_storage_policies_vcd(current_user):
    """
    ...
    try:
        ...
        if not result:
            d = {"responseType": "ERROR", "msg": "No Storage Policy found on VCD", "STATUS_CODE": 500}
            return jsonify(d), 500
        else:
...
@token_required
def get_tier0_vcd(current_user):
    """
    ...
    try:
        ...
        if not result:
            d = {"responseType": "ERROR", "msg": "No Tier-0 gateway found on VCD", "STATUS_CODE": 500}
            return jsonify(d), 500
        else:
            current_app.logger.info(result)
...
@token_required
def get_ip_ranges_tier0(current_user):
    """
...
@token_required
def get_Nsx_Manager(current_user):
    """
...
@token_required
def upload_cse_catalog_(current_user):
    """
    ...
    try:
        ...
        if not result:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to upload catalog " + msg, "STATUS_CODE": 500}
            return jsonify(d), 500
        d = {
            "responseType": "SUCCESS",
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while uploading cse catalog", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/upload_k_catalog", methods=["POST"])
@token_required
def upload_kubernetes_ova_to_catalog_(current_user):
    """
    ...
    try:
        ...
        if not result:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to upload  k8s catalog " + msg, "STATUS_CODE": 500}
            return jsonify(d), 500
        d = {
            "responseType": "SUCCESS",
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while uploading k8s  catalog", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/get_access_token_vapp", methods=["POST"])
@token_required
def get_access_token_vapp_(current_user):
    """
    Get Nsx upload k8s ova
    :return:
    """
    try:
        ...
        if not result:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to vapp access token " + msg, "STATUS_CODE": 500}
            return jsonify(d), 500
        d = {
            "responseType": "SUCCESS",
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while getting vapp access token", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/configure_cse_plugin", methods=["POST"])
@token_required
def configure_cse_plugin_(current_user):
    """
    Get cse plugin
    :return:
    """
    try:
        ...
        if not result:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to configure cse plugin " + msg, "STATUS_CODE": 500}
            return jsonify(d), 500
        d = {
            "responseType": "SUCCESS",
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while Configured cse plugin", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/create_server_config_cse", methods=["POST"])
@token_required
def create_server_config_cse_(current_user):
    """
    Get cse plugin
    :return:
    """
    try:
        ...
        if not result:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to to create server config " + msg, "STATUS_CODE": 500}
            return jsonify(d), 500
        d = {
            "responseType": "SUCCESS",
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while creating server config", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/upload_avi_cert", methods=["POST"])
@token_required
def upload_avi_cert_(current_user):
    """
    Get Nsx Manager Name
    :return:
    """
    try:
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while uploading avi cert to  VCD", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/listTier0Nsx", methods=["POST"])
@token_required
def get_tier0_nsx(current_user):
    """
    ...
    try:
        ...
        if not result:
            d = {"responseType": "ERROR", "msg": "No Tier-0 gateway found on NSX, list is empty", "STATUS_CODE": 500}
            return jsonify(d), 500
        else:
            current_app.logger.info(result)
...
@token_required
def get_seg_nsxcloud(current_user):
    """
    ...
    try:
        ...
        if seg_list[0] is None:
            if seg_list[1].__contains__("List is empty"):
                ...
                d = {"responseType": "ERROR", "msg": seg_list[1], "STATUS_CODE": 500}
                return jsonify(d), 500
        else:
...
@token_required
def validate_existing_org(current_user):
    """
    ...
    try:
        ...
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while validating the organization", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcd_ui_util.route("/api/tanzu/listCatalogVcd", methods=["POST"])
@token_required
def list_catalog_vcd(current_user):
    """
...
@token_required
def get_tier1_vcd(current_user):
    """
    ...
    try:
        ...
        d = {"responseType": "SUCCESS", "msg": "Obtained the list of Tier-0 Gateways from VCD", "STATUS_CODE": 200}
        return jsonify(d), 200
    except Exception as e:
        current_app.logger.error(str(e))
        d = {
...
@token_required
def get_networks_vcdOrg(current_user):
    """
    ...
    try:
        ...
        d = {"responseType": "SUCCESS", "msg": result[1], "STATUS_CODE": 200, "NETWORKS_LIST": result[0]}
        return jsonify(d), 200
    except Exception as e:
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occurred while fetchingNetworks from VCD", "STATUS_CODE": 500}
        return jsonify(d), 500
