...
from common.operation.constants import CertName, CseMarketPlace, KubernetesOva, VcdCSE
from common.operation.ShellHelper import runShellCommandAndReturnOutput, runShellCommandAndReturnOutputAsList
from vcd.vcd_prechecks.vcdPrechecks import (
    download_kubernetes_ova,
    get_avi_version,
    get_csrf,
    get_file_MarketPlace,
    get_org_vcd_list,
    get_vcd_session,
    run_vcd_api,
)
...
def upload_avi_cert(specFile):
    ...
    if str(deploy_avi).lower() == "true":
        ...
        return (
            False,
            "If NSX ALB is already deployed, please import the certificate to VCD, prior " "to initiating deployment",
        )
    ...
    payload = {"alias": CertName.VCD_AVI_CERT_NAME.format(avi_fqdn=avi_fqdn), "certificate": cert}
    ...
    try:
        ...
        if not ova_available:
            current_app.logger.info(
                "Uploading Kubernetes OVA to catalog "
                + catalog_name
                + ". This will take approximately 5 minutes to complete..."
            )

            upload_command = ["vcd", "--json", "catalog", "upload", catalog_name, "/tmp/" + ova_filename + ".ova"]
...
def upload_cse_ova_to_catalog(specFile):
    ...
    try:
        ...
        if not file_obj.exists():
            current_app.logger.info(
                "Downloading CSE OVA from VMware MarketPlace, " "download will take approximately 5 mins to complete..."
            )
            response = get_file_MarketPlace(
                CseMarketPlace.CSE_OVA_NAME, refresh_token, CseMarketPlace.VERSION, CseMarketPlace.CSE_OVA_GROUPNAME
            )
            if response[0] is None:
                return False, response[1]
            current_app.logger.info(response[1])
        ...
        if not ova_available:
            current_app.logger.info(
                "Uploading CSE OVA to catalog " + catalog_name + ". This will take approximately "
...
def configure_cse_plugin(vcdspec):
    ...
    try:
        ...
        if not file_obj.exists():
            ...
            response = get_file_MarketPlace(
                CseMarketPlace.CSE_PLUGIN_NAME_VCD,
                refresh_token,
                CseMarketPlace.VERSION,
                ...
                "profiles": [
                    {
                        "name": "production",
                        "active": "true",
                        "vcdKeInstances": [{"name": "vcd-container-service-extension"}],
                        "K8Config": {
                            "cni": {"name": "antrea", "version": ""},
                            "cpi": {"name": "cpi for cloud director", "version": "1.2.0"},
                            "csi": [{"name": "csi for cloud director", "version": "1.3.0"}],
                        },
                        "serverConfig": {"rdePollIntervalInMin": 1},
                        "githubConfig": {"githubPersonalAccessToken": ""},
                        "bootstrapClusterConfig": {
                            "clusterctl": {"version": "v1.1.3", "clusterctlyaml": ""},
                            "kindVersion": "v0.14.0",
                            "proxyConfig": {
                            ...
                            "capiEcosystem": {
                                "infraProvider": {
                                    "name": "capvcd",
                                    "version": "1.0.0",
                                    "capvcdRde": {"nss": "capvcdCluster", "vendor": "vmware", "version": "1.1.0"},
                                },
                                "coreCapiVersion": "v1.1.3",
                                "bootstrapProvider": {"name": "CAPBK", "version": "v1.1.3"},
                                "controlPlaneProvider": {"name": "KCP", "version": "v1.1.3"},
                            },
                            "dockerVersion": "",
                            "kubectlVersion": "",
                        },
...
def get_access_token_vapp(vcdspec):
    ...
    try:
        ...
        payload = {"client_name": token_name}
        modified_payload = json.dumps(payload, indent=4)
        ...
        header = {"Accept": "application/*;version=36.1", "Content-Type": "application/x-www-form-urlencoded"}
...
def upload_cse_plugin(vcd_address, plugin_id, vcdspec):
    try:
        ...
        header = VcdHeaders.COMMON_HEADER.format(
            response_format="vnd.vmware.vcloud.query.records+json", vcd_auth_key=vcd_session_key
        )
        header = json.loads(header)
        payload = {"fileName": CseMarketPlace.CSE_PLUGIN_NAME_VCD, "size": zip_size}
        modified_payload = json.dumps(payload, indent=4)
        response = requests.request("POST", url, headers=header, data=modified_payload, verify=False)
...
def create_interfaces_entities(vcd, vcdspec):
    try:
        ...
        header = VcdHeaders.COMMON_HEADER.format(response_format="json", vcd_auth_key=vcd_session_key)
        header = json.loads(header)
        payload = {"name": VcdCSE.VCDKE_CONFIG, "version": "1.0.0", "vendor": "vmware", "nss": "VCDKEConfig"}
        modified_payload = json.dumps(payload, indent=4)
