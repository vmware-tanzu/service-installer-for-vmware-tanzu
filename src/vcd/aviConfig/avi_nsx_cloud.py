...
from common.operation.constants import ControllerLocation, NSXtCloud
from common.replace_value import generateVsphereConfiguredSubnets
...
def createIpam_nsxtCloud(data, ip, csrf2, managementNetworkUrl, ipam_name, aviVersion):
    try:
        ...
        body = {
            "internal_profile": {"usable_networks": [{"nw_ref": managementNetworkUrl}]},
            ...
            "nsxt_configuration": {
                ...
                "data_network_config": {
                    ...
                    "tier1_segment_config": {
                        ...
                        "manual": {"tier1_lrs": [{"segment_id": status_seg["avi_mgmt"], "tier1_lr_id": status_tier1}]},
                    },
                },
...
def fetchContentLibrary(data, ip, headers, vcenter_credential):
    try:
        vc_Content_Library_name = data["envSpec"]["cseSpec"]["svcOrgVdcSpec"]["serviceEngineGroup"][
        ...
        body = {"host": vCenter, "credentials_uuid": vcenter_credential}
        json_object = json.dumps(body, indent=4)
        response_csrf = requests.request("POST", url, headers=headers, data=json_object, verify=False)
        if response_csrf.status_code != 200:
            return None, response_csrf.text
...
def configureVcenterInNSXTCloud(data, ip, csrf2, cloud_url, aviVersion):
    ...
    try:
        ...
        if found:
            ...
            if response_csrf.status_code != 200:
                return None, response_csrf.text
            cluster_name = data["envSpec"]["cseSpec"]["svcOrgVdcSpec"]["serviceEngineGroup"]["vcenterPlacementDetails"][
                "vcenterCluster"
            ]
...
def nsx_cloud_creation(file, deploy_se):
    ...
    if csrf2 is None:
        print("ERROR: Failed to get csrf from new password")
        return "Failed", 500
    ...
    if get_cloud[0] == "NOT_FOUND":
        ...
        if isDeployNsxCloud == "true":
            ...
            if seg_status[0] is None:
                print("ERROR: Failed to create Servie Engine Management Segment " + str(seg_status[1]))
    ...
    if deploy_se:
        ...
        if state is None:
            return "Failed", 500
        print("INFO: Configured servie engine cloud successfully")
    else:
...
def create_service_engine_group(data, ip, csrf2, aviVersion, cloud_url):
    ...
    if create_se == "true":
        ...
        if get_se_cloud[0] == "NOT_FOUND":
            print("INFO: Creating New service engine cloud " + seGroupName)
            ...
            if cloud_se[0] is None:
                print("ERROR: Failed to create service engine cloud " + str(cloud_se[1]))
                return None
            print("INFO: Waiting for 3 mins for service engine to be up.")
            time.sleep(180)
        else:
        ...
        get_se_cloud = getSECloudStatus(data, ip, csrf2, aviVersion, seGroupName)
        if get_se_cloud[0] is None:
            print("ERROR: Failed to get service engine cloud status " + str(get_se_cloud[1]))
            return None
        if get_se_cloud[0] == "NOT_FOUND":
...
def grabNsxtHeaders(data):
    try:
        ...
        ecod_bytes = (data["envSpec"]["aviNsxCloudSpec"]["nsxDetails"]["nsxtUser"] + ":" + password).encode("ascii")
