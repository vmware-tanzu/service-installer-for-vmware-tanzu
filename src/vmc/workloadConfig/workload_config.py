...
def networkConfig(current_user):
    ...
    if not checkAirGappedIsEnabled(env) and refreshToken != "":
        validateK8s = validateKubeVersion(env, Type.WORKLOAD)
    ...
    current_app.logger.info("Performing ping check on workload gateways")
    addresses = [
        request.get_json(force=True)["componentSpec"]["tkgWorkloadSpec"]["tkgWorkloadGatewayCidr"],
        request.get_json(force=True)["componentSpec"]["tkgWorkloadDataNetworkSpec"]["tkgWorkloadDataGatewayCidr"],
    ]

    for ip_add in addresses:
        if ping_test("ping -c 1 " + ip_add.split("/")[0]) != 0:
            current_app.logger.warn(
                "Ping test failed for " + ip_add + " gateway. It is Recommended to fix "
                "this before proceeding with deployment"
            )
            time.sleep(30)
        else:
            current_app.logger.info("Ping test passed for gateway - " + ip_add)
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        ...
        cloud_se = createSECloud(
            ip, csrf2, cloud_url, Cloud.SE_WORKLOAD_GROUP_NAME, aviVersion, "Workload", license_type=avi_license_type
        )
...
def deploy(current_user):
    ...
    velero_util: VeleroUtil = VeleroUtil(env, vmcSpec)
    aviVersion = get_avi_version(env)
    vcenter_ip = current_app.config["VC_IP"]
    vcenter_username = current_app.config["VC_USER"]
    password = current_app.config["VC_PASSWORD"]
    ...
    if not verifyPodsAreRunning(workload_cluster_name, command_status[0], RegexPattern.running):
        ...
        if not saas_util.check_tmc_enabled():
            current_app.logger.info("Deploying workload cluster using tanzu")
            ...
            if saas_util.check_tmc_enabled():
                ...
                register_payload = saas_util.payload_tmc_cluster_creation(
                    ...
                    memory,
                    machineCount,
                    AkoType.WORKLOAD_CLUSTER_SELECTOR,
                    version,
                    datacenter_path,
                    datastore_path,
                    workload_folder_path,
    ...
    lisOfCommand = [
        ...
        "cluster",
        workload_cluster_name,
        AkoType.KEY + "=" + AkoType.WORKLOAD_CLUSTER_SELECTOR,
    ]
    status = runShellCommandAndReturnOutputAsList(lisOfCommand)
    if status[1] != 0:
        if not str(status[0]).__contains__("already has a value"):
    ...
    if not velero_util.enable_velero("workload", workload_cluster_name, management_cluster):
        d = {
            "responseType": "ERROR",
            "msg": "Failed to enable velero on cluster " + workload_cluster_name,
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
def createAkoFile(ip, wipCidr, clusterName, tkgClusterVipCidr, tkgClusterVipPg):
    ...
    data = dict(
        ...
        spec=dict(
            ...
            clusterSelector=dict(matchLabels=dict(type=AkoType.WORKLOAD_CLUSTER_SELECTOR)),
            controller=ip,
