...
from common.operation.constants import ResourcePoolAndFolderName, GroupNameCgw, GroupNameMgw, FirewallRuleMgw, \
    ControllerLocation, Paths, KubernetesOva
from common.common_utilities import preChecks, envCheck, getClusterStatusOnTanzu, \
    ...
    checkAviL7EnabledForWorkload, tmcBodyClusterCreation
...
def deploy():
    ...
    if change[1] != 200:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to change Network for" + str(change[0]),
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    ...
    if command_status[0] is None:
        ...
        if not verifyPodsAreRunning(workload_cluster_name, command_status[0], RegexPattern.running):
            ...
            if tkg_cluster_vip_netmask[0] is None or tkg_cluster_vip_netmask[0] == "NOT_FOUND":
                ...
                return jsonify(d), 500
            createAkoFile(ip, wip[0], workload_cluster_name, tkg_cluster_vip_netmask[0], Cloud.WIP_CLUSTER_NETWORK_NAME)
            commands = ["tanzu", "management-cluster", "kubeconfig", "get", management_cluster, "--admin"]
            kubeContextCommand = grabKubectlCommand(commands, RegexPattern.SWITCH_CONTEXT_KUBECTL)
            if kubeContextCommand is None:
                current_app.logger.error("Failed to get switch to management cluster context command")
            ...
            if not checkTmcEnabled(env):
                ...
                if checkTmcEnabled(env):
                    ...
                    try:
                        if env == Env.VMC:
                            osName = str(request.get_json(force=True)["componentSpec"]["tkgWorkloadSpec"]
                                         ["tkgWorkloadBaseOs"])
                        else:
                            osName = "photon"
                        if osName == "photon":
                    ...
                    refreshToken = request.get_json(force=True)['saasEndpoints']['tmcDetails'][
                        'tmcRefreshToken']
                    ...
                    provisoner = "default"
                    management_url = tmc_url + "/v1alpha1/managementclusters/" + management_cluster + "/provisioners/" \
                                     + provisoner + "/tanzukubernetesclusters"
                    ...
                    register_payload = tmcBodyClusterCreation(management_cluster, provisoner, workload_cluster_name,
                                                              ...
                                                              workload_folder_path, workload_network_path,
                                                              workload_resource_path, osName, osVersion, "amd64",
                                                              templatePath, "", controlPlaneNodeCount)
...
def createAkoFile(ip, wipCidr, clusterName, tkgClusterVipCidr, tkgClusterVipPg):
    ...
    if checkAviL7EnabledForWorkload(Env.VMC):
        ...
        extra_config = dict(cniPlugin="antrea",
                            disableStaticRouteSync=True,
                            ingress=dict(disableIngressClass=True,
                                         defaultIngressController=False,
                                         nodeNetworkList=lis_))
    data = dict(
...
def connectToWorkLoadCluster(env):
    ...
    d = {
        "responseType": "SUCCESS",
        "msg": "Switched to workload cluster context ",
        "STATUS_CODE": 200
    }
    return jsonify(d), 200
