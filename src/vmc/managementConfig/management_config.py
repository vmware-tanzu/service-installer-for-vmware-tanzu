...
def controllerDeployment(ip, csrf2, data_center, data_store, cluster_name, vcenter_ip, vcenter_username, password,
    ...
    if vm_state_se is None:
        ...
        if token[0] is None:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to get token " + str(token[1]),
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
        current_app.logger.info("Get cluster uuid")
    ...
    if type == Type.WORKLOAD:
        ...
        if vrf[1] != "Already_Configured":
            ...
            if ad[0] is None:
                ...
                d = {
                    "responseType": "ERROR",
                    "msg": "Failed to add static route " + str(ad[1]),
                    "STATUS_CODE": 500
                }
                return jsonify(d), 500
...
def managementClusterYaml14(ip, datacenter, datastoreName, cluster_name, wipIpNetmask, clusterWip, _vcenter_ip,
    ...
    if size.lower() == "small":
        ...
        current_app.logger.error("Unsupported cluster size, please specify small/medium/large/extra-large/custom " + size)
        d = {
            "responseType": "ERROR",
            "msg": "Unsupported cluster size, please specify small/medium/large/extra-large/custom " + size,
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
