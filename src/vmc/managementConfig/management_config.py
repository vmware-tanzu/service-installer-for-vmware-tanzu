...
def configManagementCluster(current_user):
    ...
    if config_cloud[1] != 200:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to configure management cluster " + str(config_cloud[0].json["msg"]),
            "STATUS_CODE": 500,
        }
    ...
    if config_mgmt[1] != 200:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to configure management cluster " + str(config_mgmt[0].json["msg"]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
def configCloud(current_user):
    ...
    spec: spec_obj = spec_obj.parse_obj(json_dict)
    ...
    if refreshToken:
        ...
        network = str(request.get_json(force=True)["componentSpec"]["tkgMgmtSpec"]["tkgMgmtNetworkName"])
        market_place_utils = MarketPlaceUtils(refreshToken)
    ...
    try:
        isCreated1 = create_folder(
            vcenter_ip, vcenter_username, password, data_center, ResourcePoolAndFolderName.Template_Automation_Folder
        )
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        current_app.logger.info("Creating New service engine cloud " + Cloud.SE_GROUP_NAME)
        cloud_se = createSECloud(
            ip, csrf2, cloud_url, Cloud.SE_GROUP_NAME, aviVersion, "Mgmt", license_type=avi_license_type
        )
    ...
    if get__cluster_wip[0] == "NOT_FOUND":
        ...
        vip_net = createClusterVipNetwork(ip, csrf2, cloud_url, Cloud.WIP_CLUSTER_NETWORK_NAME, aviVersion)
        if vip_net[0] is None:
    ...
    vm_state = checkVmPresent(
        vcenter_ip, vcenter_username, password, ControllerLocation.SE_OVA_TEMPLATE_NAME + "_" + avi_uuid
    )
    if vm_state is None:
        try:
            destroy_vm(
...
def controllerDeployment(
    ...
    if vm_state_se is None:
        ...
        if token[0] is None:
            current_app.logger.error("Failed to get token " + str(token[1]))
            d = {"responseType": "ERROR", "msg": "Failed to get token " + str(token[1]), "STATUS_CODE": 500}
            return jsonify(d), 500
        current_app.logger.info("Get cluster uuid")
        uuid = getClusterUUid(ip, csrf2, aviVersion)
        if uuid[0] is None:
        ...
        if type == Type.WORKLOAD:
            replaceNetworkValuesWorkload(ip, token[0], uuid[0], seJson, env=envCheck()[0])
        else:
    ...
    if not found:
        current_app.logger.error("Controller 1 is not up, failed to get an IP")
    ...
    if not found2:
        current_app.logger.error("Controller 2 is not up, failed to get an IP")
    ...
    if details1[0] is None:
        current_app.logger.error("Failed to get details of service engine 1" + str(details1[1]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get details of service engine 1" + str(details1[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    details2 = getDetailsOfServiceEngine(ip, csrf2, urlFromServiceEngine2[0], detailsJson2, aviVersion)
    if details2[0] is None:
        current_app.logger.error("Failed to get details of service engine 2 " + str(details2[1]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get details of service engine 2 " + str(details2[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    ...
    if se_engines[0] is None:
        current_app.logger.error("Failed to set service engine 1 interfaces" + str(se_engines[1]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to set service engine 1 interfaces" + str(se_engines[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    ...
    if se_engines2[0] is None:
        current_app.logger.error("Failed to set service engine 2 interfaces" + str(se_engines2[1]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to set service engine 2 interfaces" + str(se_engines2[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    ...
    for i in listOfServiceEngine:
        ...
        if s[0] is None or s[0] == "FAILED":
            current_app.logger.error("Failed to get connected status of service engine " + str(s[1]))
            d = {
                "responseType": "ERROR",
                "msg": "Failed to get connected status of service engine " + str(s[1]),
                "STATUS_CODE": 500,
            }
            return jsonify(d), 500
    ...
    if type == Type.WORKLOAD:
        ipNetMask = seperateNetmaskAndIp(
            request.get_json(force=True)["componentSpec"]["tkgWorkloadDataNetworkSpec"]["tkgWorkloadDataGatewayCidr"]
        )
    else:
        ipNetMask = seperateNetmaskAndIp(
        ...
        if vrf[1] != "Already_Configured":
            ...
            if ad[0] is None:
                ...
                d = {"responseType": "ERROR", "msg": "Failed to add static route " + str(ad[1]), "STATUS_CODE": 500}
                return jsonify(d), 500
...
@token_required
def configTkgMgmt(current_user):
    pre = preChecks()
    if pre[1] != 200:
        current_app.logger.error(pre[0].json["msg"])
        d = {"responseType": "ERROR", "msg": pre[0].json["msg"], "STATUS_CODE": 500}
    ...
    TanzuUtil(env=env, spec=vmcSpec)
    ...
    create = createResourceFolderAndWait(
        ...
        cluster_name,
        data_center,
        ResourcePoolAndFolderName.TKG_Mgmt_RP,
        ResourcePoolAndFolderName.TKG_Mgmt_Components_Folder,
        parent_resourcepool,
    )
    if create[1] != 200:
    ...
    if isAviHaEnabled(env):
        ...
        ip = checkforIpAddress(getSi(vcenter_ip, vcenter_username, password), ControllerLocation.CONTROLLER_NAME)
    if ip is None:
        current_app.logger.error("Failed to get an IP of AVI controller")
        d = {"responseType": "ERROR", "msg": "Failed to get IP of AVI controller", "STATUS_CODE": 500}
        return jsonify(d), 500
    csrf2 = obtain_second_csrf(ip, env)
    if csrf2 is None:
    ...
    if get_wip[0] is None or get_wip[0] == "NOT_FOUND":
        current_app.logger.error("Failed to get service engine VIP network IP and netmask " + str(get_wip[1]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get service engine VIP network IP and netmask " + str(get_wip[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    ...
    if get_cluster_wip[0] is None or get_cluster_wip[0] == "NOT_FOUND":
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get cluster VIP network IP and netmask " + str(get_cluster_wip[1]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    ...
    ceip = str(request.get_json(force=True)["ceipParticipation"])
    if ceip.lower() == "true":
        os.putenv("TANZU_CLI_CEIP_OPT_IN_PROMPT_ANSWER", "Yes")
    else:
        os.putenv("TANZU_CLI_CEIP_OPT_IN_PROMPT_ANSWER", "No")
    listOfCmd = ["tanzu", "plugin", "install", "--group", "vmware-tkg/default:v2.4.0"]
    runProcess(listOfCmd)
    ...
    listOfCmd = ["tanzu", "config", "eula", "accept"]
    runProcess(listOfCmd)
    command = ["tanzu", "plugin", "install", "--group", "vmware-tkg/default:2.4.0"]
    ...
    if checkEnableIdentityManagement(env):
        ...
        if checkEnableIdentityManagement(env):
            ...
            check_pinniped_svc = kubectl_util.check_pinniped_service_status()
            ...
            if identity_mgmt_type.lower() == "ldap":
                check_pinniped_dexsvc = kubectl_util.check_pinniped_dex_service_status()
            ...
            cluster_admin_users = request.get_json(force=True)["componentSpec"]["tkgMgmtSpec"][
    ...
    if saas_util.check_tmc_enabled():
        ...
        if switch[1] != 200:
            ...
            d = {"responseType": "ERROR", "msg": switch[0], "STATUS_CODE": 500}
            return jsonify(d), 500
        state = saas_util.register_management_cluster_tmc(management_cluster, "false", "management", clusterGroup)
        if state[1] != 200:
...
def referenceTkgMNetwork(ip, csrf2, url, aviVersion):
    ...
    body = {
        ...
        "configured_subnets": [
            {
                "prefix": {
                    "ip_addr": {"addr": reference_TKG_Mgmt_Network_ip, "type": "V4"},
...
def createSECloud(ip, csrf2, newCloudUrl, seGroupName, aviVersion, se_name_prefix, license_type):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
    ...
    json_object = getNewBody(newCloudUrl, seGroupName, se_name_prefix, license_type)
...
def createClusterVipNetwork(ip, csrf2, newCloudUrl, name, aviVersion):
    ...
    start_ip = request.get_json(force=True)["componentSpec"]["tkgClusterVipNetwork"]["tkgClusterVipIpStartRange"]
    end_ip = request.get_json(force=True)["componentSpec"]["tkgClusterVipNetwork"]["tkgClusterVipIpEndRange"]
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
...
def createVipNetwork(ip, csrf2, newCloudUrl, name, type, aviVersion):
    if type == Type.WORKLOAD:
        ...
        end_ip = request.get_json(force=True)["componentSpec"]["tkgMgmtDataNetworkSpec"]["tkgMgmtDataServiceEndRange"]
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
...
def replaceNetworkValuesWorkload(ip, aviAuthToken, clusterUUid, file_name, env):
    property_mapping = {"AVICNTRL": ip, "AVICNTRL_AUTHTOKEN": aviAuthToken, "AVICNTRL_CLUSTERUUID": clusterUUid}
    for key, value in property_mapping.items():
        replaceSe(file_name, "PropertyMapping", key, "Key", "Value", value)
    if env == Env.VMC:
...
def pushSeOvaToVcenter(vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name, avi_uuid):
    ...
    replaceValueSysConfig(
        "./vmc/managementConfig/importSeOva-vc.json",
        "Name",
        "name",
...
def deploySeEngines(
    ...
    if type == Type.WORKLOAD:
        replaceNetworkValuesWorkload(ip, aviAuthToken, clusterUUid, file_name, env=envCheck()[0])
    else:
    ...
    if type == Type.WORKLOAD:
        network_connect = ["govc", "device.connect", "-vm", engine_name, "ethernet-0", "ethernet-1"]
        network_disconnect = [
            "govc",
            "device.disconnect",
            "-vm",
    ...
    try:
        current_app.logger.info("Deploying Service engine " + engine_name)
...
def listAllServiceEngine(ip, csrf2, countSe, name, aviVersion):
    ...
    if response_csrf.status_code != 200:
        ...
        current_app.logger.info("Successfully deployed Service engine")
        for se in response_csrf.json()["results"]:
            if str(se["config"]["name"]).strip() == str(name).strip():
...
def template14MgmtDeployYaml(
    ...
    if size.lower() == "small":
        ...
        control_plane_vcpu = request.get_json(force=True)["componentSpec"]["tkgMgmtSpec"]["tkgMgmtCpuSize"]
        control_plane_disk_gb = request.get_json(force=True)["componentSpec"]["tkgMgmtSpec"]["tkgMgmtStorageSize"]
        control_plane_mem_gb = request.get_json(force=True)["componentSpec"]["tkgMgmtSpec"]["tkgMgmtMemorySize"]
            ...
            if identity_mgmt_type.lower() == "oidc":
                ...
                FileHelper.write_to_file(
                    t.render(
                        ...
                        control_plane_size=size.lower(),
                        worker_size=size.lower(),
                ...
                ldap_endpoint_ip = str(
                    request.get_json(force=True)["componentSpec"]["identityManagementSpec"]["ldapSpec"][
                        "ldapEndpointIp"
                    ]
                )
