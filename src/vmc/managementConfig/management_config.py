...
from common.common_utilities import preChecks, createResourceFolderAndWait, registerWithTmc, obtain_second_csrf, \
    ...
    getVrfAndNextRoutId, addStaticRoute, checkTmcEnabled
...
from common.operation.ShellHelper import runShellCommandWithPolling, grabKubectlCommand, \
    runShellCommandAndReturnOutputAsList, runProcess
...
def configTkgMgmt():
    ...
    if Tkg_version.TKG_VERSION == "1.5":
        ...
        if checkTmcEnabled(env):
            state = registerWithTmc(management_cluster, env, "false", "management")
            if state[0] is None:
...
def template14MgmtDeployYaml(ip, datacenter, datastoreName, cluster_name, wipIpNetmask, clusterWip, vcenter_ip,
    ...
    FileHelper.write_to_file(
        t.render(config=vmcSpec, cert=cert, ip=ip, wipIpNetmask=wipIpNetmask, avi_label_key=AkoType.KEY,
                 ...
                 avi_cluster_vip_network_gateway_cidr=clusterWip, os_name=osName, os_version=osVersion, size=size,control_plane_vcpu=control_plane_vcpu,control_plane_disk_gb=control_plane_disk_gb,control_plane_mem_mb=control_plane_mem_mb),
