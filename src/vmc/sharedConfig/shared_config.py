...
def configSharedCluster(current_user):
    ...
    if not checkAirGappedIsEnabled(env) and refreshToken != "":
        validateK8s = validateKubeVersion(env, Type.SHARED)
...
def deploy(current_user):
    ...
    if command_status[0] is None:
        ...
        if not velero_util.enable_velero("shared", shared_cluster_name, management_cluster):
            d = {
                "responseType": "ERROR",
                "msg": "Failed to enable velero on cluster " + shared_cluster_name,
                "STATUS_CODE": 500,
            }
            return jsonify(d), 500
...
@token_required
def deployExtensions(current_user):
    pre = preChecks()
    if pre[1] != 200:
