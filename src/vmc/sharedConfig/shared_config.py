...
from common.operation.ShellHelper import runShellCommandAndReturnOutput, grabKubectlCommand, verifyPodsAreRunning, \
    ...
    runShellCommandAndReturnOutputAsListWithChangedDir, runProcess
...
from common.common_utilities import preChecks, installCertManagerAndContour, envCheck, get_avi_version, \
    ...
    copy_harbor_cert_to_ytt_config, isEnvTkgm, checkAviL7EnabledForShared, tmcBodyClusterCreation
            ...
            if not checkTmcEnabled(env):
                ...
                if checkTmcEnabled(env):
                    ...
                    management_url = tmc_url + "/v1alpha1/managementclusters/" + management_cluster + "/provisioners/" +\
                                     provisioner + "/tanzukubernetesclusters"
                    register_payload = tmcBodyClusterCreation(management_cluster, provisioner, shared_cluster_name,
                                                              ...
                                                              shared_folder_path, shared_network_path,
...
def installHarbor14(service, repo_address, harborCertPath, harborCertKeyPath, harborPassword, host, clusterName):
    ...
    if not verifyPodsAreRunning(AppName.HARBOR, out[0], RegexPattern.RECONCILE_SUCCEEDED):
        ...
        get_url_command = ["kubectl", "-n", "tkg-system", "get", "packages",
        ...
        if my_file.exists():
            pull = ["imgpkg", "pull", "-b", str(status[0][0]).replace("'", ""), "-o", "/tmp/harbor-package","--registry-ca-cert-path","./cacrtbase64d.crt"]
        else:
