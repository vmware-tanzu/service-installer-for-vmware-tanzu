...
@token_required
def configure_alb(current_user):
    avi_dep = deploy_alb()
    if avi_dep[1] != 200:
...
def deploy_alb(current_user):
    ...
    marketplace_refresh_token = request.get_json(force=True)["marketplaceSpec"]["refreshToken"]
    if marketplace_refresh_token:
        marketplace_util = MarketPlaceUtils(marketplace_refresh_token)
        download_status = marketplace_util.download_avi_ova(govc_operation, Avi_Version.VMC_AVI_VERSION)
        if download_status[0] is None:
            d = {"responseType": "ERROR", "msg": download_status[1], "STATUS_CODE": 500}
            return jsonify(d), 500
    else:
...
@token_required
def manage_alb_certs(current_user):
    pre = preChecks()
    if pre[1] != 200:
        current_app.logger.error(pre[0].json["msg"])
        d = {"responseType": "ERROR", "msg": pre[0].json["msg"], "STATUS_CODE": 500}
