...
function inject_app_extension() {
  dep="App"
  yq eval "select(.kind == \"$dep\").spec.fetch[0].image.url |= \"$url\"" -i "$file"
  echo "Successfully injected app extension values to  $file"
}
...
function remove_external_policy_contour() {
    yq eval '.envoy.service.externalTrafficPolicy=""' -i "$file"
    echo "Successfully update the externalTrafficPolicy to $file"
}
...
case "$2" in
        ...
        remove_policy_contour)
                remove_external_policy_contour
                ;;
        *)
                echo "Usage: $0 {cert|app_extension|contour|contour_tag|envoy_tag|data_values_harbor|fluent_bit|overlay}"
                echo ""
