...
from common.operation.constants import Env, RegexPattern
...
@token_required
def deployKaurdApp(current_user):
    env = None
    try:
        env = request.headers["Env"]
    except Exception:
        d = {"responseType": "ERROR", "msg": "Bad Request", "STATUS_CODE": "400"}
    ...
    if env == Env.VMC:
        ...
        d = {"responseType": "ERROR", "msg": "Wrong env type", "STATUS_CODE": "400"}
    ...
    if connect[1] != 200:
        d = {
            "responseType": "ERROR",
            "msg": "Failed to connect workload cluster " + str(connect[0]),
            "STATUS_CODE": 500,
        }
        current_app.logger.error("Failed to connect workload cluster " + str(connect[0]))
        return jsonify(d), 500
    deploy = depployKaurd()
    if deploy[1] != 200:
        d = {"responseType": "ERROR", "msg": "Failed to deploy kaurd " + str(deploy[0]), "STATUS_CODE": 500}
        current_app.logger.error("Failed to deploy kaurd " + str(deploy[0]))
        return jsonify(d), 500
    d = {"responseType": "SUCCESS", "msg": "Succesfully deployed kaurd ", "STATUS_CODE": 200}
...
def deployApp(current_user):
    try:
        ...
        d = {"responseType": "ERROR", "msg": "Un-Authorized", "STATUS_CODE": 401}
        current_app.logger.error("Un-Authorized")
        return jsonify(d), 401
    shared_cluster_name = request.get_json(force=True)["componentSpec"]["tkgSharedServiceSpec"]["tkgSharedClusterName"]
    ...
    if command_status_ako is None:
        ...
        d = {"responseType": "ERROR", "msg": "Failed to grab ip address ", "STATUS_CODE": 500}
        return jsonify(d), 500
    ...
    if not NsxtClient.find_object(listOfDnat, "Ext-Harbor_automation"):
        ...
        url = (
            ...
            + "/policy/api/v1/infra/tier-1s/cgw/nat/USER/nat-rules/Ext-Harbor_automation"
        )
    ...
    if not NsxtClient.find_object(listOfGroups, "Shared-Service-Ingress-IP_automation"):
        body = {
            ...
            "expression": [{"resource_type": "IPAddressExpression", "ip_addresses": [str(command_status_ako)]}],
        }
        url = (
            current_app.config["NSX_REVERSE_PROXY_URL"]
            + "orgs/"
            ...
            + "/sddcs/"
            + current_app.config["SDDC_ID"]
            + "/policy/api/v1/infra/domains/cgw/groups/Shared-Service-Ingress-IP_automation"
        )
    ...
    if not NsxtClient.find_object(listOfGroups, "Ext-Harbor_automation"):
        body = {
            ...
            "logged": False,
            "source_groups": ["ANY"],
            "destination_groups": ["/infra/domains/cgw/groups/Shared-Service-Ingress-IP_automation"],
        ...
        url = (
            ...
            + "/sddcs/"
            + current_app.config["SDDC_ID"]
            + "/policy/api/v1/infra/domains/cgw/gateway-policies/default/rules/Ext-Harbor_automation"
        )
    ...
    if connect[1] != 200:
        d = {"responseType": "ERROR", "msg": "Failed connect to workload " + str(connect[0]), "STATUS_CODE": 500}
        current_app.logger.error("Failed connect to workload " + str(connect[0]))
        return jsonify(d), 500
    deploy = deployNginxAndGetIp()
    if deploy[1] != 200:
        d = {"responseType": "ERROR", "msg": "Failed deploy ngnix " + str(deploy[0]), "STATUS_CODE": 500}
    ...
    if not NsxtClient.find_object(listOfDnat, "Ext-Ngnx_automation"):
        ...
        url = (
            ...
            + "/sddcs/"
            + current_app.config["SDDC_ID"]
            + "/policy/api/v1/infra/tier-1s/cgw/nat/USER/nat-rules/Ext-Ngnx_automation"
        )
    ...
    if not NsxtClient.find_object(listOfGroups, "ngnx-App_automation"):
        body = {
            ...
            "expression": [{"resource_type": "IPAddressExpression", "ip_addresses": [deploy[0]]}],
        }
        url = (
            current_app.config["NSX_REVERSE_PROXY_URL"]
            + "orgs/"
            ...
            + "/sddcs/"
            + current_app.config["SDDC_ID"]
            + "/policy/api/v1/infra/domains/cgw/groups/ngnx-App_automation"
        )
    ...
    if not NsxtClient.find_object(listOfGroups, "Ext-DemoApp"):
        body = {
            ...
            "logged": False,
            "source_groups": ["ANY"],
            "destination_groups": ["/infra/domains/cgw/groups/ngnx-App_automation"],
        ...
        url = (
            ...
            + "/sddcs/"
            + current_app.config["SDDC_ID"]
            + "/policy/api/v1/infra/domains/cgw/gateway-policies/default/rules/Ext-Ngnx_automation"
        )
        response = createExt(body, url, "EXt gatway policy")
        if response[1] != 200:
            return response[0]
    d = {"responseType": "SUCCESS", "msg": "Sucessfully deployed Nginx", "STATUS_CODE": 200}
    current_app.logger.info("Sucessfully deployed Nginx")
    return jsonify(d), 200
...
def createExt(body, url, type):
    ...
    if response_sddc.status_code != 200:
        d = {"responseType": "ERROR", "msg": response_sddc.text, "STATUS_CODE": response_sddc.status_code}
        current_app.logger.error("Failed to create  " + type + " " + response_sddc.text)
        return jsonify(d), response_sddc.status_code
    d = {"responseType": "SUCCESS", "msg": response_sddc.text, "STATUS_CODE": response_sddc.status_code}
...
def depployKaurd():
    ...
    if app[1] != 0:
        ...
        d = {"responseType": "ERROR", "msg": "Failed to apply ngnx " + str(app[0]), "STATUS_CODE": 500}
        return jsonify(d), 500
    ...
    if app[1] != 0:
        ...
        d = {"responseType": "ERROR", "msg": "Failed to apply kaurd expose  " + str(app[0]), "STATUS_CODE": 500}
        return jsonify(d), 500
    d = {"responseType": "SUCCESS", "msg": "Applied and exposed kaurd  ", "STATUS_CODE": 200}
    return jsonify(d), 200
...
def deployNginxAndGetIp():
    ...
    if app[1] != 0:
        current_app.logger.error("Failed to apply Nginx " + str(app[0]))
        d = {"responseType": "ERROR", "msg": "Failed to apply Nginx " + str(app[0]), "STATUS_CODE": 500}
        return jsonify(d), 500
    if not waitTillPodsRunning():
        current_app.logger.error("Ngnix is not running")
        d = {"responseType": "ERROR", "msg": "Ngnix is not running", "STATUS_CODE": 500}
        return jsonify(d), 500
...
def getListOfDnat(headers):
    ...
    firewall_rule_CGW = requests.request("GET", firewall_rule_CGW_url, headers=headers, data=payload, verify=False)
    if firewall_rule_CGW.status_code != 200:
        d = {"responseType": "ERROR", "msg": firewall_rule_CGW.json(), "STATUS_CODE": firewall_rule_CGW.status_code}
