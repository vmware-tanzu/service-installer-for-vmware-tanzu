...
from .oneDot3_extentions import getRepo
...
def installHarborTkgs(harborCertPath, harborCertKeyPath, harborPassword, host, clusterName, env):
    ...
    if not (
        ...
        os.system(
            "cp /tmp/harbor-package/config/values.yaml " + Paths.CLUSTER_PATH + clusterName + "/harbor-data-values.yaml"
        )
        ...
        if update_sc_resp[1] != 200:
            ...
            d = {"responseType": "ERROR", "msg": update_sc_resp[0].json["msg"], "STATUS_CODE": 500}
            return jsonify(d), 500
        else:
        ...
        if running:
            ...
            if overlay_status[0] is None:
                ...
                d = {"responseType": "ERROR", "msg": overlay_status[1], "STATUS_CODE": 500}
                return jsonify(d), 500
        else:
        ...
        current_app.logger.info(
            "Harbor is already deployed and it's status is - " + out[0].split()[4] + " " + out[0].split()[5]
        )
        d = {
            "responseType": "SUCCESS",
