...
class TKGCommands(str, Enum):
    ...
    INSTALL_PACKAGE = (
        "tanzu package install {name} --package-name {pkgName} --namespace {namespace} --version {version} {options}"
    )
...
class KubectlCommands(str, Enum):
    ...
    LIST_PODS = "kubectl get pods -n {namespace}"
    APPLY_YAML = "kubectl create -f {file_name}"
