        ...
        if isharbor_requested.strip("\n").strip("\r").strip() == "true":
            ...
            while count < 60:
                if os.path.exists(file_path):
                    ...
                    current_app.logger.info("Cert  file not found wating " + file_path + "  file to be available, "
                                                                                         " waited for " + str(
                        count * 30) + "s, retrying")
                time.sleep(30)
                count = count + 1
            ...
            push = ["tanzu", "isolated-cluster", "upload-bundle", "--source-directory", tanzu_extract,
                    "--destination-repo", repository, "--ca-certificate", "cacrtbase64d.crt"]
            push_harbor = runShellCommandAndReturnOutputAsList(push)
            if push_harbor[1] != 0:
                response_body = {
                    "responseType": "ERROR",
...
def harbor_preload_status():
    ...
    tanzu_extract = "/opt/vmware/arcas/tools/tanzu/"
    expected_tar_count = 198
    expected_repo_count = 155
