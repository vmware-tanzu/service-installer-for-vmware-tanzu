    ...
    try:
        ...
        if isharbor_requested.strip("\n").strip("\r").strip() == "true":
            ...
            if status is None:
                ...
                d = {"responseType": "ERROR", "msg": str(message), "STATUS_CODE": 500}
            ...
            if status1 == "NOT_FOUND":
                ...
                d = {"responseType": "SUCCESS", "msg": str(message), "STATUS_CODE": 200}
            ...
            tanzu_extract = "/opt/vmware/arcas/tools/tanzu/"
            down_path = "/opt/vmware/arcas/tools/publish-images-fromtar.yaml"
            ...
            if found:
                ...
                response_body = {
                    "responseType": "ERROR",
                    "msg": "certificate file not found in current directory",
                    "STATUS_CODE": 500,
                }
                return jsonify(response_body), 500
            ...
            push_harbor = runShellCommandAndReturnOutputAsList(push)
            if push_harbor[1] != 0:
                response_body = {"responseType": "ERROR", "msg": "Harbor image push failed ", "STATUS_CODE": 500}
                return jsonify(response_body), 500
            current_app.logger.info("Successfully Pushing images to harbor")
        ...
        response_body = {"responseType": "ERROR", "msg": str(ex) + " Failed Harbor image push", "STATUS_CODE": 500}
        return jsonify(response_body), 500
    response_body = {"responseType": "SUCCESS", "msg": msg, "STATUS_CODE": 200}
    return jsonify(response_body), 200


def create_harbor_project(address, user, password, project_name):
    ecod_bytes = (user + ":" + password).encode("ascii")
    ...
    body = {"project_name": project_name, "registry_id": None, "public": True, "storage_limit": 53687091200}
    json_object = json.dumps(body, indent=4)
    response = requests.request("POST", uri, headers=headers, data=json_object, verify=False)
...
def harbor_preload_status():
    ...
    response_body = {"responseType": "SUCCESS", "msg": "", "percentage": percentage, "STATUS_CODE": 200}
    return jsonify(response_body), 200
