...
class TkgsUtil:
    ...
    @staticmethod
    def is_env_tkgs_wcp(spec, env):
        try:
            if env == Env.VSPHERE:
                tkgs = str(spec.envSpec.envType)
    ...
    def login_to_cluster(self, workload_cluster):
        vc_operation: VCEndpointOperations = VCEndpointOperations(
            self.vcenter_server, self.vcenter_username, self.vc_password
        )
        ...
        cluster_endpoint = self.get_cluster_endpoint(cluster_name, header)
        connect_command = KubectlUtil.KUBECTL_VPSHERE_LOGIN_TKGS_CLUSTER.format(
            vc_user=self.vcenter_username,
            endpoint_ip=cluster_endpoint,
        ...
        if output[1] != 0:
            current_app.logger.error(output[0])
            return False, f"Failed to login to cluster {workload_cluster}"
        return True, f"logged in to cluster {cluster_name}"
