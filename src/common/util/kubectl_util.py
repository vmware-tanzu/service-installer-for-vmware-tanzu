...
from common.operation.constants import AppName, Extension, RegexPattern
...
class KubectlUtil:
    ...
    GET_TKR_LABELS = "kubectl get tkr --show-labels"
    ADD_SERVICES_LABEL = (
    ...
    KUBECTL_VPSHERE_LOGIN_TKGS_CLUSTER = (
        "kubectl vsphere login --vsphere-username {vc_user} --server {endpoint_ip} "
        "--tanzu-kubernetes-cluster-name {workload_cluster} "
        "--tanzu-kubernetes-cluster-namespace {namespace} --insecure-skip-tls-verify"
    )
    ...
    def check_ako_pods_running(self, cluster_name, tmc_enabled):
        ...
        if tmc_enabled:
            for i in tqdm(range(180), desc="Waiting for ako pods to be up…", ascii=False, ncols=75):
                time.sleep(1)
        if not found:
            current_app.logger.error("Ako pods are not running on waiting " + str(count_ako * 30))
            return False
        else:
            current_app.logger.info(cluster_name + " cluster is already deployed and running ")
        return True
