...
def extensionDeploy13(service_name, repo_address):
    ...
    service = service_name
    if service == "certmanager" or service == "all":
        if not kubectl_util.check_cert_manager_running():
            ...
            state = runShellCommandAndReturnOutputAsListWithChangedDir(
                command_cert_manager, Extension.TKG_EXTENSION_LOCATION
            )
            if state[1] != 0:
            ...
            if waitForGrepProcess(list1, list2, "cert-manager", Extension.TKG_EXTENSION_LOCATION)[1] == 500:
            ...
            list1 = ["kubectl", "get", "pods", "-A"]
            list2 = ["grep", "kapp"]
            wait = waitForGrepProcess(list1, list2, "kapp", Extension.TKG_EXTENSION_LOCATION)
            if wait[1] == 500:
    ...
    if service == "ingress" or service == "all":
        ...
        if not verifyPodsAreRunning("contour", command_contour[0], RegexPattern.RECONCILE_SUCCEEDED):
            ...
            state_contour = runShellCommandAndReturnOutputAsListWithChangedDir(
                command_contour, Extension.CONTOUR_LOCATION
            )
            ...
            change_repo_contour = [
                "sh",
                "./common/injectValue.sh",
                Extension.CONTOUR_LOCATION + "/vsphere/contour-data-values-lb.yaml.example",
                "contour",
                repo_address,
            ]
            ...
            state_contour_copy = runShellCommandAndReturnOutputAsListWithChangedDir(
                command_contour_copy, Extension.CONTOUR_LOCATION
            )
            ...
            state_contour_secret = runShellCommandAndReturnOutputAsListWithChangedDir(
                command_contour_secret, Extension.CONTOUR_LOCATION
            )
            if state_contour_copy[1] != 0:
                ...
                return jsonify(d), 500
            change_repo_contour_extension = [
                "sh",
                "./common/injectValue.sh",
                Extension.CONTOUR_LOCATION + "/contour-extension.yaml",
                "app_extension",
                repo_address + "/" + Extension.APP_EXTENSION,
            ]
            state_change_repo_contour_extension = runShellCommandAndReturnOutput(change_repo_contour_extension)
            if state_change_repo_contour_extension[1] != 0:
                current_app.logger.error(
                    "Failed to change contour repo in extension file " + str(state_change_repo_contour_extension[0])
                )
                d = {
                    "responseType": "ERROR",
                    "msg": "Failed to change contour repo in extension file  "
                    + str(state_change_repo_contour_extension[0]),
                    "STATUS_CODE": 500,
                }
                return jsonify(d), 500
...
def deploy_fluent_bit(end_point, cluster):
    try:
        ...
        if isEnvTkgs_ns(env) and isEnvTkgs_wcp(env) and checkTmcEnabled(env):
