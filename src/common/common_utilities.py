...
from common.operation.constants import AviSize, Extentions, MarketPlaceUrl, ResourcePoolAndFolderName, Cloud, Versions, \
    AkoType, \
    CIDR, PLAN, \
    TmcUser, \
    CertName, \
    Vcenter, VrfType, Repo, AppName, Type, VCF, ControllerLocation, KubernetesOva, EnvType, ServiceName, \
    TKG_Package_Details
import os
import re
import socket
    ...
    try:
        ...
        if fetched_ip is None:
            ...
            if check_controller_up is None:
                current_app.logger.error("Controller with IP: " + fetched_ip[0] + " is not UP, recommended to cleanup"
...
def checkRepositoryAdded(env):
    set_cmd = ["tanzu", "config", "set",
               "features.package.kctrl-command-tree", "true"]
    runProcess(set_cmd)
    if checkAirGappedIsEnabled(env):
        try:
            time.sleep(60)
            ...
            if not str(status[0]).__contains__(REPOSITORY_URL):
                list_command = ["tanzu", "package", "repository", "add", Repo.NAME, "--url",
                                os.path.join(REPOSITORY_URL,
                                             f'{os.sep}'.join(TKG_Package_Details.REPOSITORY_URL.split(os.sep)[2:])),
                                "-n", "tkg-system"]
        ...
        try:
            ...
            status = runShellCommandAndReturnOutputAsList(validate_command)
            wait_time = 0
            while status[1] != 0 and "the server is currently unable to handle the request" in str(status[0]) \
                    and wait_time < 300:
...
def installExtentionFor14(service_name, cluster, env):
    ...
    if service == "certmanager" or service == "all":
        ...
        if not verifyPodsAreRunning(AppName.CERT_MANAGER, command_cert[0], RegexPattern.RECONCILE_SUCCEEDED):
            ...
            install_command = ["tanzu", "package", "install", AppName.CERT_MANAGER, "--package",
                               "cert-manager.tanzu.vmware.com", "--namespace", f"{AppName.CERT_MANAGER}-package",
                               "--version", state]
    ...
    if service == "ingress" or service == "all":
        ...
        if not verifyPodsAreRunning(AppName.CONTOUR, command_cert[0], RegexPattern.RECONCILE_SUCCEEDED):
            ...
            out = runShellCommandAndReturnOutputAsList(verify_ns)
            for item in out[0]:
                if f"tanzu-system-ingress" in item:
                    break
            else:
            ...
            out = runShellCommandAndReturnOutputAsList(verify_ns)
            for item in out[0]:
                if f"tanzu-contour-ingress" in item:
                    break
            else:
                create_ns_cmd = ["kubectl", "create", "ns", "tanzu-contour-ingress"]
                runProcess(create_ns_cmd)

            install_command = ["tanzu", "package", "install", AppName.CONTOUR, "--package",
    ...
    try:
        if not getClusterStatusOnTanzu(sharedClusterName, "cluster"):
            ...
            if Tkg_version.TKG_VERSION == "2.1":
                ...
                set_cmd = ["tanzu", "config", "set",
                           "features.cluster.auto-apply-generated-clusterclass-based-configuration", "true"]
                runProcess(set_cmd)
...
def template14deployYaml(sharedClusterName, clusterPlan, datacenter, dataStorePath,
    ...
    FileHelper.write_to_file(
        t.render(config=vsSpec, clustercidr=clustercidr, sharedClusterName=sharedClusterName, clusterPlan=clusterPlan,
                 ...
                 avi_label_key=AkoType.KEY, avi_label_value=ako,
                 ...
                 identity_mgmt_type=identity_mgmt_type, tkg_version=Tkg_version.TKG_VERSION,
...
def checkClusterSizeForTo(env):
    current_app.logger.info(
        "Recommend to use Tanzu Observability and Tanzu Service Mesh integration with cluster size large or extra-large")
    ...
    if my_file.exists():
        ...
        if checksum is None:
            ...
            if original_checksum.strip() == checksum.strip():
                ...
                current_app.logger.warn("NSX ALB ova is present in /tmp/ directory but checksum is incorrect, "
                                        "deleting the file...")
...
def validateChecksum(filename, refreshToken, version, baseOS):
    ...
    if checksum is None:
        ...
        if original_checksum.strip() == checksum.strip():
            ...
            current_app.logger.warn("Kubernetes OVA is present in /tmp/ directory but checksum is incorrect, "
                                    "deleting the file...")
...
def getNetworkDetails(ip, csrf2, managementNetworkUrl, startIp, endIp, prefixIp, netmask, isSeRequired, aviVersion, env="vsphere"):
    ...
    try:
        ...
        if not verifyPodsAreRunning(appName, command_fluent_bit[0],
            ...
            if version is None:
                ...
                d = {
                    "responseType": "ERROR",
                    "msg": "Capture the available Fluent bit version",
                    "STATUS_CODE": 500
                }
                return jsonify(d), 500
        get_repo = ["kubectl", "-n", "tkg-system", "get", "packages",
        ...
        deploy_fluent_bit_command = ["tanzu", "package", "install", Tkg_Extention_names.FLUENT_BIT.lower(),
                                     "--package", Tkg_Extention_names.FLUENT_BIT.lower() + ".tanzu.vmware.com",
                                     "--version", version, "--values-file", yamlFile, "--namespace", extra_ns]
...
def check_tanzu_license(assignedLicense):
    try:
        ...
        return "No license found forTanzu Standard activation for vSphere", False
    except Exception as e:
    ...
    if response_csrf.status_code != 200:
        ...
        if createVs:
            ...
            getVIPNetworkDetails = getNetworkDetailsVip(ip, csrf2, vip_network_url, startIp, endIp, prefixIpNetmask[0],
                                                        prefixIpNetmask[1], avi_version, env)
        ...
        for ip in ip_addr:
            if ping_test("ping -c 1 " + ip.split("/")[0]) != 0:
                current_app.logger.warn(
                    "Ping test failed for " + ip + " gateway. It is Recommended to fix this before proceeding with deployment")
                time.sleep(30)
            else:
    ...
    for fl in cert_files:
        if os.path.splitext(fl)[1] == '.pem':
            if os.path.exists(fl):
                ...
                copy_command = "cp {source_file} {destination_file}".format(source_file=fl,
                                                                            destination_file=Env.UPDATED_YTT_FILE_LOCATION)
...
def tmcBodyClusterCreation(managementClusterName, provisionerName, clusterName, clusterGroupName, podCidr, serviceCidr,
                           sshKey, vcenter_server, cpu, diskGib, memoryMib, workerNodeCount, labels, version,
                           datacenter, datastore, folder, network, resourcePool, osName, osVersion, osArch,
                           templatePath, proxyName, controlPlaneNodeCount):
    if not proxyName:
        body = {
