...
from common.operation.constants import Versions, CIDR, TmcUser, \
    CertName, \
    VrfType, Repo, AppName, Type, VCF, ControllerLocation, KubernetesOva, EnvType
...
def registerWithTmc(management_cluster, env, isProxy, type):
    if not checkTmcRegister(management_cluster, True):
        ...
        if str(isProxy).lower() == "true":
            ...
            listOfCommandRegister = ["tmc", "managementcluster", "register", management_cluster, "-c", "default", "-p",
        ...
        if not verifyPodsAreRunning("contour", command_contour[0], RegexPattern.RECONCILE_SUCCEEDED):
            ...
            if state_contour[1] != 0:
                ...
                if state_contour[1] != 0:
                    ...
                    d = {
                        "responseType": "ERROR",
                        "msg": "Failed to apply countour " + str(state_contour[0]),
                        "ERROR_CODE": 500
                    }
                    return jsonify(d), 500
            else:
...
def getOvaMarketPlace(filename, refreshToken, version, baseOS):
    ...
    current_app.logger.info("Downloading kfubernetes ova - " + ovaName)
...
def registerTMCTKGs(vCenter, vCenter_user, VC_PASSWORD):
    ...
    try:
        ...
        if checkTmcRegister(supervisor_cluster, True):
            ...
            runProcess(listOfCmdTmcLogin)
            listOfCommandRegister = ["tmc", "managementcluster", "register", supervisor_cluster, "-c", "default", "-p",
                                     "TKGS"]
        ...
        for dns in dns_server:
            for avi_fqdn, avi_ip in avi_controller_fqdn_ip_dict.items():
                ...
                for ip in fqdn_ip_map[0]:
                    if not str(ip).__contains__(avi_ip):
                        return None, 500
                    else:
                        current_app.logger.info("Found DNS entry for " + avi_fqdn + " : " + ip)
        return "Successfully validated NSX ALB Fqdn and Ip entry in DNS Server", 200
    except:
        return None, 500
