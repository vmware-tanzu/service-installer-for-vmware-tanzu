...
def checkAviL7Enabled(env):
    if env == Env.VSPHERE or env == Env.VCF:
        try:
            enable_L7 = str(request.get_json(force=True)['tkgComponentSpec']['aviComponents']['enableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    elif env == Env.VMC:
        try:
            enable_L7 = str(request.get_json(force=True)['componentSpec']['aviComponentSpec']['enableAviL7'])
        except Exception as e:
            enable_L7 = "false"
    else:
        enable_L7 = "false"
...
def tmcBodyClusterCreation(managementClusterName, provisionerName, clusterName, clusterGroupName, podCidr, serviceCidr,
                           ...
                           datacenter, datastore, folder, network, resourcePool, osName, osVersion, osArch,templatePath, proxyName):
    if not proxyName:
        body = {
