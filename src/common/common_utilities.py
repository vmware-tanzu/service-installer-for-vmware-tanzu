    ...
    try:
        ...
        if enable_arcas_proxy.lower() == "true":
            if isEnvTkgs_wcp(env) or isEnvTkgs_ns(env):
                httpProxy = str(request.get_json(force=True)['tkgsComponentSpec']['tkgServiceConfig']['proxySpec']['httpProxy'])
                httpsProxy = str(request.get_json(force=True)['tkgsComponentSpec']['tkgServiceConfig']['proxySpec']['httpsProxy'])
                noProxy = str(request.get_json(force=True)['tkgsComponentSpec']['tkgServiceConfig']['proxySpec']['noProxy'])
    ...
    if service == "ingress" or service == "all":
        ...
        if not verifyPodsAreRunning(AppName.CONTOUR, command_cert[0], RegexPattern.RECONCILE_SUCCEEDED):
            ...
            install_command = ["tanzu", "package", "install", AppName.CONTOUR, "--package-name",
                               ...
                               "package-tanzu-system-contour", "--create-namespace"]
            states = runShellCommandAndReturnOutputAsList(install_command)
            if states[1] != 0:
                for r in states[0]:
...
def downloadAviController(env):
    ...
    if not refreshToken:
        ...
        if down[0] is None:
            ...
            current_app.logger.error("AVI OVA download from MarketPlace failed, please cross check if provided MarketPlace refresh token is valid")
            d = {
                "responseType": "ERROR",
                "msg": down[1],
                "STATUS_CODE": 500
    ...
    try:
        if env == Env.VCD:
            ...
            if deploy_avi.lower() == "false":
                ...
                enable_avi_ha = str(request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['aviComponentsSpec']['enableAviHa'])
        elif env == Env.VMC:
            ...
            if license.assignedLicense.name.__contains__('vCenter Server'):
                ...
                for prop in properties:
                    if prop.key == "ProductName" and prop.value.__contains__("VMware VirtualCenter Server"):
                        ...
                        status = verify_expired(expiration_date)
                        if status[1]:
                            current_app.logger.warn("ERROR: vCenter Server Evaluation Mode expiration check failed")
                        else:
