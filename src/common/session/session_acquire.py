...
from common.common_utilities import checkAirGappedIsEnabled, isEnvTkgs_ns, isEnvTkgs_wcp
...
from common.operation.constants import ControllerLocation, Env
...
def fetch_vmc_env(spec):
    try:
        current_app.config["access_token"] = CspClient(
            current_app.config, spec["envSpec"]["sddcRefreshToken"]
        ).get_access_token()
    ...
    response_body = {"responseType": "SUCCESS", "msg": "Captured VMC setup details successfully", "STATUS_CODE": 200}
...
def login(current_user):
    try:
        ...
        response_body = {"responseType": "ERROR", "msg": "No env headers passed", "STATUS_CODE": "400"}
        current_app.logger.error(response_body["msg"])
    ...
    try:
        if env == Env.VMC:
            ...
            # TODO: Don't need to populate content lib and ova; We can manage this from spec
            # defaults once it is implemented using pydantic model
            jwt = spec["marketplaceSpec"]["refreshToken"]
            current_app.config["VC_CONTENT_LIBRARY_NAME"] = (
                ControllerLocation.CONTROLLER_CONTENT_LIBRARY if jwt else spec["envSpec"]["contentLibraryName"]
            )
            current_app.config["VC_AVI_OVA_NAME"] = (
                ControllerLocation.CONTROLLER_NAME if jwt else spec["envSpec"]["aviOvaName"]
            )
            ...
            if checkAirGappedIsEnabled(env):
                VC_Content_Library_name = spec["envSpec"]["vcenterDetails"]["contentLibraryName"]
            ...
            if not VC_Content_Library_name:
                d = {"responseType": "ERROR", "msg": "VC Content Library Name is not provided", "STATUS_CODE": 500}
            ...
            if not VC_AVI_OVA_NAME:
                d = {"responseType": "ERROR", "msg": "VC AVI Ova Name is not provided", "STATUS_CODE": 500}
            ...
            response_body = {"responseType": "ERROR", "msg": "Un-recognised env", "STATUS_CODE": 500}
            current_app.logger.error("Unrecognised env")
            return jsonify(response_body), 500
    except Exception as ex:
        response_body = {"responseType": "ERROR", "msg": str(ex) + " check the input file", "STATUS_CODE": 400}
        current_app.logger.error("Failed to get environment details, please check the input file " + str(ex))
        return jsonify(response_body), 500
    response_body = {"responseType": "SUCCESS", "msg": "Environment details captured successfully", "STATUS_CODE": 200}
    current_app.logger.info(response_body["msg"])
    return jsonify(response_body), 200
