...
from common.constants.constants import Constants, KubectlCommands, TKGCommands
...
class TkgCliClient:
    ...
    def check_cluster_exists(self, cluster_name, expected_status="running"):
        ...
        cluster_list = [
            ...
            if cluster["name"] == cluster_name and cluster["status"] == expected_status
        ]
    ...
    def install_package(self, name, package, namespace, version, options=""):
        ...
        cmd = TKGCommands.INSTALL_PACKAGE.format(
            name=name, pkgName=package, namespace=namespace, version=version, options=options
        )
        return self.cmd_runner.run_cmd(cmd)
    ...
    def get_installed_package_details(self, name, namespace, json_format=True):
        ...
        if json_format:
            cmd = TKGCommands.GET_PACKAGE_DETAILS.format(
                name=name, namespace=namespace, options=KubectlCommands.OUTPUT_JSON
            )
            exit_code, output = self.cmd_runner.run_cmd_output(cmd)
            return json.loads(output)
        else:
