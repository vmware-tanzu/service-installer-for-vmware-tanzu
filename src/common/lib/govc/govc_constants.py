...
class GovcCommands(str, Enum):
    ...
    LIST_ALL_VMS = "govc ls /{datacenter}/vm"
    DEPLOY_OVA_TEMPLATE = (
        "govc import.ova -options {json_path} -dc={datacenter} -ds={datastore} "
        "-pool={cluster}/Resources {ova_location}"
    )
    DEPLOY_SE_OVA = (
        "govc import.ova -options {json_path} -dc={datacenter} -ds={datastore} "
        "-pool={pool} -folder={folder} {ova_location}"
    )
