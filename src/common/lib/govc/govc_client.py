...
class GOVClient:
    ...
    def list_all_vms(self):
        cmd = GovcCommands.LIST_ALL_VMS.format(datacenter=self.data_center)
        exit_code, output = self.cmd_runner.run_cmd_output(cmd)
        return output if output is None or output.strip() == "" else output.strip().split("\n")

    def deploy_ova_template(self, json_path, ova_location):
        cmd = GovcCommands.DEPLOY_OVA_TEMPLATE.format(
            json_path=json_path,
            datacenter=self.data_center,
    ...
    def import_se_ova(self, resource_pool, folder, json_path, ova_location):
        cmd = GovcCommands.DEPLOY_SE_OVA.format(
            json_path=json_path,
            datacenter=self.data_center,
            datastore=self.data_store,
            pool=resource_pool,
