...
from common.replace_value import replaceCertConfig, replaceValueSysConfig
...
class AVITemplateOperations(AVIBaseOperations):
    ...
    def generate_ssl_certificate(self, list_of_ips):
        ...
        body = AlbPayload.SELF_SIGNED_CERT.format(name=self.cert_name, common_name=CertName.COMMON_NAME, san_list=san)
        url = AlbEndpoint.CRUD_SSL_CERT.format(ip=self.avi_host)
        response = RequestApiUtil.exec_req("POST", url, headers=self.headers, data=body, verify=False)
        if not RequestApiUtil.verify_resp(response, status_code=HTTPStatus.CREATED):
            return None, response.text
    ...
    def _ipam_creator(self, ipam_body):
        ...
        response = RequestApiUtil.exec_req("POST", url, headers=self.headers, data=ipam_body, verify=False)
        if not RequestApiUtil.verify_resp(response, status_code=HTTPStatus.CREATED):
            return None, response.text
        else:
            return response.json()["url"], "SUCCESS"
    ...
    def create_ipam(self, main_network_url, data_network_url, vip_network_url, name):
        ...
        ipam_body = AlbPayload.IPAM_BODY.format(
            name=name, network_url=main_network_url, data_network_url=data_network_url, vip_network_url=vip_network_url
        )
        return self._ipam_creator(ipam_body=ipam_body)

    def create_ipam_nsxt_cloud(self, vip_network_url, mgmt_network_url, name):
        """
        ...
        ipam_body = AlbPayload.NSXT_IPAM_BODY.format(
            name=name, mgmt_network_url=mgmt_network_url, vip_network_url=vip_network_url
        )
    ...
    def create_ipam_arch(self, vip_network_url, data_network_url, name):
        ...
        ipam_body = AlbPayload.ORCH_IPAM_BODY.format(
            name=name, data_network_url=data_network_url, vip_network_url=vip_network_url
        )
        return self._ipam_creator(ipam_body)
    ...
    def get_ipam_details(self, url_ipam, dataNetwork):
        body = {}
        url = url_ipam
        ...
        if response_csrf.status_code != 200:
            ...
            json_object = response_csrf.json()
            FileHelper.delete_file(AVIDataFiles.DETAILS_OF_IPAM)
            FileHelper.dump_json(file=AVIDataFiles.DETAILS_OF_IPAM, json_dict=json_object)
            dicti = {"nw_ref": dataNetwork}
            liti = []
            data = FileHelper.load_json(spec_path=AVIDataFiles.DETAILS_OF_IPAM)
            for a in data["internal_profile"]["usable_networks"]:
                liti.append(a)
            liti.append(dicti)
            replaceValueSysConfig(AVIDataFiles.DETAILS_OF_IPAM, "internal_profile", "usable_networks", liti)
            return response_csrf.json(), "SUCCESS"

    def update_ipam_with_data_network(self, ipamUrl):
        new_cloud_json = FileHelper.load_json(spec_path=AVIDataFiles.DETAILS_OF_IPAM)
        json_object = json.dumps(new_cloud_json, indent=4)
        response_csrf = RequestApiUtil.exec_req("PUT", ipamUrl, headers=self.headers, data=json_object, verify=False)
        if response_csrf.status_code != 200:
            return None, response_csrf.text
        else:
            return response_csrf.json(), "SUCCESS"
