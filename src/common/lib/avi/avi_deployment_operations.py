...
class AVIDeploymentOps:
    ...
    def _deploy_avi_controller(self, avi_vm_name, deploy_options, cpu, memory):
        ...
        return self.govc_client.get_vm_ip(wait_time="30m", avi_vm_name=avi_vm_name)
    ...
    def _configure_avi(self):
        ...
        if csrf == "SUCCESS":
            ...
            if self.avi_admin_ops.set_avi_admin_password(first_csrf=csrf) is None:
    ...
    def deploy_and_configure_avi(self, deploy_options, perform_other_task, vm_name):
        ...
        """
        try:
            fetched_ip = self.govc_client.get_vm_ip(avi_vm_name=vm_name)
            if fetched_ip is not None:
                ...
                self.avi_admin_ops = AVIAdminOps(avi_host=fetched_ip, avi_password=self.avi_password)
                ...
                if self.avi_admin_ops.check_controller_is_up() is None:
