...
def mgmt_cluster_cleanup(govc_client: GOVClient, env, VC, saas_util, spec):
    ...
    if saas_util.check_tmc_enabled():
        ...
        if env == Env.VMC:
            os.putenv("TMC_API_TOKEN", spec.saasEndpoints.tmcDetails.tmcRefreshToken)
        else:
            os.putenv(
                "TMC_API_TOKEN",
                spec.envSpec.saasEndpoints.tmcDetails.tmcRefreshToken,
            )
        if not delete_tmc_cluster([mgmt_cluster], True):
...
def cluster_cleanup(cluster_type, govc_client, env, VC, saas_util, spec):
    ...
    if saas_util.check_tmc_enabled():
        ...
        if not delete_tmc_cluster([cluster], False):
    ...
    try:
        ...
        if saas_util.check_tmc_enabled:
            ...
            if isEnvTkgs_wcp(Env.VSPHERE):
                if not delete_tmc_cluster(workload_cluster_name, False):
                    current_app.logger.warn("Failed to delete workload clusters from TMC")
                    current_app.logger.warn("Try deleting it using :'tmc cluster delete <clusterName>' command")
        ...
        if saas_util.check_tmc_enabled:
            ...
            if isEnvTkgs_wcp(Env.VSPHERE):
                if not delete_tmc_cluster(
                    [spec.envSpec.saasEndpoints.tmcDetails.tmcSupervisorClusterName],
                    True,
                ):
                    current_app.logger.warn("Failed to delete Management clusters from TMC")
                    current_app.logger.warn(
...
def tkgs_cleanup(VC, saas_util, spec):
    try:
        ...
        current_app.logger.info(message)
        return RequestApiUtil.send_ok(message), HTTPStatus.INTERNAL_SERVER_ERROR
    except Exception as e:
        current_app.logger.error(str(e))
...
def tkgm_cleanup(env, VC, saas_util, spec):
    ...
    if saas_util.check_tmc_enabled():
        ...
        if not delete_tmc_cluster([VC[VCenter.workload_cluster], VC[VCenter.shared_cluster]], False):
            current_app.logger.warn("Failed to delete workload clusters from TMC")
            current_app.logger.warn("Try deleting it using :'tmc cluster delete <clusterName>' command")
        if not delete_tmc_cluster([VC[VCenter.management_cluster]], True):
            current_app.logger.warn(
