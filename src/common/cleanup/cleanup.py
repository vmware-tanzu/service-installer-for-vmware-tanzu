...
from common.session.session_acquire import fetch_vmc_env, login
...
def cleanup_environment():
    ...
    if env == Env.VSPHERE or env == Env.VCF:
        if isEnvTkgs_ns(env):
            current_app.logger.error(
                "Wrong spec file provided for cleanup, please use the spec file which was used " "for enabling WCP"
            )
            d = {"responseType": "ERROR", "msg": "Wrong environment provided for cleanup", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not (vCenter or vCenter_user or VC_PASSWORD):
            ...
            d = {"responseType": "ERROR", "msg": "Failed to fetch VC details", "STATUS_CODE": 500}
            return jsonify(d), 500
    ...
    if isEnvTkgs_wcp(env):
        tkgs_response = tkgs_cleanup(
            vCenter=vCenter, vCenter_user=vCenter_user, VC_PASSWORD=VC_PASSWORD, vc_cluster=vCenter_cluster
        )
        if tkgs_response[1] != 200:
            current_app.logger.error("Cleanup failed - " + tkgs_response[0].json["msg"])
            d = {"responseType": "ERROR", "msg": "Cleanup failed - " + tkgs_response[0].json["msg"], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if tkg_response[1] != 200:
            ...
            d = {"responseType": "ERROR", "msg": "Cleanup failed - " + tkg_response[0].json["msg"], "STATUS_CODE": 500}
            return jsonify(d), 500

    current_app.logger.info("Clusters are deleted successfully. Performing other components cleanup")

    response = delete_common_comp(
    ...
    if response[1] != 200:
        ...
        d = {"responseType": "ERROR", "msg": "Cleanup failed - " + response[0].json["msg"], "STATUS_CODE": 500}
        return jsonify(d), 500

    current_app.logger.info("Cleanup for " + env + " is successful")
    d = {"responseType": "SUCCESS", "msg": "Cleanup completed ", "STATUS_CODE": 200}
...
def tkgs_cleanup(vCenter, vCenter_user, VC_PASSWORD, vc_cluster):
    try:
        sess = requests.post(
            "https://" + vCenter + "/rest/com/vmware/cis/session", auth=(vCenter_user, VC_PASSWORD), verify=False
        )
        ...
        if id[1] != 200:
            d = {"responseType": "ERROR", "msg": id[0], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if delete:
            ...
            if enabled[0]:
                ...
                if disable[0] is None:
                    ...
                    d = {"responseType": "ERROR", "msg": disable[1], "STATUS_CODE": 500}
                    return jsonify(d), 500
            else:
        ...
        if not kubectl_configs_status[0]:
            ...
            d = {"responseType": "ERROR", "msg": kubectl_configs_status[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        d = {"responseType": "SUCCESS", "msg": "TKGs components deleted successfully", "STATUS_CODE": 200}
        return jsonify(d), 200
    except Exception as e:
        current_app.logger.error(str(e))
        d = {"responseType": "ERROR", "msg": "Exception occured while performing cleanup", "STATUS_CODE": 500}
        return jsonify(d), 500
...
def tkgm_cleanup(env):
    if env == Env.VMC:
        ...
        shared_cluster = request.get_json(force=True)["tkgComponentSpec"]["tkgMgmtComponents"][
            "tkgSharedserviceClusterName"
        ]
        workload_cluster = request.get_json(force=True)["tkgWorkloadComponents"]["tkgWorkloadClusterName"]
    elif env == Env.VCF:
        management_cluster = request.get_json(force=True)["tkgComponentSpec"]["tkgMgmtComponents"]["tkgMgmtClusterName"]
        shared_cluster = request.get_json(force=True)["tkgComponentSpec"]["tkgSharedserviceSpec"][
            "tkgSharedserviceClusterName"
        ]
        workload_cluster = request.get_json(force=True)["tkgWorkloadComponents"]["tkgWorkloadClusterName"]
        ...
        for cluster in worload_clusters:
            ...
            if getCluster(cluster):
                if delete_cluster(cluster):
                    ...
                    d = {"responseType": "ERROR", "msg": "Failed to delete cluster - " + cluster, "STATUS_CODE": 500}
                    return jsonify(d), 500
            else:
    ...
    d = {"responseType": "SUCCESS", "msg": "TKG Clusters and Nodes deleted successfully ", "STATUS_CODE": 200}
    return jsonify(d), 200
...
def delete_common_comp(
    govc_client: GovcClient, env, vCenter, vCenter_user, VC_PASSWORD, datacenter, cluster, parent_resourcepool
):
    ...
    if not avi_cleanup_status[0]:
        ...
        d = {"responseType": "ERROR", "msg": avi_cleanup_status[1], "STATUS_CODE": 500}
        return jsonify(d), 500
    ...
    if not cleanup_sivt[0]:
        ...
        d = {"responseType": "ERROR", "msg": cleanup_sivt[1], "STATUS_CODE": 500}
        return jsonify(d), 500
    ...
    if not delete_folders(env, vCenter, vCenter_user, VC_PASSWORD):
        ...
        d = {"responseType": "ERROR", "msg": "Failed to delete folders which were created by SIVT", "STATUS_CODE": 500}
        return jsonify(d), 500
    ...
    if env == Env.VMC or env == Env.VCF or env == Env.VSPHERE:
        if not delete_kubernetes_templates(govc_client, vCenter, vCenter_user, VC_PASSWORD, datacenter, uuid):
            ...
            d = {"responseType": "ERROR", "msg": "Failed to delete Kubernetes templates", "STATUS_CODE": 500}
            return jsonify(d), 500
        if not delete_config_yaml():
            current_app.logger.error(
                "Failed to delete config.yaml files /root/.config/tanzu/config.yaml "
                "and /root/.config/tanzu/tkg/config.yaml"
            )
            d = {"responseType": "ERROR", "msg": "Failed to delete config.yaml files", "STATUS_CODE": 500}
            return jsonify(d), 500
    ...
    if env == Env.VCF:
        ...
        if headers_[0] is None:
            d = {"responseType": "ERROR", "msg": str(headers_[1]), "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not delete_policy[0]:
            ...
            d = {"responseType": "ERROR", "msg": delete_policy[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not delete_groups[0]:
            ...
            d = {"responseType": "ERROR", "msg": delete_groups[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not delete_services[0]:
            ...
            d = {"responseType": "ERROR", "msg": delete_services[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not delete_segments[0]:
            ...
            d = {"responseType": "ERROR", "msg": delete_segments[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not del_cgw_rules[0]:
            ...
            d = {"responseType": "ERROR", "msg": del_cgw_rules[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not del_cgw_rules[0]:
            ...
            d = {"responseType": "ERROR", "msg": del_mgw_rules[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not del_services[0]:
            ...
            d = {"responseType": "ERROR", "msg": del_services[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        current_app.logger.info("Services deleted successfully... ")
        ...
        if not del_mgw_inventory_grps[0]:
            ...
            d = {"responseType": "ERROR", "msg": del_mgw_inventory_grps[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not del_cgw_inventory_grps[0]:
            ...
            d = {"responseType": "ERROR", "msg": del_cgw_inventory_grps[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        del_segments = delete_nsxt_components(
            url + "/policy/api/v1/infra/tier-1s/cgw/segments/", headers, list_network_segments(env)
        )
        if not del_segments[0]:
            current_app.logger.error(del_segments[1])
            d = {"responseType": "ERROR", "msg": del_segments[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        current_app.logger.info("Network segments deleted successfully")

    d = {"responseType": "SUCCESS", "msg": "Common Components delete successful", "STATUS_CODE": 200}
    return jsonify(d), 200
...
def cleanup_resources_list():
    ...
    if not isEnvTkgs_wcp(env) and not isEnvTkgs_ns(env):
        ...
        kubernetes_templates = get_deployed_templates(
            vCenter, vCenter_user, VC_PASSWORD, get_avi_uuid(vCenter, vCenter_user, VC_PASSWORD)
        )[1]
        ...
        if getWCPStatus(cluster_id)[0]:
            supervisor_cluster = (
                "Workload Control Plane (WCP) will be deactivated on cluster: [ " + vCenter_cluster + " ]"
            )
        else:
