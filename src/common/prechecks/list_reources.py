...
def listResources():
    ...
    if env == Env.VSPHERE or env == Env.VCF:
        ...
        str_enc = str(request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterSsoPasswordBase64'])
        base64_bytes = str_enc.encode('ascii')
        enc_bytes = base64.b64decode(base64_bytes)
        VC_PASSWORD = enc_bytes.decode('ascii').rstrip("\n")
...
def getFilesInContentLibrary():
    ...
    if env == Env.VSPHERE or env == Env.VCF:
        ...
        vCenter_user = request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterSsoUser']
        str_enc = str(request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterSsoPasswordBase64'])
        library_name = request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['contentLibraryName']
        base64_bytes = str_enc.encode('ascii')
        enc_bytes = base64.b64decode(base64_bytes)
        VC_PASSWORD = enc_bytes.decode('ascii').rstrip("\n")
    elif env == Env.VMC:
...
def getClusters():
    try:
        ...
        if env == Env.VSPHERE or env == Env.VCF:
            ...
            vCenter_user = request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterSsoUser']
            str_enc = str(request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterSsoPasswordBase64'])
            datacenter_name = request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails']['vcenterDatacenter']
            base64_bytes = str_enc.encode('ascii')
            enc_bytes = base64.b64decode(base64_bytes)
            VC_PASSWORD = enc_bytes.decode('ascii').rstrip("\n")
