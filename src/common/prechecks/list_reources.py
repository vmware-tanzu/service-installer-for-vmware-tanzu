...
from common.constants.tmc_api_constants import VeleroAPI
...
from common.operation.constants import Env, MarketPlaceUrl, Type
...
from common.prechecks.precheck import (
    checkClusterNamespace,
    get_cluster,
    get_legacy_classy_cluster_version_list,
...
@token_required
def getTer1Details(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
        ...
        if len(list_of_display_name) < 1:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to get tier1 details, list is empty ", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        d = {"responseType": "ERROR", "msg": "Failed to get details of nsxt tier 1 " + str(e), "STATUS_CODE": 500}
        return jsonify(d), 500


@vcenter_resources.route("/api/tanzu/listResources", methods=["POST"])
@token_required
def listResources(current_user):
    env = envCheck()
    if env[1] != 200:
        current_app.logger.error("Wrong env provided " + env[0])
        d = {"responseType": "ERROR", "msg": "Wrong env provided " + env[0], "STATUS_CODE": 500}
    ...
    if env == Env.VSPHERE or env == Env.VCF:
        ...
        vCenter = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"]["vcenterAddress"]
        vCenter_user = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"]["vcenterSsoUser"]
        str_enc = str(
            request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"]["vcenterSsoPasswordBase64"]
        )
        base64_bytes = str_enc.encode("ascii")
        enc_bytes = base64.b64decode(base64_bytes)
        VC_PASSWORD = enc_bytes.decode("ascii").rstrip("\n")
        ...
        if status[1] != 200:
            d = {"responseType": "ERROR", "msg": "Failed to capture VMC setup details ", "STATUS_CODE": 500}
            return jsonify(d), 500
    ...
    try:
        ...
        try:
            ...
            d = {"responseType": "ERROR", "msg": "No datacenters found " + str(e), "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        for dc in datacenterss:
            ...
            try:
                list1 = get_ds(si, dc, None)
                if list1 is not None:
                    datastores.extend(list1)
            except Exception as e:
                errors.append(e)
        ...
        if errors:
            ...
            d = {"responseType": "ERROR", "msg": "Failed " + str(errors), "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        d = {"responseType": "ERROR", "msg": "Failed to connect to vcenter. " + str(e), "STATUS_CODE": 500}
...
@token_required
def getthumbprint(current_user):
    ...
    if thumb_sha1 != 500:
        ...
        d = {"responseType": "ERROR", "msg": "Failed to obtain VC thumbprint", "STATUS_CODE": 500}
...
def getLibraryFile(vcIp, vcUser, vcPassword):
    ...
    try:
        ...
        resp = requests.get(
            url + "rest/com/vmware/content/library", verify=False, headers={"vmware-api-session-id": session_id}
        )
        ...
        for library in library_ids:
            library_item = requests.get(
                url + "rest/com/vmware/content/library/item?library_id=" + library,
                verify=False,
            ...
            lib_name = requests.get(
                url + "rest/com/vmware/content/library/id:" + library,
                verify=False,
                headers={"vmware-api-session-id": session_id},
            )
        ...
        for item in item_list:
            resp_new_item = requests.get(
                url + "rest/com/vmware/content/library/item/id:" + item,
                verify=False,
                headers={"vmware-api-session-id": session_id},
            )
...
@token_required
def getFilesInContentLibrary(current_user):
    env = envCheck()
    if env[1] != 200:
        current_app.logger.error("Wrong env provided " + env[0])
        d = {"responseType": "ERROR", "msg": "Wrong env provided " + env[0], "STATUS_CODE": 500}
    ...
    if env == Env.VSPHERE or env == Env.VCF:
        ...
        library_name = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"][
            "contentLibraryName"
        ]
        base64_bytes = str_enc.encode("ascii")
        enc_bytes = base64.b64decode(base64_bytes)
...
@token_required
def storagePolicies(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        vCenter_user = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoUser"]
        str_enc = str(request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoPasswordBase64"])
...
@token_required
def listvmclasses(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        vCenter_user = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoUser"]
        str_enc = str(request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoPasswordBase64"])
        ...
        vm_classes_response = requests.request(
            "GET", url + "api/vcenter/namespace-management/virtual-machine-classes", headers=header, verify=False
        )
        if vm_classes_response.status_code != 200:
            d = {"responseType": "ERROR", "msg": "Failed to fetch VM classes", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if vm_classes:
            ...
            d = {"responseType": "ERROR", "msg": "No VM Class found", "STATUS_CODE": 500, "VM_CLASSES": vm_classes}
...
@token_required
def getKubernetesOvaVersions(current_user):
    # TODO: This method fetches all the Kube versions from marketplace -> Updated test it out
    ...
    if product.status_code != 200:
        ...
        for metalist in product.json()["response"]["data"]["metafilesList"]:
            if (
                metalist["groupname"]
                == (KubernetesOva.MARKETPLACE_PHOTON_GROUPNAME or KubernetesOva.MARKETPLACE_UBUTNU_GROUPNAME)
                and metalist["appversion"] == MarketPlaceUrl.TKG_PRODUCT_VERSION_LATEST
            ):
    ...
    if not kubeVersionList:
        ...
        current_app.logger.info(json.dumps(d))
        return json.dumps(d), 200


def validateKubeVersion(env, cluster):
    response = getKubernetesOvaVersions()
    current_app.logger.info(response)
    if response[1] == 200:
        response = json.loads(response[0])
        getK8sVersion = response["KUBE_VERSION_LIST"]
    else:
        current_app.logger.error("Failed to fetch supported KubeVersion from marketplace")
        d = {
            "responseType": "ERROR",
            "msg": "Failed to fetch supported KubeVersion from marketplace",
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
    current_app.logger.info(getK8sVersion)
    if env == Env.VMC:
        sharedKubeVersion = request.get_json(force=True)["componentSpec"]["tkgSharedServiceSpec"][
            "tkgSharedserviceKubeVersion"
        ]
        workloadKubeVersion = request.get_json(force=True)["componentSpec"]["tkgWorkloadSpec"]["tkgWorkloadKubeVersion"]
    if env == Env.VSPHERE:
        sharedKubeVersion = request.get_json(force=True)["tkgComponentSpec"]["tkgMgmtComponents"][
            "tkgSharedserviceKubeVersion"
        ]
        workloadKubeVersion = request.get_json(force=True)["tkgWorkloadComponents"]["tkgWorkloadKubeVersion"]
    elif env == Env.VCF:
        sharedKubeVersion = request.get_json(force=True)["tkgComponentSpec"]["tkgSharedserviceSpec"][
            "tkgSharedserviceKubeVersion"
        ]
        workloadKubeVersion = request.get_json(force=True)["tkgWorkloadComponents"]["tkgWorkloadKubeVersion"]

    if cluster == Type.SHARED:
        kubeVersion = sharedKubeVersion
    if cluster == Type.WORKLOAD:
        kubeVersion = workloadKubeVersion
    if kubeVersion in getK8sVersion:
        current_app.logger.info("Successfully Validated KubVersion for" + cluster + " cluster")
        d = {
            "responseType": "SUCCESS",
            "msg": "Successfully Validated KubVersion for " + cluster + " cluster",
            "STATUS_CODE": 200,
        }
        return jsonify(d), 200
    else:
        current_app.logger.error(
            "Invalid KubeVersion:" + kubeVersion + " for " + cluster + " cluster.Please specify from supported list"
        )
        d = {
            "responseType": "ERROR",
            "msg": "Invalid KubeVersion: "
            + kubeVersion
            + " for "
            + cluster
            + " cluster.Please specify from supported list",
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
@token_required
def getWCPEnabledClusters(current_user):
    try:
        wcp_cluster = []
...
@token_required
def getWorkloadNetworks(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        ...
        if vc_session[0] is None:
            d = {"responseType": "ERROR", "msg": vc_session[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not workload_networks:
            ...
            current_app.logger.info("Below is the list of workload networks found on cluster - " + cluster)
...
@vcenter_resources.route("/api/tanzu/getVcenterVersion", methods=["POST"])
@token_required
def get_vcenter_version(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        vcenter_username = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoUser"]
        password_b64 = str(request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoPasswordBase64"])
        password = CommonUtils.decode_password(password_b64)
        ssl_obj = VCenterSSLOperations(vCenter, vcenter_username, password)
        vcenter_8 = ssl_obj.verify_vcenter_version("8")
        if vcenter_8:
            v8 = "true"
            v7 = "false"
        else:
        ...
        d = {
            "responseType": "SUCCESS",
            "msg": "Checked vcenter versions successfully",
            "VCENTER_8": v8,
            "VCENTER_7": v7,
        }
        return jsonify(d), 200
    except Exception as e:
        current_app.logger.error("Failed to fetch vcenter version " + str(e))
        d = {
            "responseType": "ERROR",
            "msg": "Exception occurred while fetching vcenter version - " + str(e),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
@token_required
def getClusterVersions(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        cluster = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterCluster"]
        vcenter_username = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoUser"]
        ...
        if versions_output[1] != 200:
            ...
            d = {"responseType": "ERROR", "msg": versions_output[0].json["msg"], "STATUS_CODE": 500}
            return jsonify(d), 500

        alpha_clusters = []
        beta_clusters = []
        alpha, beta, status = get_legacy_classy_cluster_version_list(vCenter, vcenter_username, password, cluster)
        for version in versions_output[0]:
            value_list = version.split()
            if (value_list[2] and value_list[3]) == "True":
                if vcenter_8:
                    if value_list[0] in alpha:
                        alpha_clusters.append("v" + value_list[1])
                    elif value_list[0] in beta:
                        beta_clusters.append("v" + value_list[1])
                else:
                    alpha_clusters.append("v" + value_list[1])

        if not alpha_clusters and not beta_clusters:
            current_app.logger.error("Cluster versions list is empty")
            d = {
                "responseType": "ERROR",
                "msg": "Cluster versions list is empty",
                "STATUS_CODE": 500,
                "CLUSTER_VERSIONS_ALPHA": alpha_clusters,
                "CLUSTER_VERSIONS_BETA": beta_clusters,
            }
            return jsonify(d), 500
        else:
            current_app.logger.info("Below is the list of legacy cluster versions found - ")
            current_app.logger.info(alpha_clusters)
            current_app.logger.info("Below is the list of classy cluster versions found - ")
            current_app.logger.info(beta_clusters)
            d = {
                "responseType": "SUCCESS",
...
@token_required
def getAllNamespaces(current_user):
    try:
        vCenter = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterAddress"]
        cluster = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterCluster"]
        vcenter_username = request.get_json(force=True)["envSpec"]["vcenterDetails"]["vcenterSsoUser"]
...
@token_required
def getSupervisorClusters(current_user):
    try:
        current_app.logger.debug(request.get_json(force=True))
        tmc_url = request.get_json(force=True)["saasEndpoints"]["tmcDetails"]["tmcInstanceURL"]
        tmc_refresh_token = request.get_json(force=True)["saasEndpoints"]["tmcDetails"]["tmcRefreshToken"]
        list_mgmt_clusters = SaaSUtil.return_list_of_mgmt_clusters(tmc_url, tmc_refresh_token)
        response_status = list_mgmt_clusters[0]
        mgmt_clusters = list_mgmt_clusters[1]
        current_app.logger.info(mgmt_clusters)
        if response_status:
            mgmt_clusters = list_mgmt_clusters[1]
...
@token_required
def getSupervisorClusterHealth(current_user):
    try:
        ...
        tmc_refresh_token = request.get_json(force=True)["envSpec"]["saasEndpoints"]["tmcDetails"]["tmcRefreshToken"]
        state = SaaSUtil.check_cluster_state_on_tmc(tmc_url, tmc_refresh_token, super_cluster, True)
        if state[0] == "SUCCESS":
...
def getClusters(current_user):
    try:
        ...
        if env == Env.VSPHERE or env == Env.VCF:
            ...
            vCenter = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"]["vcenterAddress"]
            vCenter_user = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"][
                "vcenterSsoUser"
            ]
            str_enc = str(
            ...
            datacenter_name = request.get_json(force=True)["envSpec"]["aviCtrlDeploySpec"]["vcenterDetails"][
                "vcenterDatacenter"
            ]
            base64_bytes = str_enc.encode("ascii")
            enc_bytes = base64.b64decode(base64_bytes)
        ...
        if isEnvTkgs_ns(env):
            ...
            if wcp_clusters[1] != 200:
                d = {"responseType": "ERROR", "msg": wcp_clusters[0].json["msg"], "STATUS_CODE": 500}
        ...
        d = {"responseType": "ERROR", "msg": "Exception occurred while fetching list of clusters ", "STATUS_CODE": 500}
        return jsonify(d), 500


@vcenter_resources.route("/api/tanzu/getDatastores", methods=["POST"])
@token_required
def getDatastores(current_user):
    try:
        datastores = []
        env = envCheck()
        if env[1] != 200:
...
@token_required
def getNamespaceDetails(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
        ...
        if namespace_info[1] != 200:
            ...
            d = {"responseType": "ERROR", "msg": namespace_info[0].json["msg"], "STATUS_CODE": 500}
            return jsonify(d), 500
...
@token_required
def fetchClusterGroups(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
        ...
        if tmc_header[0] is None:
            ...
            d = {"responseType": "ERROR", "msg": tmc_header[1], "STATUS_CODE": 500}
            return jsonify(d), 500

        response = TmcVelero.list_cluster_groups(tmc_header[1], tmc_header[0])
        if not response[0]:
            current_app.logger.error(response[1])
            d = {"responseType": "ERROR", "msg": response[1], "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not cluster_groups:
            ...
            d = {"responseType": "ERROR", "msg": "Cluster groups list for data protection is empty", "STATUS_CODE": 500}
            return jsonify(d), 500
...
@token_required
def fetchCredentials(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
        ...
        if response.status_code != 200:
            ...
            d = {"responseType": "ERROR", "msg": "Failed to fetch data protection credentials", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not credentials:
            ...
            d = {"responseType": "ERROR", "msg": "Data Protection credentials list is empty", "STATUS_CODE": 500}
            return jsonify(d), 500
...
@token_required
def fetchTargetLocations(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
...
@token_required
def validateCredentials(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
        ...
        response = TmcVelero.validate_cluster_credential(spec, env, tmc_header[1], tmc_header[0], cluster_type)
        if not response[0]:
            d = {"responseType": "ERROR", "msg": response[1], "STATUS_CODE": 500}
            current_app.logger.error(response[1])
            return jsonify(d), 500
            ...
            d = {"responseType": "SUCCESS", "msg": response[1], "STATUS_CODE": 200}
            return jsonify(d), 200
...
@token_required
def validateTargetLocations(current_user):
    try:
        env = envCheck()
        if env[1] != 200:
            current_app.logger.error("Wrong env provided " + env[0])
        ...
        response = TmcVelero.validate_backup_location(spec, env, tmc_header[1], tmc_header[0], cluster_type)
        if not response[0]:
            d = {"responseType": "ERROR", "msg": response[1], "STATUS_CODE": 500}
            current_app.logger.error(response[1])
            return jsonify(d), 500
