...
from common.operation.ShellHelper import runShellCommandWithPolling, grabPipeOutput, runProcess, \
...
from common.common_utilities import checkMachineCountForTsm, checkClusterSizeForTo, envCheck, \
    ...
    get_cluster, getNetwork, isRunningInDocker
...
def precheck_env():
    ...
    if not skip_precheck:
        ...
        val = validate_proxy_starts_wit_http(env, isShared, isWorkload)
        if val != "Success":
            error_dict["PROXY_STARTS_WITH_HTTP"] = "Error: Unsupported Proxy protocol found for " + val + \
                                                   ", The Proxy URLs must start with http://"
        ...
        if not checkAirGappedIsEnabled(env):
            if not (isEnvTkgs_wcp(env) or isEnvTkgs_ns(env)):
                if checkTmcEnabled(env):
                    osFlavor = checkOSFlavorForTMC(env, isShared, isWorkload)
                    if osFlavor[1] != 200:
                        current_app.logger.warn(str(osFlavor[0].json['msg']))
                        error_dict["OS_TYPE_WITH_TMC"] = str(osFlavor[0].json['msg'])
                        # d = {
                        #     "responseType": "ERROR",
                        #     "msg": str(osFlavor[0].json['msg']),
                        #     "STATUS_CODE": 500
        ...
        if not (isEnvTkgs_wcp(env) or isEnvTkgs_ns(env)):
            if isWorkload:
                to = checkClusterSizeForTo(env)
                if to[1] != 200:
                    current_app.logger.warn(str(to[0].json['msg']))
                    error_dict["CLUSTER_SIZE_WITH_TO"] = str(to[0].json['msg'])
                    # d = {
                    #     "responseType": "ERROR",
                    #     "msg": str(to[0].json['msg']),
                    #     "STATUS_CODE": 500
        ...
        try:
            ...
            try:
                if str(vcVersion).startswith("9"):
                    current_app.logger.warn("Deployment of TKG 2.1.1 on vSphere 9.0 is not supported")
                    error_dict["VCENTER_VERSION_9"] = "Deployment of TKG 2.1.1 on vSphere 9.0 is not supported"
                    # d = {
                    #     "responseType": "ERROR",
                    #     "msg": "Deployment of 2.1.0 on vSphere 9.0 is not supported",
                    #     "STATUS_CODE": 500
                    # }
                    # return jsonify(d), 500
            except Exception as e:
            ...
            # if errors:
            #     current_app.logger.warn("Pre-check failed with following errors")
            #     for error in errors:
        ...
        for es in esxis:
            ...
            if time_difference > float(300):
                ...
                error_dict["TIME_SYNC_ESXI"] = "Time difference between ESXi host " + es.name + "and vCenter is greater" \
        ...
        if not refreshToken:
            ...
            if token_valdity[1] != 200:
                ...
                error_dict["MARKETPLACE_TOKEN_VALIDATION"] = "Marketplace token validation failed. Please ensure " \
        ...
        if isEnvTkgs_wcp(env):
            ...
            if checTSMEnabled(env) or checkToEnabled(env):
                ...
                if int(worker_size) < 3:
                    current_app.logger.warn("Minimum required number of worker nodes for SaaS integrations is 3, "
                                            "and recommended size is medium and above")
                    error_dict["MACHINE_COUNT_FOR_TSM_TO"] = "Minimum required number of worker nodes for " \
            ...
            if not dns_compliant[0]:
                ...
                error_dict["CLUSTER_NAME_DNS_COMPLIANT"] = "Failed while checking if Supervisor cluster name " \
                                                           "is DNS Compliant"
                # d = {
                #     "responseType": "ERROR",
                #     "msg": dns_compliant[1],
        ...
        if not isEnvTkgs_ns(env):
            ...
            if not password_check[0]:
                current_app.logger.warn("NSX ALB Password and Backup passphrase must contain a combination of 3: "
                                        "Uppercase character, Lowercase character, Numeric or Special Character.")
                error_dict["AVI_PASSWORD"] = "NSX ALB Password and Backup passphrase must contain a combination of 3: " \
                                             "Uppercase character, Lowercase character, Numeric or Special Character."
                # d = {
                #     "responseType": "ERROR",
        ...
        if isEnvTkgs_wcp(env) or isEnvTkgs_ns(env):
            ...
            if not encryption_validation[0]:
                ...
                current_app.logger.warn("Deployment with encrypted storage policies is not supported. "
                                         "Please disable encryption on given policies")
                error_dict["STORAGE_POLICY_ENCRYPTION"] = "Deployment with encrypted storage policies is not supported. " \
                                                          "Please disable encryption on given policies"
                # d = {
                #     "responseType": "ERROR",
        ...
        if error_dict.__len__() != 0:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failure while performing pre-checks",
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
...
def aviNameResolution():
    ...
    if env == Env.VCD:
        ...
        current_app.config['VC_PASSWORD'] = decode_from_b64(
            str(request.get_json(force=True)['envSpec']['aviCtrlDeploySpec']['vcenterDetails'][
