...
from common.operation.constants import AkoType, Env, MarketPlaceUrl, Tkgs_Extension_Details, Versions
...
def precheck_env():
        ...
        if not (isEnvTkgs_wcp(env) or isEnvTkgs_ns(env)):
            if checkTmcEnabled(env):
                ...
                if osFlavor[1] != 200:
                    ...
                    d = {"responseType": "ERROR", "msg": str(osFlavor[0].json["msg"]), "ERROR_CODE": 500}
                    return jsonify(d), 500
    ...
    if not skip_precheck:
        ...
        if val != "Success":
            ...
            # d = {
            #     "responseType": "ERROR",
            #     "msg": "Error: Unsupported Proxy protocol found for " + val + ", The Proxy URLs must start with
            #     http://",
            #     "STATUS_CODE": 500
            # }
            # return jsonify(d), 500
        ...
        if not (isEnvTkgs_wcp(env) or isEnvTkgs_ns(env)):
            # check for AVI L7 should be disabled when AVI essentials is selected for vsphere env
            if env == Env.VSPHERE:
                if not check_for_avi_l7(env, isShared, isWorkload):
        ...
        try:
            ...
            try:
                if str(vcVersion).startswith("9"):
                    current_app.logger.warn("Deployment of TKG 2.2.0 on vSphere 9.0 is not supported")
                    error_dict["VCENTER_VERSION_9"] = "Deployment of TKG 2.2.0 on vSphere 9.0 is not supported"
                    # d = {
                    #     "responseType": "ERROR",
    ...
    try:
        ...
        if response_login.status_code != 200:
            ...
            if "message" in body:
                ...
                error = "unknown error"
            current_app.logger.error(serviceList[0] + " login failed using Refresh_Token - %s: %s" % token, error)
            return error, 500
...
def check_for_avi_l7(env, workload, shared):
    """
    verify that if AVI license is essentials then shared and workload not supported for AVI L7 support
    """
    current_app.logger.info("Verifying L7 enablement for license")
    if env == Env.VSPHERE:
        if not (isEnvTkgs_ns(env) or isEnvTkgs_wcp(env)):
            return verify_avi_l7_on_vds_tkgm(env, workload, shared)
...
#         return jsonify(d), 200
#     else:
#         current_app.logger.error("Failed toe connect to LDAP Server")
#         d = {
#             'responseType': 'ERROR',
#             'msg': ldap_bind_status[1],
#             'STATUS_CODE': 500
