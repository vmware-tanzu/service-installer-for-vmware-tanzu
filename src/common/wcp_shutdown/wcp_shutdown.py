...
from common.common_utilities import envCheck, isEnvTkgs_ns
...
def wcp_bringup(current_user):
    ...
    if env == Env.VSPHERE:
        ...
        try:
            ...
            current_app.logger.error("Caught vmodl fault : " + error.msg)
            current_app.logger.debug(error.msg)
            d = {"responseType": "ERROR", "msg": "ERROR: Caught vmodl fault", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if not vcsession.ok:
            current_app.logger.error("Session creation is failed, please check vcenter connection")
            d = {
                "responseType": "ERROR",
                "msg": "Session creation is failed, please check vcenter connection",
                "STATUS_CODE": 500,
            }
            return jsonify(d), 500
        ...
        if not restart_status[1]:
            current_app.logger.error("Unable to restart WCP Service")
            current_app.logger.debug(restart_status[0])
            d = {"responseType": "ERROR", "msg": "Unable to restart WCP Service", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if vCenter_cluster:
            for cl in get_obj(content, vim.ComputeResource):
                if cl.name == vCenter_cluster:
                    ...
                    current_app.logger.info(
                        "Found the vSphere Cluster named: " + vCenter_cluster + " with ID " + cluster_id
                    )
                    break
                else:
                    current_app.logger.error("Could not find cluster with name: ", vCenter_cluster)
                    current_app.logger.debug("Please verify Cluster Name. Exiting the program")
                    d = {
                        "responseType": "ERROR",
        ...
        current_app.logger.error("This action can only be performed on a vsphere environment")
        d = {
            "responseType": "ERROR",
            "msg": "This action can only be performed on a vsphere environment",
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
def wcp_shutdown(current_user):
    ...
    if env == Env.VSPHERE:
        ...
        try:
            ...
            current_app.logger.error("Caught vmodl fault : " + error.msg)
            current_app.logger.debug(error.msg)
            d = {"responseType": "ERROR", "msg": "Caught vmodl fault", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        if vCenter_cluster:
            for cl in get_obj(content, vim.ComputeResource):
                if cl.name == vCenter_cluster:
                    ...
                    d = {
                        "responseType": "ERROR",
                        "msg": "Could not find cluster with name: " + vCenter_cluster,
                        "STATUS_CODE": 500,
                    }
                    return jsonify(d), 500
        ...
        if not wcp_endpoint[1]:
            ...
            d = {"responseType": "ERROR", "msg": "ERROR: Failed while fetching WCP endpoint", "STATUS_CODE": 500}
            return jsonify(d), 500
        wcp_endpoint = wcp_endpoint[0]
        current_app.logger.info("WCP Endpoint for SC is: " + wcp_endpoint)
        ...
        try:
            ...
            current_app.logger.debug("Could not login to WCP SC Endpoint. Is WCP Service running ?")
            current_app.logger.error(str(e))
            d = {"responseType": "ERROR", "msg": "Could not login to WCP SC Endpoint", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        try:
            machine_list_dict = client2.list_namespaced_custom_object(
                "cluster.x-k8s.io", "v1alpha3", "", "machines", pretty="True"
            )
        ...
        for machine in machine_list_dict["items"]:
            current_app.logger.info(
                "-Found CAPI Machine Object in SC. VM Name = {0}".format(machine["metadata"]["name"])
            )
            vm = search_index.FindByDnsName(None, machine["metadata"]["name"], True)

            if vm is None:
                current_app.logger.error("Could not find a matching VM with VC API ")
            else:
                current_app.logger.info(
                    "-Found VM matching CAPI Machine Name in VC API. VM=%s. " % vm.summary.config.name
                )
        ...
        if not stop_vc_svc[1]:
            current_app.logger.error("Unable to stop WCP Service")
            current_app.logger.debug(stop_vc_svc[0])
            d = {"responseType": "ERROR", "msg": "Unable to stop WCP Service", "STATUS_CODE": 500}
            return jsonify(d), 500
        ...
        for vmobject in vmList:
            if "SupervisorControlPlaneVM" in vmobject.summary.config.name:
                ...
                current_app.logger.info(
                    "-VM", vmobject.summary.config.name, " is running on ESX host", vmobject.runtime.host.name
                )
                ...
                for vNic in netConfig.candidateVnic:
                    if netConfig.selectedVnic.index(vNic.key) != -1:
                        # Below will return the Management IP (SC Address) for
                        #  the ESxi host where SC VP VM is running.
                        current_app.logger.info(
                            "ESX host", vmobject.runtime.host.name, " has Management IP", vNic.spec.ip.ipAddress
                        )
                        # Due to permissions limitations we need to log
                        #  into each ESXi host where the SC CP VM is running
                        # To perform the shutdown operation.
                        shutdown_status = shutdown_sc_vm(
                            25,
                            vmobject.summary.config.name,
                            vmobject.summary.config.uuid,
                            vNic.spec.ip.ipAddress,
                            esxi_user,
                            esxi_password,
                        )
                        if not shutdown_status[1]:
                            current_app.logger.error("Failed to shutdown the SC VMs")
                            current_app.logger.error(shutdown_status[0])
                            d = {"responseType": "ERROR", "msg": "Failed to shutdown the SC VMs", "STATUS_CODE": 500}
                            return jsonify(d), 500
                        break
                    else:
        ...
        current_app.logger.info("\n")
        current_app.logger.info("POST - Successfully Completed Script - Cleaning up REST Session to VC.")
        d = {"responseType": "SUCCESS", "msg": "Successfully shutdown WCP", "STATUS_CODE": 200}
        return jsonify(d), 200
    else:
        current_app.logger.error("This action can only be performed on a vsphere environment")
        d = {
...
def get_obj(content, vimtype, name=None):
    return [
        item for item in content.viewManager.CreateContainerView(content.rootFolder, [vimtype], recursive=True).view
    ]
...
def check_wcp_cluster_status(s, vcip, cluster):
    try:
        ...
        current_app.logger.error("Exception occured while fetching WCP endpoint")
        current_app.logger.debug(str(e))
        return str(e), False
...
def stop_vc_svcname(s, vcip, svc_name):
    ...
    if startup_json_response.status_code == 204:
        current_app.logger.info(
            "Successfully set WCP Service Startup to MANUAL. Response Code %s " % startup_json_response.status_code
        )
    else:
        current_app.logger.error("Unable to setup Startup for WCP Service to Manual")
        return None, False
...
def restart_vc_svcname(s, vcip, svc_name):
    ...
    if stop_json_response.status_code == 204:
        current_app.logger.info(
            "Successfully restarted WCP Service. Response Code %s " % stop_json_response.status_code
        )
        return "SUCCESS", True
    else:
        current_app.logger.error("Unable to restart WCP Service")
...
def shutdown_sc_vm(delay, vm_name, vm_uuid, vm_host_ip, esx_user, esx_password):
    try:
        ...
        current_app.logger.error("Caught error trying to shutdown VM  ")
        current_app.logger.error("Caught vmodl fault : " + error.msg)
