...
def wcp_bringup():
    ...
    if env == Env.VSPHERE:
        ...
        if not restart_status[1]:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Unable to restart WCP Service",
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
...
def wcp_shutdown():
    ...
    if env == Env.VSPHERE:
        ...
        try:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Could not login to WCP SC Endpoint",
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
        ...
        if not stop_vc_svc[1]:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Unable to stop WCP Service",
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
                ...
                for vNic in netConfig.candidateVnic:
                    if (netConfig.selectedVnic.index(vNic.key) != -1):
                        ...
                        if not shutdown_status[1]:
                            ...
                            d = {
                                "responseType": "ERROR",
                                "msg": "Failed to shutdown the SC VMs",
                                "STATUS_CODE": 500
                            }
                            return jsonify(d), 500
                        break
...
def get_si(host, user, password):
    ...
    try:
        ...
        if not service_instance:
            current_app.logger.error("Could not connect to the specified vCenter host using specified username and password")
            return None, False
        else:
