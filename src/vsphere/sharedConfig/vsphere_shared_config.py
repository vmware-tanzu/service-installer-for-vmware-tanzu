...
from vmc.sharedConfig.shared_config import deployExtensions
...
def configSharedCluster():
    ...
    if deploy_extension[1] != 200:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to deploy extension " + str(deploy_extension[0].json["msg"]),
            "STATUS_CODE": 500,
        }
        return jsonify(d), 500
...
def deploy():
    ...
    if env == Env.VCF:
        gateway_address = request.get_json(force=True)["tkgComponentSpec"]["tkgSharedserviceSpec"][
    ...
    if tmc_required.lower() == "true":
        ...
        tmc_flag = False
        current_app.logger.info("TMC registration is deactivated" + str(tmc_flag))
    else:
    ...
    shared_network_path = saas_util.get_network_Path_tmc(shared_service_network, vcenter_ip, vcenter_username, password)
    if shared_network_path is None:
    ...
    if saas_util.check_tmc_enabled():
        ...
        shared_network_folder_path = saas_util.get_network_Path_tmc(
            shared_service_network, vcenter_ip, vcenter_username, password
        )
        if checkSharedServiceProxyEnabled(env) and not SaaSUtil.check_tmc_Register(shared_cluster_name, False):
            proxy_name_state = saas_util.create_proxy_credentials_tmc(
                shared_cluster_name, "true", "shared", register=False
            )
