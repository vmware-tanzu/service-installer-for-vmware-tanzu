...
from common.operation.constants import ControllerLocation, EnvType, Paths
...
from common.util.velero_util import VeleroUtil
...
class TkgsWorkloadCluster:
    ...
    def generate_yaml_file_v1beta1(self):
        try:
            ...
            current_app.logger.info("Fetching OS Type for the chosen TKR")
            tkr_os_type = self.get_tkr_type(tkr_name)
            if tkr_os_type is None:
                current_app.logger.error("Failed to find OS Type for the TKR: " + tkr_name)
                return None
            elif tkr_os_type == "ubuntu":
                current_app.logger.info("Ubuntu OS type is chosen for deployment")
            else:
                current_app.logger.info("Photon OS type is chosen for deployment")
    ...
    def generate_yaml_file(self, workload_name):
        ...
        if vcenter_8 and cluster_kind == EnvType.TKGS_CLUSTER_CLASS_KIND:
            ...
            tkr_os_type = self.get_tkr_type(tkr_name)
            if tkr_os_type is None:
                current_app.logger.error("Failed to find OS name for the chosen TKR : " + tkr_name)
                return None
            elif tkr_os_type == "ubuntu":
                current_app.logger.info("Ubuntu OS type is chosen for deployment")
                anno_map = {"run.tanzu.vmware.com/resolve-os-image": "os-name=ubuntu"}
                meta_dict = {"name": workload_name, "namespace": name_space, "annotations": anno_map}
            else:
                current_app.logger.info("Photon OS type is chosen for deployment")
    ...
    def get_tkr_type(self, tkr_name):
        """
        Method to identify the OS Name for the chosen tkr_name
        :tkr_name : Name of the TKR selected by users
        """
        try:
            sess, status_code = self.vc_operation.get_session()
            if status_code != HTTPStatus.OK:
            ...
            header = {"Accept": "application/json", "Content-Type": "application/json", "vmware-api-session-id": sess}
            cluster_endpoint, status_code = self.tkgs_util.get_cluster_end_point(self.cluster_name, header)
            if status_code != HTTPStatus.OK:
                return cluster_endpoint, status_code
            configure_kubectl_ = self.kubectl_util.configure_kubectl(cluster_endpoint)
            if configure_kubectl_[1] != HTTPStatus.OK:
            ...
            current_app.logger.info("logging into cluster - " + cluster_endpoint)
            os.putenv("KUBECTL_VSPHERE_PASSWORD", self.vc_password)
            connect_command = KubectlUtil.CLUSTER_LOGIN.format(
                cluster_ip=cluster_endpoint, vcenter_username=self.vc_user
            ).split()
            ...
            if output[1] != 0:
                ...
                return None
            switch_context = KubectlUtil.KUBECTL_USE_TKGS_NS_CONTEXT.format(namespace=cluster_endpoint).split()
            output = runShellCommandAndReturnOutputAsList(switch_context)
            if output[1] != 0:
                current_app.logger.error("Failed to use context " + str(output[0]))
                return None

            get_labels_command_main = KubectlUtil.GET_TKR_LABELS.split()
            labels_output = runShellCommandAndReturnOutputAsList(get_labels_command_main)
            for version in labels_output[0]:
                version_list = version.split()
                if version_list[0] == tkr_name or version_list[1] == tkr_name:
                    if version_list[5].__contains__("os-name=ubuntu"):
                        return "ubuntu"
                    elif version_list[5].__contains__("os-name=photon"):
                        return "photon"
                    else:
                        current_app.logger.error("Failed to find the OS Type for chosen TKR: " + tkr_name)
                        return None

            current_app.logger.error("Failed to find the OS Type for chosen TKR: " + tkr_name)
            return None
        except Exception as e:
            current_app.logger.error("Found Exception while identifying the OS Type for the chosen TKR")
            current_app.logger.error(str(e))
            return None
...
@token_required
def create_name_space(current_user):
    # TODO session login is needed to for app_config to load with vCENTER Details
    login()
...
@token_required
def create_workload(current_user):
    # TODO session login is needed to for app_config to load with vCENTER Details
    login()
    spec_json = request.get_json(force=True)
    spec: VsphereTkgsNameSpaceMasterSpec = VsphereTkgsNameSpaceMasterSpec.parse_obj(spec_json)
    ...
    if not velero_util.enable_velero("workload", workload_cluster_name, supervisor_cluster):
        return (
            RequestApiUtil.send_error(f"Failed to enable velero on cluster {workload_cluster_name}"),
            HTTPStatus.INTERNAL_SERVER_ERROR,
        )
