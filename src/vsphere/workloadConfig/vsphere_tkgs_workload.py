...
from common.operation.constants import ControllerLocation, Paths, Tkgs_Extension_Details, TmcUser
...
def createTkgWorkloadCluster(env, vc_ip, vc_user, vc_password):
    try:
        ...
        if checkTmcEnabled(env):
            ...
            node_storage_class_input = request.get_json(force=True)["tkgsComponentSpec"]["tkgsVsphereNamespaceSpec"][
                "tkgsVsphereWorkloadClusterSpec"
            ]["nodeStorageClass"]
            ...
            allowed_storage = request.get_json(force=True)["tkgsComponentSpec"]["tkgsVsphereNamespaceSpec"][
                "tkgsVsphereWorkloadClusterSpec"
            ...
            while count < 90:
                ...
                try:
                    ...
                    current_app.logger.info(
                        "Cluster creation is not yet initiated. " "Waited for " + str(count * 30) + "s, retrying"
                    )
                    count = count + 1
                    time.sleep(30)
...
def generateYamlFile(vc_ip, vc_user, vc_password, workload_name):
    ...
    if worker_vol and control_plane_vol:
        ...
        topology_dict = {
            ...
            "workers": {"count": int(worker_node_count), "class": worker_vm_class, "storageClass": node_storage_class},
        }
    elif worker_vol:
        topology_dict = {
            "controlPlane": {"count": int(count), "class": control_plane_vm_class, "storageClass": node_storage_class},
            "workers": {
                "count": int(worker_node_count),
                "class": worker_vm_class,
                "storageClass": node_storage_class,
    ...
    ytr = dict(
        apiVersion="run.tanzu.vmware.com/v1alpha1", kind="TanzuKubernetesCluster", metadata=meta_dict, spec=spec_dict
    )
...
def createNameSpace(vcenter_ip, vcenter_username, password):
    try:
        ...
        try:
            ...
            storage_limit = ""
            current_app.logger.info(
                "Storage Request Limit is not provided, will continue without setting Custom " "Storage Request Limit"
            )
...
def create_workload_network(vCenter, vc_user, password, cluster_id, network_name):
    ...
    sess = requests.post(
        "https://" + str(vCenter) + "/rest/com/vmware/cis/session", auth=(vc_user, password), verify=False
    )
    if sess.status_code != 200:
...
def checkWorkloadNetwork(vcenter_ip, vc_user, password, cluster_id, workload_network):
    sess = requests.post(
        "https://" + str(vcenter_ip) + "/rest/com/vmware/cis/session", auth=(vc_user, password), verify=False
    )
    if sess.status_code != 200:
        current_app.logger.error("Connection to vCenter failed")
...
def attach_proxy_tkgs_workload_to_tmc():
    ...
    if proxyEnabled:
        ...
        try:
            ...
            header = {"Content-Type": "application/json", "Accept": "application/json", "Authorization": access_token}
            url = (
                tmc_url
                + "/v1alpha1/managementclusters/"
                + mgmt
