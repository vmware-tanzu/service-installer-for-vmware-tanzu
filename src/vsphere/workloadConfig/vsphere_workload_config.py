...
sys.path.append(".../")
from vmc.managementConfig.management_config import getDetailsOfServiceEngine, getClusterUUid, downloadSeOva, \
    updateNewCloudSeGroup, \
    generateSeOva, generateToken, getVipNetwork

from vsphere.managementConfig.vsphere_management_config import createVipNetwork, createClusterVipNetwork
from common.lib.govc_client import GovcClient
from common.util.local_cmd_helper import LocalCmdHelper
from vmc.workloadConfig.workload_config import getIPamDetails, updateIpamWithDataNetwork
from common.operation.vcenter_operations import createResourcePool, create_folder, checkforIpAddress, getSi, \
    checkVmPresent
from vsphere.managementConfig.vsphere_management_config import controllerDeployment, listAllServiceEngine, \
    changeSeGroupAndSetInterfaces
from common.operation.constants import ResourcePoolAndFolderName,KubernetesOva, Vcenter, GroupNameCgw, GroupNameMgw, FirewallRuleMgw, \
    ControllerLocation, Env, Policy_Name, Paths, TmcUser
from common.common_utilities import isAviHaEnabled, tmcBodyClusterCreation, getDomainName, getTier1Details, \
    grabNsxtHeaders, getList, \
    obtain_second_csrf, \
    ...
    checkPinnipedDexServiceStatus, createRbacUsers, createClusterFolder, enable_data_protection_velero, \
    checkDataProtectionEnabledVelero, getClusterID, checkAviL7EnabledForWorkload, ping_test
...
from vsphere.managementConfig.vsphere_management_config import updateIpam_profile, create_virtual_service, getIpam, \
    enableDhcpForManagementNetwork, \
    updateNetworkWithIpPools, getNetworkDetails, createSECloud_Arch, getClusterUrl, createSECloud_Arch, getNetworkUrl, \
    seperateNetmaskAndIp, createSECloud, getCloudConnectUser, fetchTier1GatewayId
...
def networkConfig():
    ...
    if isNonOrchestrated:
        ...
        status = create_archostrated(ip, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
                                     workload_vip, aviVersion, env)
        if status[1] != 200:
        ...
        if csrf2 is None:
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to get csrf from new password",
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
        ...
        if get_se_cloud[0] == "NOT_FOUND":
            ...
            cloud_se = createSECloud(ip, csrf2, cloud_url, group_name, cluster_status[0],
                                     data_store, aviVersion, "Workload")
        ...
        getManagementDetails_data_pg = getNetworkDetails(ip, csrf2, get_management_data_pg[0], startIp, endIp,
                                                         prefixIpNetmask[0], prefixIpNetmask[1], aviVersion, env)
        ...
        if not createClusterFolder(workload_cluster_name):
            ...
            return jsonify(d), 500
        current_app.logger.info(
            "The config files for workload cluster will be located at: " + Paths.CLUSTER_PATH + workload_cluster_name)
        ...
        createAkoFile(ip, workload_cluster_name, wip[0], data_network_workload, cluster_vip_name, workload_network_name,
                      cluster_vip_cidr_[0], tier1, env)
        lisOfCommand = ["kubectl", "apply", "-f",
                        Paths.CLUSTER_PATH + workload_cluster_name + "/ako_vsphere_workloadset1.yaml",
                        "--validate=false"]
        ...
        if env == Env.VCF:
            ...
            if vrf_vip[1] != "Already_Configured":
                ...
                if ad[0] is None:
                    ...
                    return jsonify(d), 500
            vrf_url = vrf_vip[0]
            virtual_service, error = create_virtual_service(ip, csrf2, uuid, group_name, get_management_data_pg[0], 2,
                                                            tier1, vrf_url, aviVersion)
...
def create_name_space():
    ...
    if name_space[0] is None:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to create namespace " + str(name_space[1]),
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
...
def create_workload():
    ...
    if checkTmcEnabled(env):
        ...
        if checkDataProtectionEnabledVelero(env, "workload"):
            ...
            switch_context_workload = ["kubectl", "vsphere", "login", "--server", cluster_endpoint,
                                       "--vsphere-username",
                                       vcenter_username, "--tanzu-kubernetes-cluster-name", workload_cluster_name,
                                       "--tanzu-kubernetes-cluster-namespace", name_space, "--insecure-skip-tls-verify"]
...
def deploy():
    ...
    if env == Env.VCF:
        try:
            ...
            if ping_test("ping -c 1 " + gatewayAddress.split("/")[0]) != 0:
                current_app.logger.warn("Ping test failed for " + gatewayAddress + " gateway. It is Recommended to fix this "
    ...
    try:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get ssh key from config file " + str(e),
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
        ...
        if not verifyPodsAreRunning(workload_cluster_name, command_status[0], RegexPattern.running):
            ...
            if not checkTmcEnabled(env):
                ...
                if checkTmcEnabled(env):
                    ...
                    provisoner  = "default"
                    management_url = tmc_url + "/v1alpha1/managementclusters/"+management_cluster+"/provisioners/"+provisoner+"/tanzukubernetesclusters"
                    ...
                    register_payload = tmcBodyClusterCreation(management_cluster, provisoner, workload_cluster_name,
                                                              clusterGroup, pod_cidr, service_cidr, re,
                                                              vcenter_ip, cpu, disk, memory, machineCount,
                                                              AkoType.type_ako_set,
                                                              version, datacenter_path, datastore_path,
                                                              workload_folder_path, workload_network_folder_path,
                                                              workload_resource_path, osName, osVersion, "amd64",templatePath,
                                                              proxy_name, controlPlaneNodeCount)
                    modified_payload = json.dumps(register_payload,ensure_ascii=True, sort_keys=True, indent=4)
                    response = requests.request("POST", management_url, headers=headers_, data=modified_payload,
                                                verify=False)
                    if response.status_code != 200:
                        current_app.logger.error(
                            "Failed to run command to create shared cluster " + str(response.text))
                        d = {
                            "responseType": "ERROR",
...
def createAkoFile(ip, cluster_name, wipCidr, tkgMgmtDataPg, cluster_vip_name, workload_network,
    ...
    if checkAviL7EnabledForWorkload(env):
        ...
        extra_config = dict(cniPlugin="antrea",
                            ...
                            ingress=dict(disableIngressClass=False,
        ...
        if env == Env.VCF:
            ...
            extra_config = dict(cniPlugin="antrea",
                                ...
                                layer7Only=False,
                                networksConfig=dict(enableRHI=False, nsxtT1LR=tier1_path),
                                ingress=dict(defaultIngressController=True, disableIngressClass=True,
                                             nodeNetworkList=lis_))
    data = dict(
...
def changeNetworks(vcenter_ip, vcenter_username, password, engine_name):
    ...
    workload_network_name = request.get_json(force=True)['tkgWorkloadComponents']['tkgWorkloadNetworkName']
    change_VM_Net = ["govc", "vm.network.change", "-vm=" + engine_name, "-net", workload_network_name,
                     "ethernet-2"]
...
def create_archostrated(ip_, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
                        workload_vip,
                        aviVersion, env):
    govc_client = GovcClient(current_app.config, LocalCmdHelper())
    ip = govc_client.get_vm_ip(ip_,
                               datacenter_name=data_center)
    if ip is None:
        current_app.logger.error("Failed to get IP of AVI controller")
        d = {
            "responseType": "ERROR",
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        ...
        cloud_se = createSECloud_Arch(ip, csrf2, cloud_url, Cloud.SE_WORKLOAD_GROUP_NAME_VSPHERE,
                                      aviVersion, "Workload")
        if cloud_se[0] is None:
            current_app.logger.error("Failed to create service engine cloud " + str(cloud_se[1]))
            d = {
    ...
    if new_cloud_status[0] is None:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to update Ipam" + str(new_cloud_status[1]),
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    ...
    dep = controllerDeployment(ip, csrf2, data_center, data_store, cluster_name, vcenter_ip, vcenter_username, password,
                               ...
                               ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME2.replace("vmc", "vsphere"), 3,
                               Type.WORKLOAD, 1, aviVersion)
    if dep[1] != 200:
    ...
    cluster_vip_cidr = request.get_json(force=True)["tkgComponentSpec"]['tkgClusterVipNetwork'][
        "tkgClusterVipNetworkGatewayCidr"]
    cluster_vip_cidr_ = getVipNetworkIpNetMask(ip, csrf2, cluster_vip_cidr, aviVersion)
    if env == Env.VCF:
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
    ...
    if status[1] != 0:
        if not str(status[0]).__contains__("already has a value"):
            ...
            d = {
                "responseType": "ERROR",
                "msg": "Failed to apply Ako " + str(status[0]),
                "STATUS_CODE": 500
            }
            return jsonify(d), 500
        current_app.logger.info("Applied Ako successfully")
...
def config_service_engines(ip, csrf2, vcenter_ip, vcenter_username, password, data_center, aviVersion):
    ...
    while count < 120:
        try:
            ...
            seIp3 = govc_client.get_vm_ip(ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME.replace("vmc", "vsphere"),
                                          datacenter_name=data_center)
    ...
    if not found:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Controller 3 is not up, failed to get IP",
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    count = 0
    ...
    while count < 120:
        try:
            ...
            seIp4 = govc_client.get_vm_ip(ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME2.replace("vmc", "vsphere"),
                                          datacenter_name=data_center)
    ...
    if not found:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Controller 4 is not up, failed to get IP ",
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    urlFromServiceEngine1 = listAllServiceEngine(ip, csrf2, 3, seIp3,
                                                 ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME.replace("vmc",
                                                                                                        "vsphere"),
                                                 vcenter_ip, vcenter_username, password, aviVersion)
    if urlFromServiceEngine1[0] is None:
        ...
        return jsonify(d), 500
    urlFromServiceEngine2 = listAllServiceEngine(ip, csrf2, 3, seIp4,
                                                 ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME2.replace("vmc",
                                                                                                         "vsphere"),
                                                 vcenter_ip, vcenter_username, password, aviVersion)
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        ...
        se_cloud_url = get_se_cloud[0]
    change = changeNetworks(vcenter_ip, vcenter_username, password,
                            ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME.replace("vmc", "vsphere"))
    if change[1] != 200:
        current_app.logger.error("Failed to change Network " + str(change[0]))
        d = {
            "responseType": "ERROR",
            "msg": "Failed to change Network " + str(change[0]),
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    ...
    se_engines = changeSeGroupAndSetInterfaces(ip, csrf2, urlFromServiceEngine2[0], se_cloud_url,
                                               "detailsOfServiceEngine4.json",
                                               vcenter_ip, vcenter_username,
                                               password, ControllerLocation.CONTROLLER_SE_WORKLOAD_NAME2.replace("vmc",
                                                                                                                 "vsphere"),
                                               Type.WORKLOAD, 2, aviVersion)
    if se_engines[0] is None:
        current_app.logger.error(
    ...
    d = {
        "responseType": "ERROR",
        "msg": "Configured service engines successfully",
        "STATUS_CODE": 200
    }
    return jsonify(d), 200
