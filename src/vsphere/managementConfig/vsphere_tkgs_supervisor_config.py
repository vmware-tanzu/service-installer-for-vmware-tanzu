...
def config_wcp():
    ...
    if not (tkgs_util.is_env_tkgs_wcp(env) or tkgs_util.is_env_tkgs_ns(env)):
        license_type = str(spec.tkgsComponentSpec.aviComponents.typeOfLicense)
    if tkgs_util.is_env_tkgs_wcp(env):
...
def enable_wcp():
    ...
    if not tkgs_util.is_env_tkgs_wcp(env):
    ...
    if saas_util.check_tmc_enabled():
        ...
        if tmc_register_response[1] != HTTPStatus.OK:
            ...
            response = RequestApiUtil.create_json_object(
                "Supervisor cluster TMC registration failed " + str(tmc_register_response[0]),
                "ERROR",
                HTTPStatus.INTERNAL_SERVER_ERROR,
            )
            return response, HTTPStatus.INTERNAL_SERVER_ERROR
