    ...
    if isNonOrchestrated:
        ...
        if env == Env.VSPHERE:
            ...
            if getManagementDetails_data_pg[0] == "AlreadyConfigured":
                ...
                if update_resp[0] != 200:
                    ...
                    return jsonify(d), 500
        mgmt_pg = request.get_json(force=True)["tkgComponentSpec"]["tkgClusterVipNetwork"]["tkgClusterVipNetworkName"]
        ...
        if get_se_cloud_workload[0] == "NOT_FOUND":
            ...
            cloud_se_workload = createSECloud_Arch(
                ip, csrf2, cloud_url, clo, aviVersion, "Workload", data_store, license_type=license_type
            )
...
def configTkgMgmt():
    ...
    vsSpec: spec_obj = spec_obj.parse_obj(json_dict)

    TanzuUtil(env=env, spec=vsSpec)
        ...
        if saas_util.check_tmc_enabled():
            ...
            if checkMgmtProxyEnabled(env):
                state = saas_util.register_management_cluster_tmc(
                    management_cluster, "true", "management", clusterGroup
                )
            else:
                state = saas_util.register_management_cluster_tmc(
                    management_cluster, "false", "management", clusterGroup
                )
            if state[1] != 200:
                current_app.logger.error("Failed to register on TMC " + state[0])
                d = {"responseType": "ERROR", "msg": "Failed to register on TMC " + state[0], "STATUS_CODE": 500}
                return jsonify(d), 500
    if checkAirGappedIsEnabled(env):
...
def config_orchestrated(
    ...
    cluster_vip = request.get_json(force=True)["tkgComponentSpec"]["tkgClusterVipNetwork"]["tkgClusterVipNetworkName"]
...
def createSECloud_Arch(
    ip, csrf2, newCloudUrl, seGroupName, aviVersion, se_name_prefix, datastore, license_type="enterprise"
):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
    ...
    json_object = getNewBody(newCloudUrl, seGroupName, se_name_prefix, datastore, license_type=license_type)
    url = "https://" + ip + "/api/serviceenginegroup"
    response_csrf = requests.request("POST", url, headers=headers, data=json_object, verify=False)
    if response_csrf.status_code != 201:
        return None, response_csrf.text
...
def getNewBody(newCloudUrl, seGroupName, se_name_prefix, datastore, license_type):
    if str(datastore).__contains__("/"):
        datastore = datastore[datastore.rindex("/") + 1 :]
