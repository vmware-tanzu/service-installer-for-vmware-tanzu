...
def configCloud():
    ...
    if isNonOrchestrated:
        status = config_orchestrated(env, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
                                     aviVersion)
        if status[1] != 200:
            current_app.logger.error("Failed to config management cluster " + str(status[0].json["msg"]))
            d = {
                "responseType": "ERROR",
        ...
        if isEnvTkgs_wcp(env):
            ...
            if get_se_cloud[0] == "NOT_FOUND":
                isGen = True
                current_app.logger.info("Creating New se cloud " + seGroupName)
                if env == Env.VCF:
                    ...
                    if nsx_cloud_info[0] is None:
                        ...
                        return jsonify(d), 500
                    cloud_se = createNsxtSECloud(ip, csrf2, cloud_url, seGroupName, nsx_cloud_info[1], aviVersion)
                else:
            ...
            if get_se_cloud_workload[0] is None:
                current_app.logger.error("Failed to get se cloud status " + str(get_se_cloud_workload[1]))
                d = {
                    "responseType": "ERROR",
            ...
            if get_se_cloud_workload[0] == "NOT_FOUND":
                isGen = True
                current_app.logger.info("Creating New se cloud " + clo)
                cloud_se_workload = createSECloud_Arch(ip, csrf2, cloud_url, clo,
                                      aviVersion)
                if cloud_se_workload[0] is None:
                    current_app.logger.error("Failed to create se cloud " + str(cloud_se_workload[1]))
                    d = {
                        "responseType": "ERROR",
                        "msg": "Failed to create  se cloud " + str(cloud_se_workload[1]),
                        "STATUS_CODE": 500
                    }
                    return jsonify(d), 500
        ...
        # return jsonify(d), 500
            ...
            if env == Env.VSPHERE:
                ...
                if get_dns[0] is None:
                    current_app.logger.error("Failed to get se Ipam " + str(get_dns[1]))
                    d = {
                        "responseType": "ERROR",
                        "msg": "Failed to get ipam " + str(get_dns[1]),
                        "STATUS_CODE": 500
                    }
                    return jsonify(d), 500
...
def createNsxtSECloud(ip, csrf2, newCloudUrl, seGroupName, nsx_cloud_info, aviVersion):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
            ...
            if identity_mgmt_type.lower() == "oidc":
                ...
                FileHelper.write_to_file(
                    t.render(config=vsSpec, avi_cert=get_base64_cert(ip), ip=ip, wpName=wpName,
                             ...
                             osVersion=osVersion,
                             avi_version=avi_version, env=env,tier1_path=tier1_path,vip_cidr=cluster_vip_cidr,vip_name=cluster_vip_name,management_group=mgmt_group_name,workload_group=workload_group_name,
...
def config_orchestrated(env, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        ...
        cloud_se = createSECloud_Arch(ip, csrf2, cloud_url, Cloud.SE_GROUP_NAME.replace("vmc", "vsphere"), aviVersion)
        if cloud_se[0] is None:
            current_app.logger.error("Failed to create se cloud " + str(cloud_se[1]))
            d = {
                "responseType": "ERROR",
...
def createSECloud_Arch(ip, csrf2, newCloudUrl, seGroupName, aviVersion):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
