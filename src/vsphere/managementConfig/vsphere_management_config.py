...
from common.operation.constants import ResourcePoolAndFolderName, Cloud, AkoType, CIDR, TmcUser, Type, Avi_Version, \
    RegexPattern, Env, KubernetesOva, NSXtCloud, Versions
import os
from common.common_utilities import isAviHaEnabled, create_virtual_service, checkAndWaitForAllTheServiceEngineIsUp, \
    ...
    checkPinnipedServiceStatus, checkPinnipedDexServiceStatus, createRbacUsers, createClusterFolder, isEnvTkgm, \
    update_template_in_ova
from common.certificate_base64 import getBase64CertWriteToFile
from common.replace_value import generateVsphereConfiguredSubnets, generateVsphereConfiguredSubnetsForSe, generateVsphereConfiguredSubnetsForSeandVIP, \
...
from vmc.managementConfig.management_config import downloadSeOva, generateToken, getVipNetwork, updateNewCloudSeGroup, \
    generateSeOva, getClusterUUid, getDetailsOfServiceEngine, getConnectedStatus
...
def configCloud():
    ...
    if isNonOrchestrated:
        status = config_orchestrated(env, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
                                     aviVersion)
        if status[1] != 200:
        ...
        if isEnvTkgs_wcp(env):
            ...
            if env ==  Env.VSPHERE:
                getManagementDetails = getNetworkDetails(ip, csrf2, get_management[0], startIp, endIp, prefixIpNetmask[0],
                                                     prefixIpNetmask[1], aviVersion, env)
            ...
            if get_se_cloud[0] == "NOT_FOUND":
                ...
                if env == Env.VCF:
                    ...
                    if nsx_cloud_info[0] is None:
                        ...
                        return jsonify(d), 500
                    cloud_se = createNsxtSECloud(ip, csrf2, cloud_url, seGroupName, nsx_cloud_info[1], aviVersion, "Mgmt")
                else:
                    cloud_se = createSECloud(ip, csrf2, cloud_url, seGroupName, cluster_status[0], data_store,
                                         aviVersion, "Mgmt")
            ...
            if get_se_cloud_workload[0] == "NOT_FOUND":
                ...
                cloud_se_workload = createSECloud_Arch(ip, csrf2, cloud_url, clo,
                                      aviVersion, "Workload")
        ...
        # return jsonify(d), 500
            ...
            if env == Env.VSPHERE:
                ...
                if vrf_vip[1] != "Already_Configured":
                    ...
                    if ad[0] is None:
                        ...
                        return jsonify(d), 500
                vrf_url = vrf_vip[0]
                getManagementDetails = getNsxTNetworkDetails(ip, csrf2, get_management[0], startIp, endIp,
                                                         prefixIpNetmask[0],
                                                         prefixIpNetmask[1], aviVersion)
                if getManagementDetails[0] is None:
                    current_app.logger.error("Failed to get AVI management network details " + str(getManagementDetails[2]))
                    d = {
                        "responseType": "ERROR",
            ...
            if default[0] is None:
                ...
                d = {
                    "responseType": "ERROR",
                    "msg": "Failed to get "+cloudName+" cloud status",
                    "STATUS_CODE": 500
                }
                return jsonify(d), 500
...
def enable_wcp():
    ...
    if not isEnvTkgs_wcp(env):
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Wrong env provided, wcp can be only enabled on TKGS",
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
    # aviVersion = get_avi_version(env)
    ...
    subs_lib_name = request.get_json(force=True)['tkgsComponentSpec']['tkgsMgmtNetworkSpec']['subscribedContentLibraryName']
...
def enableDhcpForManagementNetwork(ip, csrf2, name, aviVersion):
    ...
    if getNetwork[0] is None:
        ...
        d = {
            "responseType": "ERROR",
            "msg": "Failed to get network url " + name,
            "STATUS_CODE": 500
        }
        return jsonify(d), 500
...
def getNetworkDetails(ip, csrf2, managementNetworkUrl, startIp, endIp, prefixIp, netmask, aviVersion, env="vsphere"):
    ...
    if env == Env.VSPHERE:
        ...
        generateVsphereConfiguredSubnetsForSeandVIP("managementNetworkDetails.json", startIp, endIp, prefixIp,
                                     int(netmask))
...
def getNetworkDetailsVip(ip, csrf2, vipNetworkUrl, startIp, endIp, prefixIp, netmask, aviVersion, env="vsphere"):
    ...
    if env == Env.VSPHERE:
        ...
        generateVsphereConfiguredSubnetsForSeandVIP("vipNetworkDetails.json", startIp, endIp, prefixIp,
                                     int(netmask))
    return "SUCCESS", 200, details
...
def getSeNewBody(newCloudUrl, seGroupName, clusterUrl, dataStore, se_name_prefix):
...
def createSECloud(ip, csrf2, newCloudUrl, seGroupName, clusterUrl, dataStore, aviVersion, se_name_prefix):
    ...
    json_object = getSeNewBody(newCloudUrl, seGroupName, clusterUrl, dataStore, se_name_prefix)
            ...
            if identity_mgmt_type.lower() == "oidc":
                ...
                FileHelper.write_to_file(
                    t.render(config=vsSpec, avi_cert=get_base64_cert(ip), ip=ip, wpName=wpName,
                             ...
                             osVersion=osVersion,
                             avi_version=avi_version, env=env,tier1_path=tier1_path,vip_cidr=cluster_vip_cidr,
                             tkg_cluster_vip_network_name=tkg_cluster_vip_network_name,management_group=mgmt_group_name,
...
def updateVipNetworkIpPools(ip, csrf2, get_vip, aviVersion):
    try:
        ...
        d = {
            "responseType": "SUCCESS",
            "msg": "Updated VIP IP pools successfully",
            "STATUS_CODE": 200
        }
        return jsonify(d), 200
    except Exception as e:
...
def config_orchestrated(env, vcenter_ip, vcenter_username, password, data_center, data_store, cluster_name,
                        avi_version):
    try:
    ...
    if get_se_cloud[0] == "NOT_FOUND":
        ...
        cloud_se = createSECloud_Arch(ip, csrf2, cloud_url, Cloud.SE_GROUP_NAME.replace("vmc", "vsphere"), aviVersion, "Mgmt")
        if cloud_se[0] is None:
            current_app.logger.error("Failed to create service engine cloud " + str(cloud_se[1]))
            d = {
                "responseType": "ERROR",
    ...
    if get_wip[0] == "NOT_FOUND":
        ...
        vip_net = createVipNetwork(ip, csrf2, cloud_url, data_network,
                                   Type.MANAGEMENT, aviVersion)
    ...
    if Tkg_version.TKG_VERSION == "2.1":
        cluster_vip = request.get_json(force=True)['tkgComponentSpec']["tkgClusterVipNetwork"][
            "tkgClusterVipNetworkName"]
        get__cluster_wip = getVipNetwork(ip, csrf2, cluster_vip,
                                         aviVersion)
        ...
        if get__cluster_wip[0] == "NOT_FOUND":
            isGen = True
            current_app.logger.info(
                "Creating New cluster VIP network " + cluster_vip)
            vip_net = createClusterVipNetwork(ip, csrf2, cloud_url,
                                              cluster_vip,
                                              aviVersion)
            if vip_net[0] is None:
            ...
            current_app.logger.info(
                "Created New cluster VIP network " + cluster_vip)
        else:
    ...
    prefixIpNetmask = seperateNetmaskAndIp(
        request.get_json(force=True)["tkgComponentSpec"]['tkgClusterVipNetwork'][
            "tkgClusterVipNetworkGatewayCidr"])
    getVIPNetworkDetails = getNetworkDetailsVip(ip, csrf2, wip_cluster_url, startIp, endIp, prefixIpNetmask[0],
                                                prefixIpNetmask[1], aviVersion, env)
    ...
    if get_ipam[0] == "NOT_FOUND":
        ...
        ipam = createIpam_Arch(ip, csrf2, wip_url, wip_cluster_url, Cloud.IPAM_NAME.replace("vmc", "vsphere"),
                               aviVersion)
        if ipam[0] is None:
            current_app.logger.error("Failed to create Ipam " + str(ipam[1]))
            d = {
    ...
    dep = controllerDeployment(ip, csrf2, data_center, data_store, cluster_name, vcenter_ip, vcenter_username, password,
                               se_cloud_url, "./vsphere/managementConfig/se.json", "detailsOfServiceEngine1.json",
                               "detailsOfServiceEngine2.json",
                               ControllerLocation.CONTROLLER_SE_NAME.replace("vmc", "vsphere"),
                               ControllerLocation.CONTROLLER_SE_NAME2.replace("vmc", "vsphere"), 1, Type.MANAGEMENT, 0,
                               aviVersion)
    if dep[1] != 200:
...
def controllerDeployment(ip, csrf2, data_center, data_store, cluster_name, vcenter_ip, vcenter_username, password,
    ...
    if csrf2 is None:
        ...
        return jsonify(d), 500
    urlFromServiceEngine1 = listAllServiceEngine(ip, csrf2, seCount, seIp1, controllerName1, vcenter_ip,
                                                 vcenter_username, password, aviVersion)
    if urlFromServiceEngine1[0] is None:
        ...
        return jsonify(d), 500
    urlFromServiceEngine2 = listAllServiceEngine(ip, csrf2, seCount, seIp2, controllerName2, vcenter_ip,
                                                 vcenter_username, password, aviVersion)
...
def pushSeOvaToVcenter(vcenter_ip, vcenter_username, password, data_center, data_store,
    ...
    if parent_resourcepool is not None:
        rp_pool = data_center + "/host/" + cluster_name + "/Resources/" + parent_resourcepool + "/" + ResourcePoolAndFolderName.AVI_RP.replace(
            "vmc", "vsphere")
    else:
        rp_pool = data_center + "/host/" + cluster_name + "/Resources/" + ResourcePoolAndFolderName.AVI_RP.replace(
            "vmc", "vsphere")
    replaceValueSysConfig("./vsphere/managementConfig/importSeOva-vc.json", "Name", "name",
                          ControllerLocation.SE_OVA_TEMPLATE_NAME.replace("vmc", "vsphere") + "_" + avi_uuid)
    ova_deploy_command = [
        "govc", "import.ova", "-options", "./vsphere/managementConfig/importSeOva-vc.json", "-dc=" + data_center,
                                                                                            "-ds=" + data_store,
                                                                                            "-folder=" + ResourcePoolAndFolderName.Template_Automation_Folder.replace(
                                                                                                "vmc", "vsphere"),
                                                                                            "-pool=/" + rp_pool,
...
def createSECloud_Arch(ip, csrf2, newCloudUrl, seGroupName, aviVersion, se_name_prefix):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
    ...
    json_object = getNewBody(newCloudUrl, seGroupName, se_name_prefix)
    url = "https://" + ip + "/api/serviceenginegroup"
    response_csrf = requests.request("POST", url, headers=headers, data=json_object, verify=False)
    if response_csrf.status_code != 201:
        return None, response_csrf.text
...
def getNewBody(newCloudUrl, seGroupName, se_name_prefix):
    body = {
        "max_vs_per_se": 10,
        "min_scaleout_per_vs": 2,
        "max_scaleout_per_vs": 4,
...
def createClusterVipNetwork(ip, csrf2, newCloudUrl, name, aviVersion):
    ipNetMask = seperateNetmaskAndIp(
        request.get_json(force=True)['tkgComponentSpec']['tkgClusterVipNetwork'][
            'tkgClusterVipNetworkGatewayCidr'])
    start_ip = request.get_json(force=True)['tkgComponentSpec']['tkgClusterVipNetwork'][
        'tkgClusterVipIpStartRange']
    end_ip = request.get_json(force=True)['tkgComponentSpec']['tkgClusterVipNetwork'][
        'tkgClusterVipIpEndRange']
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
...
def deploySeEngines(vcenter_ip, vcenter_username, password, ip, aviAuthToken, clusterUUid, data_center, data_store,
    ...
    ova_deploy_command = [
        "govc", "import.ova", "-options", file_name, "-dc=" + data_center,
                                                     "-ds=" + data_store,
                                                     "-folder=" + ResourcePoolAndFolderName.AVI_Components_FOLDER.replace(
                                                         "vmc", "vsphere"),
                                                     "-pool=/" + rp_pool,
        current_app.config['se_ova_path']]
    if type == Type.WORKLOAD:
        ...
        network_disconnect = ["govc", "device.disconnect", "-vm", engine_name, "ethernet-2",
                              "ethernet-3", "ethernet-4", "ethernet-6", "ethernet-7", "ethernet-8",
                              "ethernet-9"]

    else:
        if Tkg_version.TKG_VERSION == "2.1":
            network_connect = ["govc", "device.connect", "-vm", engine_name, "ethernet-0",
                               "ethernet-1", "ethernet-2", "ethernet-4"]
            network_disconnect = ["govc", "device.disconnect", "-vm", engine_name,
                                  "ethernet-5", "ethernet-3", "ethernet-6", "ethernet-7", "ethernet-8", "ethernet-9"]
        else:
            network_connect = ["govc", "device.connect", "-vm", engine_name, "ethernet-0",
...
def createNewCloud_Arch(ip, csrf2, aviVersion):
    ...
    body = {"name": Cloud.CLOUD_NAME.replace("vmc", "vsphere"),
            "vtype": "CLOUD_NONE",
            "dhcp_enabled": False,
            "mtu": 1500,
            "prefer_static_routes": False,
