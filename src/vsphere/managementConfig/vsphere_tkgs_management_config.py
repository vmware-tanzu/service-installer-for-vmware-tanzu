...
from common.common_utilities import fetchContentLibrary, getClusterID, getAviCertificate, getLibraryId, getCountOfIpAdress, \
    ...
    if proxyEnabled:
        try:
            ...
            proxyPath = request.get_json(force=True)['tkgsComponentSpec']['tkgServiceConfig']['additionalTrustedCAs'][
                'paths']
            proxyEndpoints = \
            request.get_json(force=True)['tkgsComponentSpec']['tkgServiceConfig']['additionalTrustedCAs'][
                'endpointUrls']
            if proxyPath:
                proxyCert = proxyPath
                isProxyAddCert = True
