...
def configTkgsCloud(ip, csrf2, aviVersion, license_type="enterprise"):
    try:
        ...
        startIp = request.get_json(force=True)["tkgsComponentSpec"]["aviMgmtNetwork"]["aviMgmtServiceIpStartRange"]
        ...
        if getManagementDetails_vip[0] == "AlreadyConfigured":
            ...
            update_resp = updateNetworkWithIpPools(ip, csrf2, get_vip[0], "managementNetworkDetails.json", aviVersion)
            if update_resp[0] != 200:
        ...
        if get_ipam[0] == "NOT_FOUND":
            ...
            ipam = createIpam(ip, csrf2, get_management[0], get_vip[0], Cloud.IPAM_NAME_VSPHERE, aviVersion)
            if ipam[0] is None:
...
def createIpam(ip, csrf2, managementNetworkUrl, vip_network, name, aviVersion):
    ...
    body = {
        "name": name,
        "internal_profile": {"ttl": 30, "usable_networks": [{"nw_ref": managementNetworkUrl}, {"nw_ref": vip_network}]},
...
def updateSeEngineDetails(ip, csrf2, seUrl, clusterUrl, aviVersion, license_type):
    headers = {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Cookie": csrf2[1],
...
def enableWCP(ip, csrf2, aviVersion):
    try:
        ...
        if isRuning:
            ...
            ephemeral_storage_policy_name = request.get_json(force=True)["tkgsComponentSpec"]["tkgsStoragePolicySpec"][
            ...
            master_storage_policy_name = request.get_json(force=True)["tkgsComponentSpec"]["tkgsStoragePolicySpec"][
                "masterStoragePolicy"
            ]
...
def configureTkgConfiguration(vCenter_user, vc_password, cluster_endpoint):
    ...
    if proxyEnabled:
        try:
            ...
            proxyEndpoints = request.get_json(force=True)["tkgsComponentSpec"]["tkgServiceConfig"][
